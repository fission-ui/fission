---
title: Fission 0.4.1: runtime shell fixes, static server apps, and route actions
authors:
  - fission
tags:
  - release
...
The main runtime-shell addition is route action plumbing for host shells. Fission 0.4.0 already introduced scoped raw action support with `ActionScopeId`, `ActionScope`, and `ActionInput::ScopedRaw`. Fission 0.4.1 builds on that foundation by adding shell route-change actions and route-location data that host shells can dispatch into normal application reducers.

## Runtime shell route actions

0.4.1 adds `RouteLocation` and `ShellRouteChanged`.
...
This matters for shells that mount generated or remote guest UI. Normal applications still use typed actions and reducers. Host shells can combine the existing scoped raw action surface with route-change actions to keep mounted guest UI associated with the correct runtime and route state.
