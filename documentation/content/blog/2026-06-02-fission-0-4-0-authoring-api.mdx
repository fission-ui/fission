...
Fission 0.4.0 is the authoring API release. It changes how application and library authors write widgets, how state is accessed during component conversion, and how local retained state is modeled. The work is tracked in [PR #63](https://github.com/fission-ui/fission/pull/63).
...
0.4.0 also includes the base scoped raw action primitives used by host shells that mount or compose isolated subtrees. `ActionScopeId` identifies a dispatch scope, `ActionScope` annotates a subtree in the lowered semantics tree, and `ActionInput::ScopedRaw` lets shell/runtime code inspect the nearest scope and target before recovering the original unscoped input. These primitives are intentionally low-level; they establish the raw dispatch boundary, while later shell releases can add higher-level route or guest-runtime policies on top.
...
1. Replace public widget-authoring-boundary implementations with `impl From<Component> for Widget`.
