...
Do not treat desktop as the only real loop. It is often the fastest place to start, but browser, mobile, terminal, static site, and server-rendered site targets are first-class delivery paths. Choose the host that can actually answer the current question.
