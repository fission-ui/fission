...
If the Web app uses a Cargo feature for a development-only data source or
diagnostic surface, select it explicitly in the lifecycle command:

```bash
fission run --target web --features fixtures
```

The same `--features` and `--no-default-features` options work with
`fission build --target web` and `fission test --target web`. Fission forwards
them to `wasm-pack`; it does not guess a feature from custom environment
variables.
...
| Does a command-line workflow need persistent interaction? | Terminal | The terminal shell proves keyboard, mouse, logs, and scrollback behavior. |
...
| Does an ecommerce, account, dashboard, or portal page need request-time data? | Server-rendered site | The server target answers sessions, cache policy, server jobs, signed actions, workers, and islands. |
