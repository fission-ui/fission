...
```rust
use fission::charts::{Axis, BarSeries, Chart, Dataset, Encode, LineSeries};
...
3. Component conversion creates a new chart snapshot from that state.
...
When the user scrolls or zooms backward, the chart should emit an interaction event or the surrounding controls should dispatch an action such as `SetTelemetryWindow(range)`. The reducer updates the requested window. A resource/job/service fetches missing pages. When the pages arrive, the reducer inserts them into the cache and the chart reconversions with the new snapshot.
