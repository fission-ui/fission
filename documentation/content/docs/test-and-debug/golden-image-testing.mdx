...
`compare_golden` fails the test when the configured threshold is exceeded and
writes the optional heatmap before returning. Keep golden images in the app
repository. Review diffs manually when a design change is intentional.
