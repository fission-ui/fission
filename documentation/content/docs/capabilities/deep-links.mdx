...
```rust
fn on_deep_link(state: &mut AppState, action: DeepLinkReceived) {
...
impl Widget<AppState> for CurrentRoute {
    fn build(&self, view: &View<AppState>, ctx: &mut BuildCtx<AppState>) -> Node {
        match view.state.pending_route.as_deref() {
            Some(route) if route.starts_with("document:") => {
                DocumentScreen::new(route.to_owned()).build(view, ctx)
            }
            Some(_) => Text::new("This link is not supported yet").build(view, ctx),
            None => HomeScreen.build(view, ctx),
        }
    }
}
```
