...
struct CameraDemoState {
    ...
    profile_photo_stream: Option<DataStreamId>,
    camera_error: Option<String>,
    capturing: bool,
}

impl GlobalState for CameraDemoState {}
```
...
fn on_photo_captured(state: &mut CameraDemoState, capture: CameraCapture) {
    state.capturing = false;
    state.profile_photo_stream = Some(capture.stream);
    state.camera_error = None;
}

#[fission_reducer(PhotoCaptureFailed)]
fn on_photo_capture_failed(state: &mut CameraDemoState, error: CameraError) {
    state.capturing = false;
...
impl From<CaptureProfilePhotoButton> for Widget {
    fn from(component: CaptureProfilePhotoButton) -> Self {
        let (ctx, view) = fission::build::current::<CameraDemoState>();
...
Test no-camera devices, permission denial, successful capture, cancellation, and missing flashlight support. Memory tests should assert returned metadata, stream handles, and recorded flashlight requests. If the app previews or uploads the image, test the follow-up job or service that consumes the stream.
...
A complete `Camera and flashlight` integration has all of these pieces in place:
