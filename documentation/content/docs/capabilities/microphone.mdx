...
struct MicrophoneState {
    ...
    voice_note_stream: Option<DataStreamId>,
    microphone_error: Option<String>,
    recording: bool,
}

impl GlobalState for MicrophoneState {}
```
...
fn on_audio_captured(state: &mut MicrophoneState, capture: MicrophoneCapture) {
    state.recording = false;
    state.voice_note_stream = Some(capture.stream);
    state.microphone_error = None;
}

#[fission_reducer(AudioCaptureFailed)]
fn on_audio_capture_failed(state: &mut MicrophoneState, error: MicrophoneError) {
    state.recording = false;
...
impl From<RecordVoiceNoteButton> for Widget {
    fn from(component: RecordVoiceNoteButton) -> Self {
        let (ctx, view) = fission::build::current::<MicrophoneState>();
