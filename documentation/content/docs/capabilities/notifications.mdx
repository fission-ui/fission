...
fn on_notification_shown(state: &mut AppState, receipt: NotificationReceipt) {
    state.last_notification_id = Some(receipt.id);
    state.notification_error = None;
}

#[fission_reducer(NotificationFailed)]
fn on_notification_failed(state: &mut AppState, error: NotificationError) {
...
impl Widget<AppState> for ShowSyncNotificationButton {
