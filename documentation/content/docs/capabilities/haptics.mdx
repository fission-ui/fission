...
impl GlobalState for HapticsState {}
```
...
fn on_haptic_failed(state: &mut HapticsState, error: HapticError) {
...
impl From<PlayHapticFeedbackButton> for Widget {
    fn from(component: PlayHapticFeedbackButton) -> Self {
        let (ctx, view) = fission::build::current::<HapticsState>();
...
A complete `Haptics` integration has all of these pieces in place:
