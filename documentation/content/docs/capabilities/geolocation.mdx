...
fn on_position_loaded(state: &mut AppState, position: GeolocationPosition) {
...
fn on_position_failed(state: &mut AppState, error: GeolocationError) {
    state.locating = false;
...
impl Widget<AppState> for FindNearbyStoresButton {
