...
fn on_barcode_scanned(state: &mut AppState, results: BarcodeScanResults) {
...
fn on_barcode_scan_failed(state: &mut AppState, error: BarcodeScannerError) {
    state.scanning = false;
...
impl Widget<AppState> for ScanBarcodeButton {
