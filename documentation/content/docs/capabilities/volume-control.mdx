...
fn on_volume_changed(state: &mut AppState, level: VolumeLevel) {
    state.volume = Some(level);
    state.volume_error = None;
}

#[fission_reducer(VolumeChangeFailed)]
fn on_volume_change_failed(state: &mut AppState, error: VolumeError) {
...
impl Widget<AppState> for RaiseVolumeButton {
