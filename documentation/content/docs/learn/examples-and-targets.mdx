...
An example is a teaching app. It exists to show you how a Fission app is structured and to prove that a feature works in real code. A target is a host for your app. It is the platform-specific wrapper that runs the same shared app model on macOS, Windows, Linux, Web, Android, iOS, Terminal, Static site, or SSR.
...
So when people say "add the Web target," "generate the Android target," or "build the Static site target," they do not mean "rewrite the app for that platform." They mean "use the host wrapper that lets the same app model run there."

That is why Fission can aim for one app model across macOS, Windows, Linux, Web, Android, iOS, Terminal, Static site, and SSR. The app stays shared. The host changes.
...
When you add scaffolded targets with the command-line interface, Fission generates folders such as `platforms/web/`, `platforms/android/`, `platforms/ios/`, `platforms/static-site/`, or `platforms/ssr/`. Those folders contain the files and scripts that belong to that host. Terminal apps use the terminal shell and package path rather than a mobile-style generated host folder.
...
It is important not to think of desktop as the only "real" target and everything else as a later add-on. Fission is intended to power real macOS, Windows, Linux, Web, Android, iOS, Terminal, Static site, and SSR output. Those are all genuine targets for the same runtime model, with different host responsibilities.

What does vary is the local development experience.

Desktop is often the easiest first loop because many developers can run it immediately and iterate quickly there. That makes it a very practical way to learn reducers, layout, selectors, and shared user interface behavior. But that convenience should not be confused with status. Web is not a lesser target because it runs through a browser. Android is not a lesser target because it needs emulator setup. iOS is not a lesser target because simulator and device validation are distinct steps. Terminal, Static site, and SSR are not side modes; they are real targets with different validation rules.
...
If your product is browser-first, the Web target may belong in your loop from the beginning. If your app is touch-heavy, phone-shaped, or depends on mobile presentation details, Android or iOS should enter the loop early. If your product is documentation or marketing, Static site should be the loop. If it needs request-time data, sessions, signed actions, workers, or islands, SSR should be in the loop. If it is a command workflow, Terminal should be validated as Terminal. If you are still learning the framework and want the least host friction first, desktop is often the easiest place to begin. All of those choices are reasonable.
...
Bring in Web when browser serving, browser viewport behavior, or browser integration details matter. Bring in Android when emulator packaging, mobile viewport behavior, or Android-specific validation matters. Bring in iOS when simulator behavior, iOS packaging, or mobile presentation details matter there. Bring in Terminal when terminal cells, keyboard navigation, command sessions, and scrollback matter. Bring in Static site when crawlable output, metadata, search, sitemap, and route generation matter. Bring in SSR when request lifecycle, cache policy, sessions, signed actions, workers, or islands matter.
...
`examples/terminal` and `fission ui` are the checked-in Terminal paths. They prove that Terminal is a real target built from the same app model, not a separate command prompt wrapper.

The documentation site is the checked-in Static site path. It is built by the site shell from custom Fission widgets, Markdown content routes, generated search, metadata, and assets.

`examples/pokemon-card-store` is the checked-in SSR path. It demonstrates request-time routes, sessions, signed actions, cache policy, server jobs, workers, and islands.

The command-line interface can generate scaffolded host folders for targets that need them with `fission add-target ...`, so you are not limited to the checked-in examples when you move into your own project.
...
If you keep that structure in mind, the site becomes much easier to navigate. Open examples when you want to learn how a concept looks in an app. Open target and shell docs when you want to understand how that app is launched, packaged, or validated on macOS, Windows, Linux, Web, Android, iOS, Terminal, Static site, or SSR.
