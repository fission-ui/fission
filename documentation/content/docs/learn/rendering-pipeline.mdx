...
Fission makes those layers explicit on purpose. The framework is easier to debug, easier to test, and more consistent across targets because it does not treat "draw the screen" as one opaque step.
...
First, accessibility is not added later as a shell-only guess. The core runtime defines the meaning. Second, tests can ask questions like "is there a button labeled Save?" instead of depending only on pixels. Third, shells can translate the same core meaning into the accessibility interfaces each target supports without inventing different behavior on each platform.
...
On desktop, that means creating a native window, forwarding raw input, and presenting rendered pixels. On Web, it means attaching the app to the browser renderer. On Android and iOS, it means integrating with the mobile app host and lifecycle. On Terminal, Static site, and SSR targets, it means producing terminal cells, static HTML, or request-time HTML through the matching shell.
...
If this page gave you the outline but you still want the mental model for app code itself, read [Runtime model](/docs/learn/runtime-model) next. If you want to understand how examples and generated hosts fit into target work, continue to [Examples and targets](/docs/learn/examples-and-targets).

## What you should be able to do next

After reading `Rendering pipeline`, you should be able to say what part of the Fission lifecycle the page explained, which command or source file you would inspect next, and what observation proves the idea is working. If the idea still feels abstract, run the nearest example and connect the visible behavior back to the terms on this page.
