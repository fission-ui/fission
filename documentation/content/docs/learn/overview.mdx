...
description: Understand what Fission is, why its architecture matters, and how one app model reaches macOS, Windows, Linux, Web, Android, iOS, Terminal, Static site, and SSR.
---
...
That sounds technical, but the benefit is very human. Predictable behavior makes bugs easier to explain. It makes automated tests worth trusting. It makes refactors safer because you can compare the new behavior to the old behavior with something stronger than guesswork. It also makes cross-platform work more realistic, because the framework is not asking each target to invent a separate version of the app's logic.
...
Fifth, shells run the app on each platform. A shell is the thin platform-specific layer that connects the shared app to a real environment such as a desktop window, browser renderer, mobile app, terminal, static site build, or SSR process. The shell gathers raw input, forwards lifecycle events, presents rendered output, and bridges platform accessibility APIs where the target supports them. It does not redefine your product logic. It is the adapter around the app, not the place where the app's meaning changes.
