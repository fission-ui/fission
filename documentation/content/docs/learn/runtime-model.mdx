...
A Fission app follows one steady loop no matter whether it runs on macOS, Windows, Linux, Web, Android, iOS, Terminal, Static site, or SSR. Your app keeps its important data in `GlobalState`. User intent becomes an `Action`. A reducer updates the state. A `Widget` reads the current inputs and describes the interface in `component conversion`. The runtime turns that description into layout, semantics, and output the platform shell can host.
...
When the note editor runs, the loop looks like this. `NoteState` holds the draft and save status. Typing produces an `UpdateDraft` action. The reducer updates the draft in state. The `NoteEditor` component converts again. It reads the latest inputs through `ViewHandle`, uses `BuildCtxHandle` to bind events to actions, and returns a new user interface description. If the user presses Save, the reducer uses `ReducerContext` to mark the app as saving and request the actual save effect through the runtime. A selector prepares tidy display values for the screen. The same model works across Fission targets because the product logic lives in the shared core instead of being scattered across platform code.
