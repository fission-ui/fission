...
```bash
fission readiness package --project-dir . --target android --format aab --release
...
fission distribute --project-dir . --provider github-pages --artifact target/fission/release/static-site/static/artifact-manifest.json --site production
fission distribute --project-dir . --provider cloudflare-pages --artifact target/fission/release/static-site/static/artifact-manifest.json --site production
...
Receipts record provider IDs, URLs, deployment IDs, submitted tracks, uploaded bytes, artifact hashes, provider status, and follow-up steps. If package creation, metadata sync, release-content sync, or provider publish fails after the shared workflow starts, Fission also writes a failed workflow receipt with the stage event and error in the relevant payload such as `release_metadata.error`, `release_content.error`, or `distribution.error`. Direct `fission distribute` config, publish, and lifecycle failures also write failed distribution receipts. CI and guided shells therefore get the same diagnostics. Receipts are not source files; they are audit output.

## Credentials stay out of project files

Secrets do not belong in [`fission.toml`](/reference/config/fission-toml/). Store tokens, OAuth refresh tokens, signing passwords, API keys, and private keys should come from CI secrets, environment variables, provider CLIs, or platform key stores. Fission does not persist release secrets in project files.

For local publishing, run `fission publish --provider <provider>`. The guided flow uses `~/.fission/<app-name>/release.env` and sibling files for local-only signing/provider inputs, with private filesystem permissions. This gives developers a safe local workflow while keeping CI as an explicit env/base64-secret flow.

Use the auth commands to inspect required credential sources:
...
The project can reference account IDs, bundle IDs, package names, certificate thumbprints, or environment variable names, but it should not contain credential material or repo-local secret file paths. Interactive local flows may copy or move selected secret files into `~/.fission/<app-name>/`, but they do not keep live references to secret files inside the project tree.
