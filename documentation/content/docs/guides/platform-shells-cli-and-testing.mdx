...
iOS also runs through a real host path and currently has a verified simulator flow in the repository. The simulator can select Fission's Skia raster profile when CoreSimulator cannot satisfy the normal graphics path, which keeps the simulator useful for development and validation without returning to the previous standalone software renderer. That is a current host-environment detail, not a downgrade of the platform story. As with any production iOS app, simulator runs and device validation answer different questions.
