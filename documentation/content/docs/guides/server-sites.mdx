...
## 1. Enable the SSR target

A server project declares `ssr` as a target and points `[server].entry` at the Rust function that creates the server app.
...
After this guide, you should be able to explain where `Server-rendered sites` fits in the Fission lifecycle, identify the file or component you changed, run the relevant `fission` command, and verify the result in a real target or test.
