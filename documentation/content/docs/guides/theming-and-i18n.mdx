...
That design matters for the same reason explicit state matters elsewhere in the framework. If the current theme and locale are visible inputs, the user interface stays predictable, tests can provide known values, and the same app model can behave consistently across macOS, Windows, Linux, Web, Android, iOS, Terminal, Static site, and SSR.
...
This pattern is simple and deterministic. The same translations are bundled into the app on macOS, Windows, Linux, Web, Android, iOS, Terminal, Static site, and SSR. You do not need an early network request just to render the first screen in the right language.
...
After this guide, you should be able to explain where `Theming and internationalization` fits in the Fission lifecycle, identify the file or component you changed, run the relevant `fission` command, and verify the result in a real target or test.
...
SSR apps and Static site targets use the same `Env` data, but they resolve it at request time or build time instead of once per frame:
...
Static site and SSR expose the same bundle and default-locale conveniences:
`with_env(...)`, `i18n(...)`, `translation_bundle(...)`, and
`default_locale(...)`. Both also expose `locale_resolver(...)`. Static site runs
the resolver at build time after applying page `locale` front matter; SSR runs
it per request before `.with_request_env(...)`, so route parameters, headers,
cookies, or sessions can override the request locale.
