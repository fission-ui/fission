...
State lives in `GlobalState`. Widgets describe the current user interface in `component conversion`. `BuildCtxHandle` registers runtime-managed behavior such as animations, portals, and embeds. The runtime lowers, lays out, and renders the result. Shells host the output on macOS, Windows, Linux, Web, Android, iOS, Terminal, Static site, and SSR targets.
