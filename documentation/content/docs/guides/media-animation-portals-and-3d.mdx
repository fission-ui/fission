...
In Fission, animations are not hidden in ad hoc widget-local timers. `Motion`
applies declarative tracks to a retained child, `Presence` owns enter and exit
lifecycle, and `RippleLayer` owns pointer-origin feedback. Each declaration uses
a stable widget identity, and the runtime tracks its current values.
...
You will usually use motion in one of two forms. Start with a widget preset such
as `ButtonMotion`, `ModalMotion`, or `TabsMotion` when the widget already owns
the lifecycle. Use `MotionTrack` with `Motion` or `Presence` when a custom
retained component needs direct control over compositor, layout, or paint
properties. The same runtime path drives both forms.
...
### Default widget motion

Built-in widgets animate subtly out of the box: modals and popovers fade and
scale, drawers slide, tooltips and toasts ease in, tabs move their indicator,
accordions collapse, and buttons ripple from the pointer. Each widget's
`motion` field chooses another preset, and `Some(ModalMotion::None)` (or the
matching `None` preset) turns one widget's motion off.

To ship an app with no built-in motion, set `Env::widget_motion` once:

```rust
use fission::prelude::*;

let mut env = Env::default();
env.widget_motion = WidgetMotion::Off;
```

Shells never change `widget_motion`, so the choice holds on every platform.
Widgets with an explicit `motion` preset still play it.

### Respecting reduced-motion preferences

Reduced motion is one environment-level policy, not a collection of unrelated
widget flags. `Env::motion_preference` accepts `MotionPreference::Standard`, the
default, or `MotionPreference::Reduced`:

```rust
use fission::prelude::*;

let mut env = Env::default();
env.motion_preference = MotionPreference::Reduced;
...
With `Reduced`, the runtime converts every registered `Motion` and `Presence`
track to `Instant`, including built-in widget presets and loading indicators.
It applies target values synchronously, suppresses ripple instances, and clears
any retained ripples. A closing `Presence` child disappears on the closing
build unless `keep_rendered` is set; a retained hidden child is always
interaction-inert.

The Web canvas shell reads `(prefers-reduced-motion: reduce)` when it starts and
tracks changes unless your `sync_env` callback explicitly chooses another
value. Static site and SSR output use immediate final-state animation and
transitions under the same browser preference. Other interactive shells and
tests use the same runtime policy when you supply the `Env` value through
`with_env` or keep it synchronized through `with_sync_env`. This lets widget
code keep one declarative motion configuration instead of selecting separate
presets for each target.
...
State lives in `GlobalState`. Widgets describe the current user interface in
`component conversion`. Motion widgets and higher-level presets register
runtime-owned animation declarations; `BuildCtxHandle` registers portals and
embeds where lower-level composition needs them. The runtime lowers, lays out,
and renders the result. Shells host the output on macOS, Windows, Linux, Web,
Android, iOS, Terminal, Static site, and SSR targets.
...
After this guide, you should be able to explain where `Media, animation, portals, and 3D` fits in the Fission lifecycle, identify the file or component you changed, run the relevant `fission` command, and verify the result in a real target or test.
