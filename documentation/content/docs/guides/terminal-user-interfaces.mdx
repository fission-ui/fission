...
impl Widget<ToolState> for ToolApp {
    fn build(&self, ctx: &mut BuildCtx<ToolState>, view: &View<ToolState>) -> impl IntoWidget<ToolState> {
        ...
        Row {
            children: vec![
                Column {
                    children: vec![
                        Button { on_press: Some(dashboard), child: Some(Box::new(Text::new("Dashboard").into_node())), ..Default::default() }.into_node(),
                        Button { on_press: Some(settings), child: Some(Box::new(Text::new("Settings").into_node())), ..Default::default() }.into_node(),
                    ],
                    ..Default::default()
                }.into_node(),
                Text::new(format!("Selected screen: {}", view.state.selected)).into_node(),
            ],
            ..Default::default()
        }.into_node()
    }
...
Terminal tools often run slow commands: build, test, doctor, deploy, logs, and server processes. Do not run that work directly inside `build()` and do not block the terminal event loop while a command is running.
...
2. Start the command outside `build()` and immediately return control to the UI.
