...
| SSR | Server-side SQLite application-data file | Yes | This is server state, not browser `localStorage` or cookies. Supply a request-aware provider when storage must follow server request identity. |
