...
After this guide, you should be able to explain where `Testing and diagnostics` fits in the Fission lifecycle, identify the file or component you changed, run the relevant `fission` command, and verify the result in a real target or test.
...
## Golden images and visual fidelity

For visual regressions, use LiveTest screenshots and `fission-test-driver` golden comparisons. Resize the real host, wait for a stable frame, drive the UI through semantic selectors, then compare the screenshot with `GoldenOptions`.

This is the recommended path for responsive QA, app-store screenshots, chart galleries, marketing pages, and any screen where design-system tokens, packaged fonts, gradients, shadows, or text wrapping are part of the contract. See [Golden image testing](/docs/test-and-debug/golden-image-testing/) and [Responsive visual QA](/docs/test-and-debug/responsive-visual-qa/).
