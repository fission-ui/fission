...
Server-rendered Fission routes need a clear data story. A page may be public and reusable, public but reconverted after a short time, or private to one browser session. Fission keeps those decisions explicit through route modes, cache configuration, jobs, and signed action dispatch.
...
After this guide, you should be able to explain where `Server caching, jobs, and actions` fits in the Fission lifecycle, identify the file or component you changed, run the relevant `fission` command, and verify the result in a real target or test.
...
```toml
[dependencies]
fission = { version = "0.10.0", features = ["server", "server-redis"] }
```
...
let catalog = FutureBuilder::new(
    ...
    CardGrid {
        snapshot: view.state().catalog.clone(),
    },
)
...
| --- | --- |
| Cache expiry | Fresh entries are reused, stale entries can be reconverted, and invalidation tags clear the right routes. |
