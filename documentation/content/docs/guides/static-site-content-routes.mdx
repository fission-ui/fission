...
Start with a Fission project that has the `static-site` target enabled.
...
After this guide, you should be able to explain where `Static site content routes` fits in the Fission lifecycle, identify the file or component you changed, run the relevant `fission` command, and verify the result in a real target or test.
...
Use `template = "fission::site::blog"` for a post collection. Blog behavior is explicit rather than tied to a `/blog` path, so `/journal`, `/news`, or another configured mount works the same way. See [Static site custom pages](/docs/guides/static-site-custom-pages/) for retained header and footer overrides, project CSS, and route-scoped document enhancements.
