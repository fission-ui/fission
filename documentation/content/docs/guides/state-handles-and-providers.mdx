...
Fission assigns that identity automatically from the application root and the
widget's structural path. The root defaults to `WidgetId::app_root()`. Retained
runtime shells also provide `with_root_id(...)` when an embedder mounts multiple
independent applications and needs a separate namespace for each one. An
explicit ID set directly on the root widget remains authoritative.
...
For static sibling widgets, Fission distinguishes local state by structural position. Unidentified dynamic-list items deliberately use the same positional rule. Add explicit identity when state must follow a logical item as its position changes.
...
You do not need to assign IDs to every widget. Fission automatically gives the
item's descendants stable IDs scoped beneath the explicit item ID. Use an
explicit `WidgetId` when identity must follow data across insertion, removal,
filtering, or reordering; leave an item unidentified when positional identity is
...
After this guide, you should be able to explain where `State, handles, and providers` fits in the Fission lifecycle, identify the file or component you changed, run the relevant `fission` command, and verify the result in a real target or test.
...
## State and responsive layout

Do not store breakpoint state in `GlobalState` just to make a screen responsive. If the branch is only about available width, use [`Responsive`](/reference/widgets/responsive/) and [`Length`](/reference/core/length/). The runtime can then inspect the layout rule and lower it consistently across shells.

Use ordinary Rust branching with `view.viewport_size()` only when the branch also depends on product state, environment values, previous layout geometry, or another runtime input. State should hold durable facts such as the selected thread or open inspector, not a cached copy of the current breakpoint.
