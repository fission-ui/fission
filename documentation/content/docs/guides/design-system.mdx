...
Fission ships several bundled DSP packages for common product directions. They are not magic modes inside the renderer. Each one is a normal `dsp.json` and `tokens.json` package under `crates/core/fission-theme/design`, generated into a Rust `DesignSystem` implementation at build time.
...
That keeps the same architecture: design information enters as DSP JSON at build time, and widgets consume typed Rust theme values at runtime.
