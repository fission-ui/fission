...
If you are trying to upload files specifically, read [How do I do file uploads in Fission?](/docs/guides/file-uploads/) after this overview. If you want the architectural reason reducers stay synchronous, read [Why reducers are synchronous](/docs/guides/why-reducers-are-synchronous/).
...
`PICK_OPEN_FILES` returns `PickedFile` values with metadata plus a `DataStreamId`. It does not put the selected file contents in the reducer payload. If you need to parse, preview, upload, or transform the file, pass the stream handle to a job or service and consume it there with `JobCtx::open_data_stream(...)` or `ServiceCtx::open_data_stream(...)`.

The default desktop shell provider streams selected files from disk in chunks. Web, mobile, server, static-site, and terminal targets can supply their own provider when file picking makes sense for that host.
...
After this guide, you should be able to explain where `Resources and async` fits in the Fission lifecycle, identify the file or component you changed, run the relevant `fission` command, and verify the result in a real target or test.
