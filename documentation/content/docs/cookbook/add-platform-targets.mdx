...
That is what `fission add-target` is for.
...
First, it updates [`fission.toml`](/reference/config/fission-toml/) so the project records which targets exist.
...
- `fission` means "run the Fission command-line interface through Cargo"
...
By the end of `Add platform targets`, you should have a runnable change, not just a set of snippets. Run the command shown in the page, exercise the feature once manually, and add or update the smallest test that protects the behavior.
...
[`fission.toml`](/reference/config/fission-toml/) now records the targets you added.
...
`GlobalState` is still shared.

Your reducers are still shared.

Your components still convert with `ViewHandle` and `BuildCtxHandle`.
...
Do not fork your app model too early. The value of Fission is that macOS, Windows, Linux, Web, Android, iOS, Terminal, Static site, and SSR share one core runtime model.
