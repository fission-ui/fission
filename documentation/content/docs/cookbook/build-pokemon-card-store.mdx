...
This cookbook recreates the structure of the Pokémon Card Store example. The goal is a server-rendered ecommerce page: HTML is produced on the server, product data is loaded through a job, the page is cacheable with a TTL, filters use a progressive worker, and the cart uses a focused WASM island.
...
The cache settings say that generated pages can be held in memory and revalidated. The worker and island settings say browser artifacts should be generated separately rather than bundled into one large file.
