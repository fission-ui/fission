...
Second, once the picker returns a stream handle, the app may want to parse the file contents or transform them into domain data. That is ordinary app-owned async work.
...
impl GlobalState for ImportState {}
```
...
struct ParseImportedTextRequest {
    stream: DataStreamId,
}
...
fn on_files_picked(
    ...
    ctx.effects
        .app(PARSE_IMPORTED_TEXT_JOB, ParseImportedTextRequest { stream: file.stream })
        .on_ok(ctx.effects.bind(
            ImportParsed,
...
Parsing the returned stream is ordinary app work, so it becomes a job. The job can call `JobCtx::open_data_stream(request.stream)` and consume the `FissionDataStream` asynchronously while the reducer stays synchronous.

Why not keep using a capability here? Because the host is no longer the right owner. Once the host has granted a stream handle, parsing belongs to your application logic.
...
```rust
impl From<ImportScreen> for Widget {
    fn from(component: ImportScreen) -> Self {
        let (ctx, view) = fission::build::current::<ImportState>();
        let import = ctx.bind(
            ImportPressed,
        ...
        Column {
            ...
            children: vec![
                Button {
                    ...
                    child: Some(Text::new("Import file").into()),
                    ..Default::default()
                }
                .into(),
...
Use a job when the work is app-owned, one-shot, and request-and-response shaped. Parsing an imported data stream, saving a document, loading one response from a web service, or generating one export all fit that shape.
...
Do not put platform calls directly in `component conversion`. That will make reconversions unsafe and unpredictable.
...
By the end of `Run typed host work`, you should have a runnable change, not just a set of snippets. Run the command shown in the page, exercise the feature once manually, and add or update the smallest test that protects the behavior.
