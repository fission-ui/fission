...
Third, the translation files should already be available at startup on macOS, Windows, Linux, Web, Android, iOS, Terminal, Static site, and SSR.
...
Start by adding checked-in translation files such as `i18n/en-US.yaml` and `i18n/es-ES.yaml`.
...
With those files in place, add `serde_yaml = "0.9"` to your app and load the YAML into `Env` during startup.
