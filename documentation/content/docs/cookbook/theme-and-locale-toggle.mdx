...
impl Widget<SettingsState> for SettingsApp {
    fn build(&self, ctx: &mut BuildCtx<SettingsState>, view: &View<SettingsState>) -> impl IntoWidget<SettingsState> {
        let light = ctx.bind(
            SetLightTheme,
...
If you want the broader explanation of how `Env`, `View`, and synchronized environment values work, read [Input, text, and environment](/docs/guides/input-events-text-and-env) and [Theming and internationalization](/docs/guides/theming-and-i18n). If you want a repo-backed example after finishing this recipe, the `inbox` example is the strongest current demonstration of theme and locale switching.
