...
The Fission command-line interface is the tool you use to put a new Fission app on disk, connect that shared app to real platform hosts, and keep the daily development loop consistent across desktop, web, Android, iOS, terminal, static site, and server-rendered site targets.

That distinction matters. Fission's core architecture lives in your shared Rust code: your app state, actions, reducers, widgets, and effects. The command-line interface does not replace that architecture. Its job is narrower and more concrete. It gives you a correct starting scaffold, records which targets your app supports, generates the host folders that let the shared app run on desktop, web, Android, iOS, terminal, static site, and server-rendered site targets, and then gives you one place to discover devices, run the app, attach logs, build artifacts, and run smoke tests.
...
- the shell hosts, which package and launch that shared app in a desktop window, a browser page, an Android app, an iOS app bundle, a terminal surface, static HTML output, or a server-rendered web process.
...
The second moment is when your shared app is ready to run in another host. At that point, you use `fission add-target` to generate the extra platform folder for web, Android, iOS, terminal, static site, or server, or to add a specific desktop target if needed. The important idea is that you are not rewriting your app for each platform. You are generating the host wrapper around the same shared app model.
...
| `fission devtools` | Inspects widget trees, frame performance, and FDTP snapshots from a development app | When debugging a running app started with `fission run --devtools` |
...
| `server` | Server-rendered web host | `platforms/server/` notes plus `[server].entry` in [`fission.toml`](/reference/config/fission-toml/) |
| `site` | Static HTML site | `content/` and static-site configuration |
...
Records host capabilities in [`fission.toml`](/reference/config/fission-toml/) and updates generated platform config where Fission can do that safely. Current manifest values include `barcode-scanner`, `biometric`, `bluetooth`, `camera`, `geolocation`, `haptics`, `microphone`, `nfc`, `notifications`, `passkeys`, `volume-control`, and `wifi`.
...
```sh
fission run [--target <target>] [--device <id>] [--project-dir <dir>] [--release] [--detach] [--devtools] [--performance-overlay]
```
...
Use `--devtools` during development when you want the running app to expose FDTP snapshots for inspection. The command sets `FISSION_DEVTOOLS=1` and `FISSION_TEST_CONTROL_PORT=<port>` for the launched app. The default devtools port is `9876`; override it with `--devtools-port <port>` if another process is using that port. Add `--performance-overlay` to draw the runtime frame timing overlay inside the app.

### `fission devtools`

```sh
fission devtools capabilities [--json]
fission devtools tree [--port <port>] [--json]
fission devtools perf [--port <port>] [--json]
fission devtools snapshot [--port <port>] [--pretty] [--output <file>]
fission devtools env [--port <port>] [--performance-overlay]
```

Inspects a running development app. Start the app with `fission run --devtools`, then use `tree` for the authored widget tree, `perf` for the latest frame timing sample, or `snapshot` for the complete FDTP frame payload. `env` prints the environment variables for shells or scripts that launch an app without going through `fission run`.
...
Use this checklist when wiring `Command-line interface overview` into an app or shell implementation.
