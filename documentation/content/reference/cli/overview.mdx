...
Records host capabilities in `fission.toml` and updates generated platform config where Fission can do that safely. Current values include `nfc`, `biometric`, `bluetooth`, `barcode-scanner`, `camera`, `geolocation`, `haptics`, `microphone`, `volume-control`, and `wifi`.
