...
Runs the configured `[server].entry`, renders every declared server route, and fails if the route cannot produce HTML.
...
Runs the server-rendered app locally. The default port is `8124`.
