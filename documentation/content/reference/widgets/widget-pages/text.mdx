...
| `text_scaler` | `Option<TextScaler>` | Explicit linear or nonlinear font-size scaling. | When absent, the widget uses `Env.text_scaler`, allowing the shell or app to supply accessibility scaling once for the whole tree. |
| `typography` | `TextTypography` | Fallback families, word spacing, decoration, shadows, paints, OpenType features, variable axes, baseline, and leading policy. | Uses the same paragraph path as ordinary text rather than a separate advanced renderer. |
| `typography.line_break` | `TextLineBreakPolicy` | Line-breaking strictness. | Passed through the shared paragraph contract; native behavior is limited by the shaping engine. |
| `typography.hyphenation` | `TextHyphenation` | Whether language-aware hyphenation is requested. | Static/SSR lowers the applicable CSS; native behavior is limited by the shaping engine. |
...
Wrap several selectable paragraphs in `SelectionRegion` when selection must
cross widget boundaries. Its `SelectionRegionController` can inspect, set,
clear, or select all of the coordinated content. Double click selects a word,
triple click selects a paragraph, touch long press selects a word, and dragging
continues through participating descendants in retained document order.

Fission retains one `ResolvedParagraphLayout` for the final constraint of each
text node. Measurement, painting, cluster hit testing, caret stops, selection
rectangles, inline placement, and input-method geometry consume that decision,
so a viewport resize is not required to correct first-frame wrapping.

The Vello backend currently approximates blurred text shadows and non-solid
decoration patterns with its supported text primitives; Static site and SSR use
the browser's CSS implementation. Ideographic baseline and leading policies are
retained in the shared contract even where the active font/shaper cannot make a
distinct adjustment.
