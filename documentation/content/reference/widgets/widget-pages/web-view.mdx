...
| --- | --- | --- | --- |
| `id` | `WidgetId` | Stable identity for the embedded browser node. | Required. The runtime uses it as the key in `runtime.web.states`. |
...
The public widget exists. The conversion path registers the web surface with `BuildCtxHandle::register_web_view(...)`. The runtime syncs that registration into `runtime.web.states` and tracks the requested web address and related metadata fields.
