...
let help_panel = WebView {
    id: WidgetId::explicit("help_docs"),
    label: Some("Help documentation".into()),
...
| --- | --- | --- | --- |
| `id` | `WidgetId` | Stable identity for the embedded browser node. | Required. The runtime uses it as the key in `runtime web state store`. |
...
The public widget exists. The conversion path registers the web surface with `BuildCtxHandle::register_web_view(...)`. The runtime syncs that registration into `runtime web state store` and tracks the requested web address and related metadata fields.
...
That caveat belongs to `WebView` itself. It is a statement about this specialized embedded browser surface, not about the readiness of Fission's shared runtime or its broader macOS, Windows, Linux, Web, Android, iOS, Terminal, Static site, and SSR app model.
...
Also be careful not to assume that all metadata in `runtime web state store` will be populated end to end today. The state fields exist, but the current shells do not yet drive the complete browser-control story through them.

## Production checklist

For `WebView`, review the fields that change behavior before treating the widget as finished: `id`, `url`, `user_agent`, `width`, `height`. The goal is to make the product rule visible in state and actions, not hidden inside ad-hoc construction code.
...
If a screen starts repeating the same `WebView` setup, extract a named component around this widget. That keeps the reference API small while making product code easier to read and safer for generated code to copy.
