...
SplitView {
    id: WidgetId::explicit("mail_split"),
    direction,
    first: mail_list.into(),
    second: message_detail.into(),
    split_ratio: 0.35,
    on_resize: None,
}
```
...
If your direction changes at runtime, keep that responsive rule in ordinary app logic or a named component. `SplitDirection` should stay the result of the decision, not the place where the decision logic lives.
...
[`SplitView`](/reference/widgets/split-view), [`Row`](/reference/widgets/row), and [`Column`](/reference/widgets/column).
