...
| `on_drag_cancel` | `Option<ActionEnvelope>` | Action for an active drag interrupted by the platform. When absent, `on_drag_end` is used as a compatibility fallback. | Defaults to `None`. |
...
A normal pointer release dispatches `on_drag_end`. A platform interruption dispatches `on_drag_cancel` instead. If no cancel action is configured, Fission falls back to `on_drag_end` so existing drag declarations keep their behavior; one cancellation never dispatches both actions.
