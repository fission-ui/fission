...
| Field | Type | Required | Default | Meaning |
| --- | --- | --- | --- | --- |
| `entry` | string | Yes for server commands | none | Rust path to the function that returns the `FissionServerApp`, such as `pokemon_card_store::pokemon_card_store_server`. |
| `default_route_mode` | string | No | app default | Route mode used when a route does not specify its own mode. Common values are `static`, `dynamic`, and `revalidated`. |
| `render_pass_limit` | integer | No | shell default | Maximum render passes allowed while jobs/resources settle before the server returns HTML. |

### `[server.cache]`

| Field | Type | Required | Default | Meaning |
| --- | --- | --- | --- | --- |
| `provider` | string | No | `moka` | Cache backend. `moka` is the built-in in-memory provider. |
| `max_capacity` | integer | No | provider default | Maximum number of cache entries for providers that support a bounded in-memory cache. |
| `ttl` | duration string | No | route/provider default | Default time-to-live for revalidated pages, such as `5m` or `30s`. |
| `stale_while_revalidate` | duration string | No | none | How long stale content may be served while a background rebuild refreshes it. |

### `[server.workers]` and `[server.islands]`

| Table | Field | Type | Required | Meaning |
| --- | --- | --- | --- | --- |
| `[server.workers]` | `separate_artifacts` | boolean | No | When `true`, worker declarations generate separate browser artifacts. |
| `[server.workers]` | `bridge` | string | No | Browser bridge mode for generated worker shims. Current projects use `generated`. |
| `[server.islands]` | `separate_artifacts` | boolean | No | When `true`, island declarations generate separate browser artifacts. |
| `[server.islands]` | `preload` | string | No | Preload policy for island artifacts. `route` means preload artifacts required by the current route. |
