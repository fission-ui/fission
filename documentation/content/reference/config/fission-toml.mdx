...
| `capabilities` | array of strings | No | `barcode-scanner`, `biometric`, `bluetooth`, `camera`, `filesystem`, `geolocation`, `haptics`, `microphone`, `nfc`, `notifications`, `passkeys`, `storage`, `volume-control`, `wifi` | Declares host capabilities the app intends to use. `fission add-capability` updates this list, optional Cargo integrations, and generated Android/iOS files where the configuration is deterministic. |
...
| `template` | string | No | default documentation page | Template identity. `fission::site::documentation` selects the standard documentation layout. `fission::site::blog` enables post indexes, pagination, adjacent-post navigation, and taxonomy routes at the configured mount path. |
...
| `locale` | string | No | `[site].default_locale` | Locale used to resolve translated text and emit the document `lang` and `og:locale` metadata for this page. `lang` and `language` are accepted aliases. |
