...
If your question is about how the app reaches desktop, web, Android, iOS, terminal, static HTML, or server-rendered HTML output, move to the platform pages. Those pages explain shells, targets, target-level testing, and where platform responsibility begins and ends.
...
The core runtime is the heart of Fission. These pages describe the shared model that stays consistent across desktop, web, Android, iOS, terminal, static site, and server-rendered site targets.

Start with [Widget authoring boundary](/reference/core/widget-authoring-boundary) when you need to confirm what a component is, what conversion is allowed to do, or how `BuildCtxHandle` and `ViewHandle` divide responsibility.
...
[Targets](/reference/platform/targets) is the page to open when you need to understand generated host output, target folders, and the practical expectations for desktop, web, Android, iOS, terminal, static site, and server-rendered site targets.
