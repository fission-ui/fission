...
Android uses the generated AndroidX GameActivity host and the AccessKit Android adapter, so the same Fission semantic tree is available to TalkBack and other Android accessibility services. Web, Static site, SSR, and Terminal targets do not use the winit AccessKit bridge; they use target-specific accessibility output. The Web canvas text adapter exposes its focused native input, while Static site and SSR produce semantic HTML for the complete rendered document.
...
| Terminal | Keyboard navigation, visible focus, readable contrast, screen-reader terminal behavior, and non-color-only status. |
