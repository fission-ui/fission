...
sidebar_label: Mark point component
description: Fission Charts reference for Mark point component.
---

# Mark point component

![Mark point component screenshot](/img/charts/component-mark-point.png)

Line chart with named event points. Use it for incidents and milestones. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Mark point component is part of the **Components and interaction** chart family and is intended for `MarkPoint plus LineSeries`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`MarkPoint plus LineSeries.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct MarkPointComponentChart;

impl From<MarkPointComponentChart> for Widget {
    fn from(_: MarkPointComponentChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
