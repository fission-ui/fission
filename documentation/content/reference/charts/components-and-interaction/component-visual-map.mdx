...
sidebar_label: Visual map component
description: Fission Charts reference for Visual map component.
---

# Visual map component

![Visual map component screenshot](/img/charts/component-visual-map.png)

Visible color scale for heatmap values. Use it where color represents a numeric value. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Visual map component is part of the **Components and interaction** chart family and is intended for `VisualMap plus HeatmapSeries`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`VisualMap plus HeatmapSeries.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct VisualMapComponentChart;

impl From<VisualMapComponentChart> for Widget {
    fn from(_: VisualMapComponentChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
