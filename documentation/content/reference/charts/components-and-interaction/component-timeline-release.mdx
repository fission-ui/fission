...
![Timeline releases screenshot](/img/charts/component-timeline-release.png)

Timeline component for release phases. Use it for phase or scenario playback. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Timeline releases is part of the **Components and interaction** chart family and is intended for `ChartTimeline plus series`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`ChartTimeline plus series.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
