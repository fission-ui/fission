...
sidebar_label: Brush horizontal
description: Fission Charts reference for Brush horizontal.
---

# Brush horizontal

![Brush horizontal screenshot](/img/charts/component-brush-horizontal.png)

Horizontal brush preview over scatter data. Use it for x-range selection. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Brush horizontal is part of the **Components and interaction** chart family and is intended for `ChartBrush plus ScatterSeries`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`ChartBrush plus ScatterSeries.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct BrushHorizontalChart;

impl From<BrushHorizontalChart> for Widget {
    fn from(_: BrushHorizontalChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
