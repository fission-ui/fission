---
title: Toolbox zoom
sidebar_label: Toolbox zoom
description: Fission Charts reference for Toolbox zoom.
---

# Toolbox zoom

![Toolbox zoom screenshot](/img/charts/component-toolbox-zoom.png)

Toolbox actions rendered near the chart. Use it for chart utility operations. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Toolbox zoom is part of the **Components and interaction** chart family and is intended for `ChartInteraction toolbox actions`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`ChartInteraction toolbox actions.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ToolboxZoomChart;

impl From<ToolboxZoomChart> for Widget {
    fn from(_: ToolboxZoomChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
