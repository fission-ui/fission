...
Graphic overlay is part of the **Components and interaction** chart family and is intended for `ChartGraphic rect, text, line, or circle records`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
```rust
use fission::charts::{Axis, Chart, ChartGraphic, LineSeries};
...
pub struct GraphicOverlayChart;

impl From<GraphicOverlayChart> for Widget {
    fn from(_: GraphicOverlayChart) -> Widget {
        Chart::new()
            .title("Graphic overlay")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
