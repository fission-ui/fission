...
sidebar_label: Mark line component
description: Fission Charts reference for Mark line component.
---

# Mark line component

![Mark line component screenshot](/img/charts/component-mark-line.png)

Line chart with target threshold line. Use it for goals and alert thresholds. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Mark line component is part of the **Components and interaction** chart family and is intended for `MarkLine plus LineSeries`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`MarkLine plus LineSeries.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct MarkLineComponentChart;

impl From<MarkLineComponentChart> for Widget {
    fn from(_: MarkLineComponentChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
