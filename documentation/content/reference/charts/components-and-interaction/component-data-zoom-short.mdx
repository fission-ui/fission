...
sidebar_label: Data zoom short
description: Fission Charts reference for Data zoom short.
---

# Data zoom short

![Data zoom short screenshot](/img/charts/component-data-zoom-short.png)

Small data zoom window over ordered data. Use it for local inspection of long series. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Data zoom short is part of the **Components and interaction** chart family and is intended for `DataZoom plus LineSeries`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`DataZoom plus LineSeries.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DataZoomShortChart;

impl From<DataZoomShortChart> for Widget {
    fn from(_: DataZoomShortChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
