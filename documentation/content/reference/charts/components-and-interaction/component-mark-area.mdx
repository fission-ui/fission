...
sidebar_label: Mark area component
description: Fission Charts reference for Mark area component.
---

# Mark area component

![Mark area component screenshot](/img/charts/component-mark-area.png)

Line chart with a highlighted range. Use it for safe operating zones. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Mark area component is part of the **Components and interaction** chart family and is intended for `MarkArea plus LineSeries`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`MarkArea plus LineSeries.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct MarkAreaComponentChart;

impl From<MarkAreaComponentChart> for Widget {
    fn from(_: MarkAreaComponentChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
