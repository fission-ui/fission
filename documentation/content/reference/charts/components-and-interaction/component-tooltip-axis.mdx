...
sidebar_label: Tooltip axis component
description: Fission Charts reference for Tooltip axis component.
---

# Tooltip axis component

![Tooltip axis component screenshot](/img/charts/component-tooltip-axis.png)

Axis tooltip over shared bar and line data. Use it for exact values across series. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Tooltip axis component is part of the **Components and interaction** chart family and is intended for `Tooltip plus ChartInteraction`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Tooltip plus ChartInteraction.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct TooltipAxisComponentChart;

impl From<TooltipAxisComponentChart> for Widget {
    fn from(_: TooltipAxisComponentChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
