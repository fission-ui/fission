...
sidebar_label: Tooltip item component
description: Fission Charts reference for Tooltip item component.
---

# Tooltip item component

![Tooltip item component screenshot](/img/charts/component-tooltip-item.png)

Item tooltip over single series data. Use it for exact item readout. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Tooltip item component is part of the **Components and interaction** chart family and is intended for `Tooltip item trigger`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Tooltip item trigger.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct TooltipItemComponentChart;

impl From<TooltipItemComponentChart> for Widget {
    fn from(_: TooltipItemComponentChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
