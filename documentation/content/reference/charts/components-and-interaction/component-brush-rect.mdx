...
sidebar_label: Brush rectangle
description: Fission Charts reference for Brush rectangle.
---

# Brush rectangle

![Brush rectangle screenshot](/img/charts/component-brush-rect.png)

Rectangular brush preview over a scatter plot. Use it for region selection. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Brush rectangle is part of the **Components and interaction** chart family and is intended for `ChartBrush plus ScatterSeries`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct BrushRectangleChart;

impl From<BrushRectangleChart> for Widget {
    fn from(_: BrushRectangleChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
