...
sidebar_label: Graphic callout
description: Fission Charts reference for Graphic callout.
---

# Graphic callout

![Graphic callout screenshot](/img/charts/component-graphic-callout.png)

Graphic annotation overlay on a line chart. Use it to explain a change in context. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Graphic callout is part of the **Components and interaction** chart family and is intended for `ChartGraphic plus LineSeries`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`ChartGraphic plus LineSeries.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct GraphicCalloutChart;

impl From<GraphicCalloutChart> for Widget {
    fn from(_: GraphicCalloutChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
