---
title: Visual map
sidebar_label: Visual map
description: Fission Charts reference for Visual map.
---

# Visual map

![Visual map screenshot](/img/charts/visual-map.png)

Color encodes a numeric range consistently across a chart. Use it for heatmap and scatter intensity. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Visual map is part of the **Data pipeline and interaction** chart family and is intended for `Numeric values plus a color scale`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct VisualMapChart;

impl From<VisualMapChart> for Widget {
    fn from(_: VisualMapChart) -> Widget {
        Chart::new()
            .title("Visual map")
            .width(720.0)
            .height(480.0)
