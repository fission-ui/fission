...
sidebar_label: Encoded dataset
description: Fission Charts reference for Encoded dataset.
---
...
Line and bar series bound to named dataset dimensions. Use it when chart code should name fields instead of copying arrays. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Encoded dataset is part of the **Data pipeline and interaction** chart family and is intended for `Dataset rows, dimensions, and encode mappings`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct EncodedDatasetChart;

impl From<EncodedDatasetChart> for Widget {
    fn from(_: EncodedDatasetChart) -> Widget {
        Chart::new()
            .title("Encoded dataset")
            .width(720.0)
            .height(480.0)
