...
sidebar_label: Service coverage map
description: Fission Charts reference for Service coverage map.
---

# Service coverage map

![Service coverage map screenshot](/img/charts/map-service-coverage.png)

Map with coverage values by region. Use it for coverage and rollout screens. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Service coverage map is part of the **Geo and route** chart family and is intended for `GeoJSON regions plus values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct ServiceCoverageMapChart;

impl From<ServiceCoverageMapChart> for Widget {
    fn from(_: ServiceCoverageMapChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
