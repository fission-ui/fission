...
sidebar_label: Flight-like lines
description: Fission Charts reference for Flight-like lines.
---
...
Directional curved lines with effect points. Use it for route animation-style visuals. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Flight-like lines is part of the **Geo and route** chart family and is intended for `LineSegment values plus effect`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`LineSegment values plus effect.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct FlightLikeLinesChart;

impl From<FlightLikeLinesChart> for Widget {
    fn from(_: FlightLikeLinesChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
