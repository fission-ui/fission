---
title: Traffic lines
sidebar_label: Traffic lines
description: Fission Charts reference for Traffic lines.
---

# Traffic lines

![Traffic lines screenshot](/img/charts/lines-traffic.png)

Curved lines with different values. Use it for traffic or flow intensity. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Traffic lines is part of the **Geo and route** chart family and is intended for `LineSegment values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct TrafficLinesChart;

impl From<TrafficLinesChart> for Widget {
    fn from(_: TrafficLinesChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
