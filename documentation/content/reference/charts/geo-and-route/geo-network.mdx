---
title: Geo network
sidebar_label: Geo network
description: Fission Charts reference for Geo network.
---

# Geo network

![Geo network screenshot](/img/charts/geo-network.png)

Map with network-like route overlays. Use it for network coverage and paths. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Geo network is part of the **Geo and route** chart family and is intended for `GeoJSON plus lines`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`GeoJSON plus lines.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct GeoNetworkChart;

impl From<GeoNetworkChart> for Widget {
    fn from(_: GeoNetworkChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
