---
title: Risk map
sidebar_label: Risk map
description: Fission Charts reference for Risk map.
---

# Risk map

![Risk map screenshot](/img/charts/map-risk.png)

Regions colored by risk score. Use it for geographic risk dashboards. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Risk map is part of the **Geo and route** chart family and is intended for `GeoJSON regions plus numeric values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`GeoJSON regions plus numeric values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct RiskMapChart;

impl From<RiskMapChart> for Widget {
    fn from(_: RiskMapChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
