---
title: Sales map
sidebar_label: Sales map
description: Fission Charts reference for Sales map.
---

# Sales map

![Sales map screenshot](/img/charts/map-sales.png)

Region sales values on a map. Use it for regional business metrics. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Sales map is part of the **Geo and route** chart family and is intended for `GeoJSON regions plus values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct SalesMapChart;

impl From<SalesMapChart> for Widget {
    fn from(_: SalesMapChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
