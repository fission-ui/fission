...
sidebar_label: Migration routes
description: Fission Charts reference for Migration routes.
---

# Migration routes

![Migration routes screenshot](/img/charts/geo-migration.png)

Route lines representing movement intensity. Use it for movement and transfer flows. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Migration routes is part of the **Geo and route** chart family and is intended for `LineSegment values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`LineSegment values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct MigrationRoutesChart;

impl From<MigrationRoutesChart> for Widget {
    fn from(_: MigrationRoutesChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
