...
Region value map is part of the **Geo and route** chart family and is intended for `GeoJSON regions plus values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct RegionValueMapChart;

impl From<RegionValueMapChart> for Widget {
    fn from(_: RegionValueMapChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
