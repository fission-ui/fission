...
sidebar_label: Source theme river
description: Fission Charts reference for Source theme river.
---

# Source theme river

![Source theme river screenshot](/img/charts/analytics-source-river.png)

Source theme river uses Fission Charts typed Rust data to render a production-ready analytics view. Use it for changing source composition. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Source theme river is part of the **Analytics** chart family and is intended for `Source values over time`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Source values over time.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct SourceThemeRiverChart;

impl From<SourceThemeRiverChart> for Widget {
    fn from(_: SourceThemeRiverChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
