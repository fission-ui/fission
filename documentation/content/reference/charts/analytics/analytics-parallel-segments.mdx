...
sidebar_label: Segment parallel
description: Fission Charts reference for Segment parallel.
---

# Segment parallel

![Segment parallel screenshot](/img/charts/analytics-parallel-segments.png)

Segment parallel uses Fission Charts typed Rust data to render a production-ready analytics view. Use it for segment tradeoff analysis. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Segment parallel is part of the **Analytics** chart family and is intended for `Segment rows across several dimensions`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Segment rows across several dimensions.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct SegmentParallelChart;

impl From<SegmentParallelChart> for Widget {
    fn from(_: SegmentParallelChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
