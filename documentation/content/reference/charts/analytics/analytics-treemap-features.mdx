...
sidebar_label: Feature treemap
description: Fission Charts reference for Feature treemap.
---

# Feature treemap

![Feature treemap screenshot](/img/charts/analytics-treemap-features.png)

Feature treemap uses Fission Charts typed Rust data to render a production-ready analytics view. Use it for feature usage share. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Feature treemap is part of the **Analytics** chart family and is intended for `Feature values in a hierarchy`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Feature values in a hierarchy.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct FeatureTreemapChart;

impl From<FeatureTreemapChart> for Widget {
    fn from(_: FeatureTreemapChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
