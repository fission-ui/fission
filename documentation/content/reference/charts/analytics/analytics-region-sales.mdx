...
sidebar_label: Region sales map
description: Fission Charts reference for Region sales map.
---

# Region sales map

![Region sales map screenshot](/img/charts/analytics-region-sales.png)

Region sales map uses Fission Charts typed Rust data to render a production-ready analytics view. Use it for geographic sales analysis. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Region sales map is part of the **Analytics** chart family and is intended for `Sales values by named region`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Sales values by named region.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct RegionSalesMapChart;

impl From<RegionSalesMapChart> for Widget {
    fn from(_: RegionSalesMapChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
