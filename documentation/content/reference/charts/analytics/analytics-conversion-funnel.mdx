...
sidebar_label: Conversion funnel
description: Fission Charts reference for Conversion funnel.
---

# Conversion funnel

![Conversion funnel screenshot](/img/charts/analytics-conversion-funnel.png)

Conversion funnel uses Fission Charts typed Rust data to render a production-ready analytics view. Use it for product funnel analysis. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Conversion funnel is part of the **Analytics** chart family and is intended for `Ordered conversion stage values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Ordered conversion stage values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ConversionFunnelChart;

impl From<ConversionFunnelChart> for Widget {
    fn from(_: ConversionFunnelChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
