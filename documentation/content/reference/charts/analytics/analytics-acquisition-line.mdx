...
sidebar_label: Acquisition trend
description: Fission Charts reference for Acquisition trend.
---
...
Acquisition trend uses Fission Charts typed Rust data to render a production-ready analytics view. Use it for growth dashboards. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Acquisition trend is part of the **Analytics** chart family and is intended for `Acquisition values over time`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Acquisition values over time.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct AcquisitionTrendChart;

impl From<AcquisitionTrendChart> for Widget {
    fn from(_: AcquisitionTrendChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
