...
sidebar_label: Journey sankey
description: Fission Charts reference for Journey sankey.
---

# Journey sankey

![Journey sankey screenshot](/img/charts/analytics-journey-sankey.png)

Journey sankey uses Fission Charts typed Rust data to render a production-ready analytics view. Use it for user movement between stages. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Journey sankey is part of the **Analytics** chart family and is intended for `Journey stages and flow links`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Journey stages and flow links.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct JourneySankeyChart;

impl From<JourneySankeyChart> for Widget {
    fn from(_: JourneySankeyChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
