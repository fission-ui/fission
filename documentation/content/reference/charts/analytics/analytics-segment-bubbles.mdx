...
Segment bubbles is part of the **Analytics** chart family and is intended for `Segment x, y, and value triples`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct SegmentBubblesChart;

impl From<SegmentBubblesChart> for Widget {
    fn from(_: SegmentBubblesChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
