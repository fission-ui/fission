...
sidebar_label: Report toolbox chart
description: Fission Charts reference for Report toolbox chart.
---

# Report toolbox chart

![Report toolbox chart screenshot](/img/charts/analytics-toolbox-report.png)

Report toolbox chart uses Fission Charts typed Rust data to render a production-ready analytics view. Use it when analytics views need built-in controls. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Report toolbox chart is part of the **Analytics** chart family and is intended for `Chart with report actions`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Chart with report actions.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ReportToolboxChart;

impl From<ReportToolboxChart> for Widget {
    fn from(_: ReportToolboxChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
