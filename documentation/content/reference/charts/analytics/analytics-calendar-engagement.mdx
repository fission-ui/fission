...
sidebar_label: Engagement calendar
description: Fission Charts reference for Engagement calendar.
---

# Engagement calendar

![Engagement calendar screenshot](/img/charts/analytics-calendar-engagement.png)

Engagement calendar uses Fission Charts typed Rust data to render a production-ready analytics view. Use it for engagement consistency over time. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Engagement calendar is part of the **Analytics** chart family and is intended for `Daily engagement values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Daily engagement values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct EngagementCalendarChart;

impl From<EngagementCalendarChart> for Widget {
    fn from(_: EngagementCalendarChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
