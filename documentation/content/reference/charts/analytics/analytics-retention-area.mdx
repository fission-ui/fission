...
sidebar_label: Retention area
description: Fission Charts reference for Retention area.
---

# Retention area

![Retention area screenshot](/img/charts/analytics-retention-area.png)

Retention area uses Fission Charts typed Rust data to render a production-ready analytics view. Use it when volume and trend both matter. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Retention area is part of the **Analytics** chart family and is intended for `Retention values with area fill`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Retention values with area fill.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct RetentionAreaChart;

impl From<RetentionAreaChart> for Widget {
    fn from(_: RetentionAreaChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
