...
sidebar_label: Portfolio sunburst
description: Fission Charts reference for Portfolio sunburst.
---

# Portfolio sunburst

![Portfolio sunburst screenshot](/img/charts/analytics-sunburst-portfolio.png)

Portfolio sunburst uses Fission Charts typed Rust data to render a production-ready analytics view. Use it for hierarchical portfolio composition. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Portfolio sunburst is part of the **Analytics** chart family and is intended for `Portfolio hierarchy in radial layers`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Portfolio hierarchy in radial layers.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct PortfolioSunburstChart;

impl From<PortfolioSunburstChart> for Widget {
    fn from(_: PortfolioSunburstChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
