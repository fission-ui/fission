---
title: Journey graph
sidebar_label: Journey graph
description: Fission Charts reference for Journey graph.
---

# Journey graph

![Journey graph screenshot](/img/charts/analytics-journey-graph.png)

Journey graph uses Fission Charts typed Rust data to render a production-ready analytics view. Use it for relationship-heavy behavior views. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Journey graph is part of the **Analytics** chart family and is intended for `Journey nodes and links`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Journey nodes and links.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct JourneyGraphChart;

impl From<JourneyGraphChart> for Widget {
    fn from(_: JourneyGraphChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
