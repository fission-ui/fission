...
sidebar_label: Alert annotations
description: Fission Charts reference for Alert annotations.
---
...
Alert annotations uses Fission Charts typed Rust data to render a production-ready line view. Use it when incidents or milestones need to stay attached to the trend. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Alert annotations is part of the **Line** chart family and is intended for `Ordered values with event markers`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Ordered values with event markers.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct AlertAnnotationsChart;

impl From<AlertAnnotationsChart> for Widget {
    fn from(_: AlertAnnotationsChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
