...
sidebar_label: Inventory end-step
description: Fission Charts reference for Inventory end-step.
---

# Inventory end-step

![Inventory end-step screenshot](/img/charts/line-inventory-end-step.png)

Inventory end-step uses Fission Charts typed Rust data to render a production-ready line view. Use it when the value changes after the period closes. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Inventory end-step is part of the **Line** chart family and is intended for `Discrete samples that update at the end of each interval`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Discrete samples that update at the end of each interval.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct InventoryEndStepChart;

impl From<InventoryEndStepChart> for Widget {
    fn from(_: InventoryEndStepChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
