...
sidebar_label: Service latency trend
description: Fission Charts reference for Service latency trend.
---

# Service latency trend

![Service latency trend screenshot](/img/charts/line-service-latency.png)

Service latency trend uses Fission Charts typed Rust data to render a production-ready line view. Use it for operations screens that need drift and spikes in one glance. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Service latency trend is part of the **Line** chart family and is intended for `Latency samples over ordered time buckets`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Latency samples over ordered time buckets.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ServiceLatencyTrendChart;

impl From<ServiceLatencyTrendChart> for Widget {
    fn from(_: ServiceLatencyTrendChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
