...
sidebar_label: Log-shaped line
description: Fission Charts reference for Log-shaped line.
---

# Log-shaped line

![Log-shaped line screenshot](/img/charts/line-log-shaped.png)

A line with rapid early growth and slower later growth. Use it for adoption curves and saturation effects. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Log-shaped line is part of the **Line** chart family and is intended for `Vec<f32> with nonlinear values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Vec<f32> with nonlinear values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct LogShapedLineChart;

impl From<LogShapedLineChart> for Widget {
    fn from(_: LogShapedLineChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
