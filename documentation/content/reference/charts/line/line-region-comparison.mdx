...
sidebar_label: Regional trend comparison
description: Fission Charts reference for Regional trend comparison.
---

# Regional trend comparison

![Regional trend comparison screenshot](/img/charts/line-region-comparison.png)

Regional trend comparison uses Fission Charts typed Rust data to render a production-ready line view. Use it when teams compare regions over the same period. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Regional trend comparison is part of the **Line** chart family and is intended for `Two regional series on shared axes`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Two regional series on shared axes.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct RegionalTrendComparisonChart;

impl From<RegionalTrendComparisonChart> for Widget {
    fn from(_: RegionalTrendComparisonChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
