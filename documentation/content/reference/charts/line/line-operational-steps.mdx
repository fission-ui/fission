...
sidebar_label: Operational step series
description: Fission Charts reference for Operational step series.
---

# Operational step series

![Operational step series screenshot](/img/charts/line-operational-steps.png)

Operational step series uses Fission Charts typed Rust data to render a production-ready line view. Use it for state changes, quotas, or inventory counts. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Operational step series is part of the **Line** chart family and is intended for `Discrete states represented as stepped values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Discrete states represented as stepped values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct OperationalStepSeriesChart;

impl From<OperationalStepSeriesChart> for Widget {
    fn from(_: OperationalStepSeriesChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
