...
Line with alert events is part of the **Line** chart family and is intended for `LineSeries plus MarkPoint`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct LineWithAlertEventsChart;

impl From<LineWithAlertEventsChart> for Widget {
    fn from(_: LineWithAlertEventsChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
