...
sidebar_label: Support volume area
description: Fission Charts reference for Support volume area.
---

# Support volume area

![Support volume area screenshot](/img/charts/line-support-volume.png)

Support volume area uses Fission Charts typed Rust data to render a production-ready line view. Use it when volume should be visible at a glance. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Support volume area is part of the **Line** chart family and is intended for `Support counts with an area fill`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Support counts with an area fill.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct SupportVolumeAreaChart;

impl From<SupportVolumeAreaChart> for Widget {
    fn from(_: SupportVolumeAreaChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
