...
sidebar_label: Error budget burn
description: Fission Charts reference for Error budget burn.
---

# Error budget burn

![Error budget burn screenshot](/img/charts/line-error-budget.png)

Error budget burn uses Fission Charts typed Rust data to render a production-ready line view. Use it when teams need to see whether a service is inside its operating band. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Error budget burn is part of the **Line** chart family and is intended for `Budget percentage samples plus threshold marks`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Budget percentage samples plus threshold marks.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ErrorBudgetBurnChart;

impl From<ErrorBudgetBurnChart> for Widget {
    fn from(_: ErrorBudgetBurnChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
