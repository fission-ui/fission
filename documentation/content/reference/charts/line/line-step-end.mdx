---
title: Step end line
sidebar_label: Step end line
description: Fission Charts reference for Step end line.
---

# Step end line

![Step end line screenshot](/img/charts/line-step-end.png)

Discrete state changes drawn at interval ends. Use it when values settle at the end of each interval. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Step end line is part of the **Line** chart family and is intended for `Vec<f32> with end step behavior`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Vec<f32> with end step behavior.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct StepEndLineChart;

impl From<StepEndLineChart> for Widget {
    fn from(_: StepEndLineChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
