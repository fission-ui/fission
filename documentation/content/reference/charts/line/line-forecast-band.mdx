---
title: Forecast band
sidebar_label: Forecast band
description: Fission Charts reference for Forecast band.
---

# Forecast band

![Forecast band screenshot](/img/charts/line-forecast-band.png)

Line chart with an expected range behind the observed values. Use it when forecasts need uncertainty context. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Forecast band is part of the **Line** chart family and is intended for `Vec<f32> plus MarkArea`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Vec<f32> plus MarkArea.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ForecastBandChart;

impl From<ForecastBandChart> for Widget {
    fn from(_: ForecastBandChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
