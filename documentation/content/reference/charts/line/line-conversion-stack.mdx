...
sidebar_label: Conversion stack area
description: Fission Charts reference for Conversion stack area.
---

# Conversion stack area

![Conversion stack area screenshot](/img/charts/line-conversion-stack.png)

Conversion stack area uses Fission Charts typed Rust data to render a production-ready line view. Use it to explain how categories make up a total over time. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Conversion stack area is part of the **Line** chart family and is intended for `Several ordered series sharing a stack key`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Several ordered series sharing a stack key.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ConversionStackAreaChart;

impl From<ConversionStackAreaChart> for Widget {
    fn from(_: ConversionStackAreaChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
