...
sidebar_label: Annotated line
description: Fission Charts reference for Annotated line.
---

# Annotated line

![Annotated line screenshot](/img/charts/line-annotation.png)

Line chart with a callout annotation in the plot area. Use it to explain a visible change directly on the chart. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Annotated line is part of the **Line** chart family and is intended for `LineSeries plus ChartGraphic`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`LineSeries plus ChartGraphic.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct AnnotatedLineChart;

impl From<AnnotatedLineChart> for Widget {
    fn from(_: AnnotatedLineChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
