...
sidebar_label: Quality operating band
description: Fission Charts reference for Quality operating band.
---

# Quality operating band

![Quality operating band screenshot](/img/charts/line-quality-band.png)

Quality operating band uses Fission Charts typed Rust data to render a production-ready line view. Use it for production quality metrics with explicit guardrails. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Quality operating band is part of the **Line** chart family and is intended for `Samples with a target and acceptable range`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Samples with a target and acceptable range.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct QualityOperatingBandChart;

impl From<QualityOperatingBandChart> for Widget {
    fn from(_: QualityOperatingBandChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
