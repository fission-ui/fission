...
sidebar_label: API throughput line
description: Fission Charts reference for API throughput line.
---

# API throughput line

![API throughput line screenshot](/img/charts/line-api-throughput.png)

API throughput line uses Fission Charts typed Rust data to render a production-ready line view. Use it for telemetry panels with many points. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

API throughput line is part of the **Line** chart family and is intended for `Dense ordered samples from a service counter`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Dense ordered samples from a service counter.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct APIThroughputLineChart;

impl From<APIThroughputLineChart> for Widget {
    fn from(_: APIThroughputLineChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
