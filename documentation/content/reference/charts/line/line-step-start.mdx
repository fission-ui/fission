...
sidebar_label: Step start line
description: Fission Charts reference for Step start line.
---

# Step start line

![Step start line screenshot](/img/charts/line-step-start.png)

Discrete state changes drawn as step segments. Use it when values jump at the start of each interval. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Step start line is part of the **Line** chart family and is intended for `Vec<f32> with start step behavior`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Vec<f32> with start step behavior.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct StepStartLineChart;

impl From<StepStartLineChart> for Widget {
    fn from(_: StepStartLineChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
