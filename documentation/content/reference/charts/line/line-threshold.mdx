...
sidebar_label: Line with threshold
description: Fission Charts reference for Line with threshold.
---

# Line with threshold

![Line with threshold screenshot](/img/charts/line-threshold.png)

Line chart with a target line and highlighted operating band. Use it for service levels, quotas, and alert thresholds. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Line with threshold is part of the **Line** chart family and is intended for `Vec<f32> plus MarkLine and MarkArea`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Vec<f32> plus MarkLine and MarkArea.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct LineWithThresholdChart;

impl From<LineWithThresholdChart> for Widget {
    fn from(_: LineWithThresholdChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
