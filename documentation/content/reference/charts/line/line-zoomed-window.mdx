...
sidebar_label: Zoomed line window
description: Fission Charts reference for Zoomed line window.
---

# Zoomed line window

![Zoomed line window screenshot](/img/charts/line-zoomed-window.png)

Ordered line data filtered through a data zoom window. Use it for long series with local inspection. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Zoomed line window is part of the **Line** chart family and is intended for `LineSeries plus DataZoom`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`LineSeries plus DataZoom.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ZoomedLineWindowChart;

impl From<ZoomedLineWindowChart> for Widget {
    fn from(_: ZoomedLineWindowChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
