...
Retention zoom window is part of the **Line** chart family and is intended for `Long ordered samples with a visible zoom window`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct RetentionZoomWindowChart;

impl From<RetentionZoomWindowChart> for Widget {
    fn from(_: RetentionZoomWindowChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
