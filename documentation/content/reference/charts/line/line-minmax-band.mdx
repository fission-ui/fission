---
title: Min/max band
sidebar_label: Min/max band
description: Fission Charts reference for Min/max band.
---

# Min/max band

![Min/max band screenshot](/img/charts/line-minmax-band.png)

Trend line with upper and lower guide marks. Use it to show expected bounds around a metric. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Min/max band is part of the **Line** chart family and is intended for `Vec<f32> plus mark lines`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Vec<f32> plus mark lines.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct MinMaxBandChart;

impl From<MinMaxBandChart> for Widget {
    fn from(_: MinMaxBandChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
