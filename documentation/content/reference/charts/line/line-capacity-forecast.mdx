...
sidebar_label: Capacity forecast band
description: Fission Charts reference for Capacity forecast band.
---

# Capacity forecast band

![Capacity forecast band screenshot](/img/charts/line-capacity-forecast.png)

Capacity forecast band uses Fission Charts typed Rust data to render a production-ready line view. Use it when a forecast needs both the line and the safe range. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Capacity forecast band is part of the **Line** chart family and is intended for `Observed values with a highlighted expected range`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Observed values with a highlighted expected range.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct CapacityForecastBandChart;

impl From<CapacityForecastBandChart> for Widget {
    fn from(_: CapacityForecastBandChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
