...
sidebar_label: Traffic rolling average
description: Fission Charts reference for Traffic rolling average.
---

# Traffic rolling average

![Traffic rolling average screenshot](/img/charts/line-traffic-rolling-average.png)

Traffic rolling average uses Fission Charts typed Rust data to render a production-ready line view. Use it when the user needs the current signal and the trend baseline. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Traffic rolling average is part of the **Line** chart family and is intended for `Raw and smoothed ordered series`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Raw and smoothed ordered series.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct TrafficRollingAverageChart;

impl From<TrafficRollingAverageChart> for Widget {
    fn from(_: TrafficRollingAverageChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
