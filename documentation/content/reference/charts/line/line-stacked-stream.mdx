...
sidebar_label: Stacked stream area
description: Fission Charts reference for Stacked stream area.
---

# Stacked stream area

![Stacked stream area screenshot](/img/charts/line-stacked-stream.png)

Stacked area series showing total and composition. Use it for composition over time. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Stacked stream area is part of the **Line** chart family and is intended for `Multiple LineSeries with one stack key`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Multiple LineSeries with one stack key.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct StackedStreamAreaChart;

impl From<StackedStreamAreaChart> for Widget {
    fn from(_: StackedStreamAreaChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
