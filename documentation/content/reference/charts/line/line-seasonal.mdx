---
title: Seasonal line
sidebar_label: Seasonal line
description: Fission Charts reference for Seasonal line.
---

# Seasonal line

![Seasonal line screenshot](/img/charts/line-seasonal.png)

Trend samples shaped as a seasonal curve. Use it for seasonal demand and capacity planning. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Seasonal line is part of the **Line** chart family and is intended for `Vec<f32> over ordered periods`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Vec<f32> over ordered periods.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct SeasonalLineChart;

impl From<SeasonalLineChart> for Widget {
    fn from(_: SeasonalLineChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
