...
sidebar_label: Demand sparkline
description: Fission Charts reference for Demand sparkline.
---

# Demand sparkline

![Demand sparkline screenshot](/img/charts/line-demand-spark.png)

Demand sparkline uses Fission Charts typed Rust data to render a production-ready line view. Use it inside dense dashboard cards. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Demand sparkline is part of the **Line** chart family and is intended for `Compact dense ordered values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Compact dense ordered values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DemandSparklineChart;

impl From<DemandSparklineChart> for Widget {
    fn from(_: DemandSparklineChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
