...
sidebar_label: Market index line
description: Fission Charts reference for Market index line.
---

# Market index line

![Market index line screenshot](/img/charts/line-market-index.png)

Market index line uses Fission Charts typed Rust data to render a production-ready line view. Use it when direction and volatility matter more than individual samples. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Market index line is part of the **Line** chart family and is intended for `Indexed ordered values on a value axis`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Indexed ordered values on a value axis.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct MarketIndexLineChart;

impl From<MarketIndexLineChart> for Widget {
    fn from(_: MarketIndexLineChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
