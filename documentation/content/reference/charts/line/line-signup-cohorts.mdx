...
sidebar_label: Signup cohort comparison
description: Fission Charts reference for Signup cohort comparison.
---

# Signup cohort comparison

![Signup cohort comparison screenshot](/img/charts/line-signup-cohorts.png)

Signup cohort comparison uses Fission Charts typed Rust data to render a production-ready line view. Use it when users need to compare cohorts over the same time range. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Signup cohort comparison is part of the **Line** chart family and is intended for `Two ordered series sharing the same axis`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Two ordered series sharing the same axis.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct SignupCohortComparisonChart;

impl From<SignupCohortComparisonChart> for Widget {
    fn from(_: SignupCohortComparisonChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
