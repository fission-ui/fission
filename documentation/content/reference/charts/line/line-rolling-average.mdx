...
sidebar_label: Rolling average line
description: Fission Charts reference for Rolling average line.
---

# Rolling average line

![Rolling average line screenshot](/img/charts/line-rolling-average.png)

Raw metric and smoothed average on one chart. Use it when readers need both movement and trend. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Rolling average line is part of the **Line** chart family and is intended for `Two Vec<f32> series`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Two Vec<f32> series.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct RollingAverageLineChart;

impl From<RollingAverageLineChart> for Widget {
    fn from(_: RollingAverageLineChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
