...
sidebar_label: Weekly cycle line
description: Fission Charts reference for Weekly cycle line.
---

# Weekly cycle line

![Weekly cycle line screenshot](/img/charts/line-weekly-cycle.png)

Ordered samples over a repeating weekly cycle. Use it for operational rhythms and weekly dashboards. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Weekly cycle line is part of the **Line** chart family and is intended for `Vec<f32> aligned to weekdays`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Vec<f32> aligned to weekdays.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct WeeklyCycleLineChart;

impl From<WeeklyCycleLineChart> for Widget {
    fn from(_: WeeklyCycleLineChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
