...
sidebar_label: Revenue seasonality
description: Fission Charts reference for Revenue seasonality.
---

# Revenue seasonality

![Revenue seasonality screenshot](/img/charts/line-revenue-seasonality.png)

Revenue seasonality uses Fission Charts typed Rust data to render a production-ready line view. Use it when weekly or monthly rhythm is the main signal. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Revenue seasonality is part of the **Line** chart family and is intended for `Ordered samples over a repeated seasonal interval`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Ordered samples over a repeated seasonal interval.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct RevenueSeasonalityChart;

impl From<RevenueSeasonalityChart> for Widget {
    fn from(_: RevenueSeasonalityChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
