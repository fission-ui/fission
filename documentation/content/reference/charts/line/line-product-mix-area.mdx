...
sidebar_label: Product mix area
description: Fission Charts reference for Product mix area.
---

# Product mix area

![Product mix area screenshot](/img/charts/line-product-mix-area.png)

Product mix area uses Fission Charts typed Rust data to render a production-ready line view. Use it when the total and category mix both matter. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Product mix area is part of the **Line** chart family and is intended for `Stacked product series over ordered buckets`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Stacked product series over ordered buckets.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ProductMixAreaChart;

impl From<ProductMixAreaChart> for Widget {
    fn from(_: ProductMixAreaChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
