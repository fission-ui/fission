...
sidebar_label: Dense spark line
description: Fission Charts reference for Dense spark line.
---

# Dense spark line

![Dense spark line screenshot](/img/charts/line-spark-dense.png)

Compact dense line with many telemetry samples. Use it for monitoring and telemetry surfaces. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Dense spark line is part of the **Line** chart family and is intended for `Long Vec<f32> on ordered categories`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Long Vec<f32> on ordered categories.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DenseSparkLineChart;

impl From<DenseSparkLineChart> for Widget {
    fn from(_: DenseSparkLineChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
