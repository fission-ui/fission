...
sidebar_label: Gradient area line
description: Fission Charts reference for Gradient area line.
---

# Gradient area line

![Gradient area line screenshot](/img/charts/line-gradient-area.png)

Line chart with translucent area fill for volume emphasis. Use it when magnitude and trend should be read together. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Gradient area line is part of the **Line** chart family and is intended for `Vec<f32> with area style`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Vec<f32> with area style.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct GradientAreaLineChart;

impl From<GradientAreaLineChart> for Widget {
    fn from(_: GradientAreaLineChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
