...
sidebar_label: Deployment event line
description: Fission Charts reference for Deployment event line.
---

# Deployment event line

![Deployment event line screenshot](/img/charts/line-deployment-events.png)

Deployment event line uses Fission Charts typed Rust data to render a production-ready line view. Use it to connect product changes to metric movement. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Deployment event line is part of the **Line** chart family and is intended for `Trend values with deployment markers`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Trend values with deployment markers.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DeploymentEventLineChart;

impl From<DeploymentEventLineChart> for Widget {
    fn from(_: DeploymentEventLineChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
