...
sidebar_label: Dual line comparison
description: Fission Charts reference for Dual line comparison.
---

# Dual line comparison

![Dual line comparison screenshot](/img/charts/line-dual-series.png)

Two related line series sharing one chart. Use it to compare two trends without changing units. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Dual line comparison is part of the **Line** chart family and is intended for `Two Vec<f32> series on shared axes`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Two Vec<f32> series on shared axes.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DualLineComparisonChart;

impl From<DualLineComparisonChart> for Widget {
    fn from(_: DualLineComparisonChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
