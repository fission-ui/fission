...
sidebar_label: Scatter clusters
description: Fission Charts reference for Scatter clusters.
---

# Scatter clusters

![Scatter clusters screenshot](/img/charts/scatter-clusters.png)

Point cloud showing clustered observations. Use it for correlation and clustering. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Scatter clusters is part of the **Scatter and statistical** chart family and is intended for `Vec<(f32, f32)>`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Vec<(f32, f32)>.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ScatterClustersChart;

impl From<ScatterClustersChart> for Widget {
    fn from(_: ScatterClustersChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
