...
sidebar_label: Candlestick with movement
description: Fission Charts reference for Candlestick with movement.
---
...
Open-close-low-high financial movement. Use it for market and range-over-time data. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Candlestick with movement is part of the **Scatter and statistical** chart family and is intended for `Rows of open, close, low, high values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Rows of open, close, low, high values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct CandlestickWithMovementChart;

impl From<CandlestickWithMovementChart> for Widget {
    fn from(_: CandlestickWithMovementChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
