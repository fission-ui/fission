...
sidebar_label: Latency boxplot
description: Fission Charts reference for Latency boxplot.
---

# Latency boxplot

![Latency boxplot screenshot](/img/charts/boxplot-latency.png)

Distribution summary for latency groups. Use it when distribution matters more than averages. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Latency boxplot is part of the **Scatter and statistical** chart family and is intended for `Five-number summary rows`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Five-number summary rows.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct LatencyBoxplotChart;

impl From<LatencyBoxplotChart> for Widget {
    fn from(_: LatencyBoxplotChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
