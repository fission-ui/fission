...
sidebar_label: Quality boxplot
description: Fission Charts reference for Quality boxplot.
---

# Quality boxplot

![Quality boxplot screenshot](/img/charts/boxplot-quality.png)

Boxplot comparing quality ranges. Use it for process quality and batch comparisons. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Quality boxplot is part of the **Scatter and statistical** chart family and is intended for `Five-number summary rows`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct QualityBoxplotChart;

impl From<QualityBoxplotChart> for Widget {
    fn from(_: QualityBoxplotChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
