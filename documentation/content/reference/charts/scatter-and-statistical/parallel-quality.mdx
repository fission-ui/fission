...
sidebar_label: Quality parallel coordinates
description: Fission Charts reference for Quality parallel coordinates.
---

# Quality parallel coordinates

![Quality parallel coordinates screenshot](/img/charts/parallel-quality.png)

Parallel coordinates for quality dimensions. Use it for filtering and comparing profiles. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Quality parallel coordinates is part of the **Scatter and statistical** chart family and is intended for `Vec<Vec<f32>> observations`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct QualityParallelCoordinatesChart;

impl From<QualityParallelCoordinatesChart> for Widget {
    fn from(_: QualityParallelCoordinatesChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
