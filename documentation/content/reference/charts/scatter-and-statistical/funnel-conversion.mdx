...
![Conversion funnel screenshot](/img/charts/funnel-conversion.png)

Stages drawn as a narrowing funnel. Use it for sales or onboarding funnels. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Conversion funnel is part of the **Scatter and statistical** chart family and is intended for `Ordered label/value pairs`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Ordered label/value pairs.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
