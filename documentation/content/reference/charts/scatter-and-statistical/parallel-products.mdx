...
Product parallel coordinates is part of the **Scatter and statistical** chart family and is intended for `Vec<Vec<f32>> observations`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct ProductParallelCoordinatesChart;

impl From<ProductParallelCoordinatesChart> for Widget {
    fn from(_: ProductParallelCoordinatesChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
