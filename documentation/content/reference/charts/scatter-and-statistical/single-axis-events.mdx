...
sidebar_label: Event single axis
description: Fission Charts reference for Event single axis.
---

# Event single axis

![Event single axis screenshot](/img/charts/single-axis-events.png)

Weighted events along a single timeline. Use it for event density and distributions. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Event single axis is part of the **Scatter and statistical** chart family and is intended for `Vec<(value, size)>`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct EventSingleAxisChart;

impl From<EventSingleAxisChart> for Widget {
    fn from(_: EventSingleAxisChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
