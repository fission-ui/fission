...
sidebar_label: Scatter outliers
description: Fission Charts reference for Scatter outliers.
---

# Scatter outliers

![Scatter outliers screenshot](/img/charts/scatter-outliers.png)

Scatter chart with emphasized outlier points. Use it to make exceptions visible. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Scatter outliers is part of the **Scatter and statistical** chart family and is intended for `Vec<(f32, f32)> plus effect scatter styling`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Vec<(f32, f32)> plus effect scatter styling.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ScatterOutliersChart;

impl From<ScatterOutliersChart> for Widget {
    fn from(_: ScatterOutliersChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
