...
sidebar_label: Intraday candlestick
description: Fission Charts reference for Intraday candlestick.
---

# Intraday candlestick

![Intraday candlestick screenshot](/img/charts/candlestick-intraday.png)

Candlestick variant with tighter movement. Use it for intraday or short-range market views. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Intraday candlestick is part of the **Scatter and statistical** chart family and is intended for `OHLC rows`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`OHLC rows.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct IntradayCandlestickChart;

impl From<IntradayCandlestickChart> for Widget {
    fn from(_: IntradayCandlestickChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
