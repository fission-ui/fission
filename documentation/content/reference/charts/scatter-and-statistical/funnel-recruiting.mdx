...
sidebar_label: Recruiting funnel
description: Fission Charts reference for Recruiting funnel.
---

# Recruiting funnel

![Recruiting funnel screenshot](/img/charts/funnel-recruiting.png)

Funnel chart for hiring stages. Use it when stage drop-off matters. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Recruiting funnel is part of the **Scatter and statistical** chart family and is intended for `Ordered label/value pairs`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct RecruitingFunnelChart;

impl From<RecruitingFunnelChart> for Widget {
    fn from(_: RecruitingFunnelChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
