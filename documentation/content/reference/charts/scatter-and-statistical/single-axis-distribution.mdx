...
sidebar_label: Distribution single axis
description: Fission Charts reference for Distribution single axis.
---

# Distribution single axis

![Distribution single axis screenshot](/img/charts/single-axis-distribution.png)

Weighted samples on one numeric scale. Use it when a second axis adds noise. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Distribution single axis is part of the **Scatter and statistical** chart family and is intended for `Vec<(value, size)>`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Vec<(value, size)>.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DistributionSingleAxisChart;

impl From<DistributionSingleAxisChart> for Widget {
    fn from(_: DistributionSingleAxisChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
