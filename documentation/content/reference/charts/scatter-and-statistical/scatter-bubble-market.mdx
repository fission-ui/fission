...
Market bubble chart is part of the **Scatter and statistical** chart family and is intended for `Vec<(x, y, size)>`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct MarketBubbleChart;

impl From<MarketBubbleChart> for Widget {
    fn from(_: MarketBubbleChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
