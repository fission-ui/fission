...
description: Fission Charts reference for Basic bar.
---

# Basic bar

![Basic bar screenshot](/img/charts/bar-basic.png)

A category comparison using rectangular bars. Use it when individual values need easy comparison. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Basic bar is part of the **Cartesian** chart family and is intended for `Vec<f32> aligned to category labels`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct BasicBarChart;

impl From<BasicBarChart> for Widget {
    fn from(_: BasicBarChart) -> Widget {
        Chart::new()
            .title("Category comparison")
            .x_axis(Axis::category(vec!["A", "B", "C", "D"]))
            .y_axis(Axis::value())
