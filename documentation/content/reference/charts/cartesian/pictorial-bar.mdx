...
Pictorial bar is part of the **Cartesian** chart family and is intended for `Numeric values plus symbol choice`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct PictorialBarChart;

impl From<PictorialBarChart> for Widget {
    fn from(_: PictorialBarChart) -> Widget {
        Chart::new()
            .title("Category comparison")
            .x_axis(Axis::category(vec!["A", "B", "C", "D"]))
            .y_axis(Axis::value())
