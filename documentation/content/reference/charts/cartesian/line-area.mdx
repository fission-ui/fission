---
title: Area line
sidebar_label: Area line
description: Fission Charts reference for Area line.
---
...
A line with filled area to emphasize magnitude over time. Use it when total volume is as important as the outline. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Area line is part of the **Cartesian** chart family and is intended for `Vec<f32> plus area style`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Vec<f32> plus area style.` Keep the data close to the type that describes it. Fission Charts is typed Rust, so a line uses `LineSeries`, a calendar heatmap uses `CalendarHeatmapSeries`, and chart components such as zoom, marks, and graphics are explicit fields on `Chart` rather than hidden string configuration.
...
impl From<AreaLineChart> for Widget {
    fn from(_: AreaLineChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]))
            .y_axis(Axis::value())
