...
description: Fission Charts reference for Stacked area.
---

# Stacked area

![Stacked area screenshot](/img/charts/line-stacked-area.png)

Multiple area series stacked into one total. Use it to show composition over time without losing the total. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Stacked area is part of the **Cartesian** chart family and is intended for `Several line series sharing one stack key`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
impl From<StackedAreaChart> for Widget {
    fn from(_: StackedAreaChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]))
            .y_axis(Axis::value())
