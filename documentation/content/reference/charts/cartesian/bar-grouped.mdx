...
description: Fission Charts reference for Grouped bar.
---

# Grouped bar

![Grouped bar screenshot](/img/charts/bar-grouped.png)

Several bar series shown side by side for each category. Use it to compare related measures within each group. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Grouped bar is part of the **Cartesian** chart family and is intended for `Multiple bar series sharing the same category axis`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
impl From<GroupedBarChart> for Widget {
    fn from(_: GroupedBarChart) -> Widget {
        Chart::new()
            .title("Category comparison")
            .x_axis(Axis::category(vec!["A", "B", "C", "D"]))
            .y_axis(Axis::value())
