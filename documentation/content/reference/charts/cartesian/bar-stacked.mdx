...
Stacked bar is part of the **Cartesian** chart family and is intended for `Multiple bar series sharing one stack key`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
impl From<StackedBarChart> for Widget {
    fn from(_: StackedBarChart) -> Widget {
        Chart::new()
            .title("Category comparison")
            .x_axis(Axis::category(vec!["A", "B", "C", "D"]))
            .y_axis(Axis::value())
