---
title: Step line
sidebar_label: Step line
description: Fission Charts reference for Step line.
---

# Step line

![Step line screenshot](/img/charts/line-step.png)

A line that changes in discrete jumps instead of continuous slopes. Use it for counters, states, quotas, and event-driven changes. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Step line is part of the **Cartesian** chart family and is intended for `Vec<f32> with start, middle, or end step behavior`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct StepLineChart;

impl From<StepLineChart> for Widget {
    fn from(_: StepLineChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]))
            .y_axis(Axis::value())
