...
sidebar_label: Market regions map
description: Fission Charts reference for Market regions map.
---

# Market regions map

![Market regions map screenshot](/img/charts/map-market-regions.png)

Market regions map uses Fission Charts typed Rust data to render a production-ready geo view. Use it for regional comparisons where shape matters. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Market regions map is part of the **Geo** chart family and is intended for `Named regions with values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Named regions with values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct MarketRegionsMapChart;

impl From<MarketRegionsMapChart> for Widget {
    fn from(_: MarketRegionsMapChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
