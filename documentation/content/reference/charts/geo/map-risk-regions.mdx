...
sidebar_label: Risk regions map
description: Fission Charts reference for Risk regions map.
---

# Risk regions map

![Risk regions map screenshot](/img/charts/map-risk-regions.png)

Risk regions map uses Fission Charts typed Rust data to render a production-ready geo view. Use it to scan geographic risk. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Risk regions map is part of the **Geo** chart family and is intended for `Region values mapped to color`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Region values mapped to color.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct RiskRegionsMapChart;

impl From<RiskRegionsMapChart> for Widget {
    fn from(_: RiskRegionsMapChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
