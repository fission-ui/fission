...
sidebar_label: Network traffic lines
description: Fission Charts reference for Network traffic lines.
---

# Network traffic lines

![Network traffic lines screenshot](/img/charts/lines-network-traffic.png)

Network traffic lines uses Fission Charts typed Rust data to render a production-ready geo view. Use it for flow over a spatial surface. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Network traffic lines is part of the **Geo** chart family and is intended for `Route lines with effect markers`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Route lines with effect markers.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct NetworkTrafficLinesChart;

impl From<NetworkTrafficLinesChart> for Widget {
    fn from(_: NetworkTrafficLinesChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
