...
sidebar_label: Courier routes
description: Fission Charts reference for Courier routes.
---

# Courier routes

![Courier routes screenshot](/img/charts/lines-courier-routes.png)

Courier routes uses Fission Charts typed Rust data to render a production-ready geo view. Use it for logistics routing. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Courier routes is part of the **Geo** chart family and is intended for `Point-to-point route lines`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Point-to-point route lines.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct CourierRoutesChart;

impl From<CourierRoutesChart> for Widget {
    fn from(_: CourierRoutesChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
