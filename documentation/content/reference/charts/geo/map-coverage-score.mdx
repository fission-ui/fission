...
sidebar_label: Coverage score map
description: Fission Charts reference for Coverage score map.
---

# Coverage score map

![Coverage score map screenshot](/img/charts/map-coverage-score.png)

Coverage score map uses Fission Charts typed Rust data to render a production-ready geo view. Use it for service coverage surfaces. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Coverage score map is part of the **Geo** chart family and is intended for `Coverage scores by region`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Coverage scores by region.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct CoverageScoreMapChart;

impl From<CoverageScoreMapChart> for Widget {
    fn from(_: CoverageScoreMapChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
