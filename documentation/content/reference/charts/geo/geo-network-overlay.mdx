...
sidebar_label: Network overlay
description: Fission Charts reference for Network overlay.
---

# Network overlay

![Network overlay screenshot](/img/charts/geo-network-overlay.png)

Network overlay uses Fission Charts typed Rust data to render a production-ready geo view. Use it for linked geographic systems. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Network overlay is part of the **Geo** chart family and is intended for `Region values with connection lines`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Region values with connection lines.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct NetworkOverlayChart;

impl From<NetworkOverlayChart> for Widget {
    fn from(_: NetworkOverlayChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
