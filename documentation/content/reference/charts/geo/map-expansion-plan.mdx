...
sidebar_label: Expansion plan map
description: Fission Charts reference for Expansion plan map.
---

# Expansion plan map

![Expansion plan map screenshot](/img/charts/map-expansion-plan.png)

Expansion plan map uses Fission Charts typed Rust data to render a production-ready geo view. Use it for geographic planning dashboards. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Expansion plan map is part of the **Geo** chart family and is intended for `Planning values by region`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Planning values by region.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ExpansionPlanMapChart;

impl From<ExpansionPlanMapChart> for Widget {
    fn from(_: ExpansionPlanMapChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
