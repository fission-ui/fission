...
sidebar_label: Sales route map
description: Fission Charts reference for Sales route map.
---

# Sales route map

![Sales route map screenshot](/img/charts/geo-sales-routes.png)

Sales route map uses Fission Charts typed Rust data to render a production-ready geo view. Use it for field team planning. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Sales route map is part of the **Geo** chart family and is intended for `Sales territories with route overlays`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Sales territories with route overlays.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct SalesRouteMapChart;

impl From<SalesRouteMapChart> for Widget {
    fn from(_: SalesRouteMapChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
