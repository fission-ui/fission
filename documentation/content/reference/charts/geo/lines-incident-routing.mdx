...
sidebar_label: Incident routing lines
description: Fission Charts reference for Incident routing lines.
---

# Incident routing lines

![Incident routing lines screenshot](/img/charts/lines-incident-routing.png)

Incident routing lines uses Fission Charts typed Rust data to render a production-ready geo view. Use it for dispatch and response analytics. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Incident routing lines is part of the **Geo** chart family and is intended for `Routes representing response movement`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Routes representing response movement.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct IncidentRoutingLinesChart;

impl From<IncidentRoutingLinesChart> for Widget {
    fn from(_: IncidentRoutingLinesChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
