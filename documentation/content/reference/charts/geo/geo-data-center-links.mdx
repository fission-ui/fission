...
sidebar_label: Data center links
description: Fission Charts reference for Data center links.
---

# Data center links

![Data center links screenshot](/img/charts/geo-data-center-links.png)

Data center links uses Fission Charts typed Rust data to render a production-ready geo view. Use it for infrastructure network views. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Data center links is part of the **Geo** chart family and is intended for `Route-like links between points`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Route-like links between points.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DataCenterLinksChart;

impl From<DataCenterLinksChart> for Widget {
    fn from(_: DataCenterLinksChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
