...
sidebar_label: Service health map
description: Fission Charts reference for Service health map.
---

# Service health map

![Service health map screenshot](/img/charts/map-service-health.png)

Service health map uses Fission Charts typed Rust data to render a production-ready geo view. Use it for operational geography. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Service health map is part of the **Geo** chart family and is intended for `Region health values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Region health values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ServiceHealthMapChart;

impl From<ServiceHealthMapChart> for Widget {
    fn from(_: ServiceHealthMapChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
