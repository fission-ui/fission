...
sidebar_label: Migration flow
description: Fission Charts reference for Migration flow.
---

# Migration flow

![Migration flow screenshot](/img/charts/geo-migration-flow.png)

Migration flow uses Fission Charts typed Rust data to render a production-ready geo view. Use it for source-to-destination stories. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Migration flow is part of the **Geo** chart family and is intended for `Region map plus movement routes`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Region map plus movement routes.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct MigrationFlowChart;

impl From<MigrationFlowChart> for Widget {
    fn from(_: MigrationFlowChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
