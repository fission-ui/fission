...
sidebar_label: Sales territory map
description: Fission Charts reference for Sales territory map.
---

# Sales territory map

![Sales territory map screenshot](/img/charts/map-sales-territory.png)

Sales territory map uses Fission Charts typed Rust data to render a production-ready geo view. Use it when territory is the mental model. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Sales territory map is part of the **Geo** chart family and is intended for `Territory values over named regions`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Territory values over named regions.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct SalesTerritoryMapChart;

impl From<SalesTerritoryMapChart> for Widget {
    fn from(_: SalesTerritoryMapChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
