...
sidebar_label: Capacity regions map
description: Fission Charts reference for Capacity regions map.
---
...
Capacity regions map uses Fission Charts typed Rust data to render a production-ready geo view. Use it for capacity by geography. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Capacity regions map is part of the **Geo** chart family and is intended for `Region values with visual scale`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Region values with visual scale.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct CapacityRegionsMapChart;

impl From<CapacityRegionsMapChart> for Widget {
    fn from(_: CapacityRegionsMapChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
