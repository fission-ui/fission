...
description: Fission Charts reference for Boxplot.
---
...
A distribution summary showing min, quartiles, median, and max. Use it when distribution matters more than a single average. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Boxplot is part of the **Statistical and finance** chart family and is intended for `Rows of five-number summaries or raw groups`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
impl From<BoxplotChart> for Widget {
    fn from(_: BoxplotChart) -> Widget {
        Chart::new()
            .title("Boxplot")
            .width(720.0)
            .height(480.0)
