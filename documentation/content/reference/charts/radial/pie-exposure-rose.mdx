...
sidebar_label: Exposure area rose
description: Fission Charts reference for Exposure area rose.
---

# Exposure area rose

![Exposure area rose screenshot](/img/charts/pie-exposure-rose.png)

Exposure area rose uses Fission Charts typed Rust data to render a production-ready radial view. Use it when relative shape is more important than a precise angle. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Exposure area rose is part of the **Radial** chart family and is intended for `Label/value pairs with area-oriented rose layout`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Label/value pairs with area-oriented rose layout.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ExposureAreaRoseChart;

impl From<ExposureAreaRoseChart> for Widget {
    fn from(_: ExposureAreaRoseChart) -> Widget {
        Chart::new()
            .title("Share")
            .legend(Legend::top_right())
            .series(vec![PieSeries::new("Share")
