...
sidebar_label: Wind speed polar line
description: Fission Charts reference for Wind speed polar line.
---

# Wind speed polar line

![Wind speed polar line screenshot](/img/charts/polar-wind-speed.png)

Wind speed polar line uses Fission Charts typed Rust data to render a production-ready radial view. Use it for directional or cyclic measures. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Wind speed polar line is part of the **Radial** chart family and is intended for `Angle/radius samples`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Angle/radius samples.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct WindSpeedPolarLineChart;

impl From<WindSpeedPolarLineChart> for Widget {
    fn from(_: WindSpeedPolarLineChart) -> Widget {
        Chart::new()
            .title("Share")
            .legend(Legend::top_right())
            .series(vec![PieSeries::new("Share")
