---
title: Risk rose
sidebar_label: Risk rose
description: Fission Charts reference for Risk rose.
---

# Risk rose

![Risk rose screenshot](/img/charts/pie-risk-rose.png)

Risk rose uses Fission Charts typed Rust data to render a production-ready radial view. Use it for executive risk snapshots where visual emphasis helps. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Risk rose is part of the **Radial** chart family and is intended for `Risk category values in radial form`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Risk category values in radial form.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct RiskRoseChart;

impl From<RiskRoseChart> for Widget {
    fn from(_: RiskRoseChart) -> Widget {
        Chart::new()
            .title("Share")
            .legend(Legend::top_right())
            .series(vec![PieSeries::new("Share")
