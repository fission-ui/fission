...
sidebar_label: Build confidence gauge
description: Fission Charts reference for Build confidence gauge.
---

# Build confidence gauge

![Build confidence gauge screenshot](/img/charts/gauge-build-confidence.png)

Build confidence gauge uses Fission Charts typed Rust data to render a production-ready radial view. Use it when a status panel needs a primary quality number. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Build confidence gauge is part of the **Radial** chart family and is intended for `One confidence score on a gauge arc`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`One confidence score on a gauge arc.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct BuildConfidenceGaugeChart;

impl From<BuildConfidenceGaugeChart> for Widget {
    fn from(_: BuildConfidenceGaugeChart) -> Widget {
        Chart::new()
            .title("Share")
            .legend(Legend::top_right())
            .series(vec![PieSeries::new("Share")
