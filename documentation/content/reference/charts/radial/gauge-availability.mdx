...
sidebar_label: Availability gauge
description: Fission Charts reference for Availability gauge.
---
...
Availability gauge uses Fission Charts typed Rust data to render a production-ready radial view. Use it when a single current status dominates the screen. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Availability gauge is part of the **Radial** chart family and is intended for `One bounded value mapped to an arc`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`One bounded value mapped to an arc.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
