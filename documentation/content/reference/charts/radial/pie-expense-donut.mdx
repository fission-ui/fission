---
title: Expense donut
sidebar_label: Expense donut
description: Fission Charts reference for Expense donut.
---

# Expense donut

![Expense donut screenshot](/img/charts/pie-expense-donut.png)

Expense donut uses Fission Charts typed Rust data to render a production-ready radial view. Use it for budget composition. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Expense donut is part of the **Radial** chart family and is intended for `Expense categories as a small whole`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Expense categories as a small whole.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ExpenseDonutChart;

impl From<ExpenseDonutChart> for Widget {
    fn from(_: ExpenseDonutChart) -> Widget {
        Chart::new()
            .title("Share")
            .legend(Legend::top_right())
            .series(vec![PieSeries::new("Share")
