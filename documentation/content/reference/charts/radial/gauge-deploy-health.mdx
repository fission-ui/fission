...
sidebar_label: Deploy health gauge
description: Fission Charts reference for Deploy health gauge.
---

# Deploy health gauge

![Deploy health gauge screenshot](/img/charts/gauge-deploy-health.png)

Deploy health gauge uses Fission Charts typed Rust data to render a production-ready radial view. Use it for health and readiness summaries. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Deploy health gauge is part of the **Radial** chart family and is intended for `Single score value in a bounded range`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Single score value in a bounded range.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DeployHealthGaugeChart;

impl From<DeployHealthGaugeChart> for Widget {
    fn from(_: DeployHealthGaugeChart) -> Widget {
        Chart::new()
            .title("Share")
            .legend(Legend::top_right())
            .series(vec![PieSeries::new("Share")
