...
sidebar_label: Platform readiness radar
description: Fission Charts reference for Platform readiness radar.
---

# Platform readiness radar

![Platform readiness radar screenshot](/img/charts/radar-platform-readiness.png)

Platform readiness radar uses Fission Charts typed Rust data to render a production-ready radial view. Use it to summarize readiness across product areas. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Platform readiness radar is part of the **Radial** chart family and is intended for `Platform dimensions as comparable vectors`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Platform dimensions as comparable vectors.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct PlatformReadinessRadarChart;

impl From<PlatformReadinessRadarChart> for Widget {
    fn from(_: PlatformReadinessRadarChart) -> Widget {
        Chart::new()
            .title("Share")
            .legend(Legend::top_right())
            .series(vec![PieSeries::new("Share")
