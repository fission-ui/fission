...
sidebar_label: Cycle polar line
description: Fission Charts reference for Cycle polar line.
---

# Cycle polar line

![Cycle polar line screenshot](/img/charts/polar-cycle-line.png)

Cycle polar line uses Fission Charts typed Rust data to render a production-ready radial view. Use it for circular process metrics. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Cycle polar line is part of the **Radial** chart family and is intended for `Angle/radius samples around a cycle`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Angle/radius samples around a cycle.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct CyclePolarLineChart;

impl From<CyclePolarLineChart> for Widget {
    fn from(_: CyclePolarLineChart) -> Widget {
        Chart::new()
            .title("Share")
            .legend(Legend::top_right())
            .series(vec![PieSeries::new("Share")
