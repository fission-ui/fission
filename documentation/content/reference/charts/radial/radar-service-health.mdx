...
sidebar_label: Service health radar
description: Fission Charts reference for Service health radar.
---

# Service health radar

![Service health radar screenshot](/img/charts/radar-service-health.png)

Service health radar uses Fission Charts typed Rust data to render a production-ready radial view. Use it for compact status summaries. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Service health radar is part of the **Radial** chart family and is intended for `Health dimensions on a radial profile`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Health dimensions on a radial profile.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ServiceHealthRadarChart;

impl From<ServiceHealthRadarChart> for Widget {
    fn from(_: ServiceHealthRadarChart) -> Widget {
        Chart::new()
            .title("Share")
            .legend(Legend::top_right())
            .series(vec![PieSeries::new("Share")
