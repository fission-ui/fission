...
sidebar_label: Revenue mix donut
description: Fission Charts reference for Revenue mix donut.
---

# Revenue mix donut

![Revenue mix donut screenshot](/img/charts/pie-revenue-mix.png)

Revenue mix donut uses Fission Charts typed Rust data to render a production-ready radial view. Use it when contribution to total revenue matters. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Revenue mix donut is part of the **Radial** chart family and is intended for `Revenue categories as label/value pairs`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Revenue categories as label/value pairs.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct RevenueMixDonutChart;

impl From<RevenueMixDonutChart> for Widget {
    fn from(_: RevenueMixDonutChart) -> Widget {
        Chart::new()
            .title("Share")
            .legend(Legend::top_right())
            .series(vec![PieSeries::new("Share")
