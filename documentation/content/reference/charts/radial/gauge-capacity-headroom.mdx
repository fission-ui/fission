...
sidebar_label: Capacity headroom gauge
description: Fission Charts reference for Capacity headroom gauge.
---

# Capacity headroom gauge

![Capacity headroom gauge screenshot](/img/charts/gauge-capacity-headroom.png)

Capacity headroom gauge uses Fission Charts typed Rust data to render a production-ready radial view. Use it when headroom is the only number that matters. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Capacity headroom gauge is part of the **Radial** chart family and is intended for `Single capacity value in a bounded range`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Single capacity value in a bounded range.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct CapacityHeadroomGaugeChart;

impl From<CapacityHeadroomGaugeChart> for Widget {
    fn from(_: CapacityHeadroomGaugeChart) -> Widget {
        Chart::new()
            .title("Share")
            .legend(Legend::top_right())
            .series(vec![PieSeries::new("Share")
