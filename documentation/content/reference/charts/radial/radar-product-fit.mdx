...
sidebar_label: Product fit radar
description: Fission Charts reference for Product fit radar.
---

# Product fit radar

![Product fit radar screenshot](/img/charts/radar-product-fit.png)

Product fit radar uses Fission Charts typed Rust data to render a production-ready radial view. Use it to compare shapes across a few dimensions. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Product fit radar is part of the **Radial** chart family and is intended for `Several same-length dimensional vectors`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Several same-length dimensional vectors.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ProductFitRadarChart;

impl From<ProductFitRadarChart> for Widget {
    fn from(_: ProductFitRadarChart) -> Widget {
        Chart::new()
            .title("Share")
            .legend(Legend::top_right())
            .series(vec![PieSeries::new("Share")
