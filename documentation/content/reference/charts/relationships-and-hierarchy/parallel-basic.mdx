...
Parallel coordinates is part of the **Relationships and hierarchy** chart family and is intended for `Vec<Vec<f32>> with one row per observation`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct ParallelCoordinatesChart;

impl From<ParallelCoordinatesChart> for Widget {
    fn from(_: ParallelCoordinatesChart) -> Widget {
        Chart::new()
            .title("Parallel coordinates")
            .width(720.0)
            .height(480.0)
