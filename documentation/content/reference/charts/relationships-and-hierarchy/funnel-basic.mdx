...
description: Fission Charts reference for Funnel.
---

# Funnel

![Funnel screenshot](/img/charts/funnel-basic.png)

Stage values drawn as narrowing bands. Use it for conversion stages and pipeline health. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Funnel is part of the **Relationships and hierarchy** chart family and is intended for `Ordered label/value pairs`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Ordered label/value pairs.` Keep the data close to the type that describes it. Fission Charts is typed Rust, so a line uses `LineSeries`, a calendar heatmap uses `CalendarHeatmapSeries`, and chart components such as zoom, marks, and graphics are explicit fields on `Chart` rather than hidden string configuration.
...
impl From<FunnelChart> for Widget {
    fn from(_: FunnelChart) -> Widget {
        Chart::new()
            .title("Funnel")
            .width(720.0)
            .height(480.0)
