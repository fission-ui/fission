...
sidebar_label: Equity session candlestick
description: Fission Charts reference for Equity session candlestick.
---

# Equity session candlestick

![Equity session candlestick screenshot](/img/charts/candlestick-equity-session.png)

Equity session candlestick uses Fission Charts typed Rust data to render a production-ready statistical view. Use it for financial price movement. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Equity session candlestick is part of the **Statistical** chart family and is intended for `Open, close, low, and high values by period`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Open, close, low, and high values by period.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct EquitySessionCandlestickChart;

impl From<EquitySessionCandlestickChart> for Widget {
    fn from(_: EquitySessionCandlestickChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
