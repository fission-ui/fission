...
sidebar_label: Lab sample scatter
description: Fission Charts reference for Lab sample scatter.
---

# Lab sample scatter

![Lab sample scatter screenshot](/img/charts/scatter-lab-samples.png)

Lab sample scatter uses Fission Charts typed Rust data to render a production-ready statistical view. Use it for correlation and clustering questions. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Lab sample scatter is part of the **Statistical** chart family and is intended for `Independent x/y samples`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Independent x/y samples.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct LabSampleScatterChart;

impl From<LabSampleScatterChart> for Widget {
    fn from(_: LabSampleScatterChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
