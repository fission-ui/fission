...
sidebar_label: Support resolution funnel
description: Fission Charts reference for Support resolution funnel.
---

# Support resolution funnel

![Support resolution funnel screenshot](/img/charts/funnel-support-resolution.png)

Support resolution funnel uses Fission Charts typed Rust data to render a production-ready statistical view. Use it for operational process attrition. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Support resolution funnel is part of the **Statistical** chart family and is intended for `Stage/value pairs through a process`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Stage/value pairs through a process.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct SupportResolutionFunnelChart;

impl From<SupportResolutionFunnelChart> for Widget {
    fn from(_: SupportResolutionFunnelChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
