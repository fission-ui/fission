...
sidebar_label: Activation funnel
description: Fission Charts reference for Activation funnel.
---
...
Activation funnel uses Fission Charts typed Rust data to render a production-ready statistical view. Use it when conversion loss matters more than a shared axis. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Activation funnel is part of the **Statistical** chart family and is intended for `Ordered conversion stage values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct ActivationFunnelChart;

impl From<ActivationFunnelChart> for Widget {
    fn from(_: ActivationFunnelChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
