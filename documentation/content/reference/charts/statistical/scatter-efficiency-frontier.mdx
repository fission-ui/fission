...
sidebar_label: Efficiency frontier
description: Fission Charts reference for Efficiency frontier.
---

# Efficiency frontier

![Efficiency frontier screenshot](/img/charts/scatter-efficiency-frontier.png)

Efficiency frontier uses Fission Charts typed Rust data to render a production-ready statistical view. Use it to identify leading and lagging points. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Efficiency frontier is part of the **Statistical** chart family and is intended for `Efficiency points over value axes`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Efficiency points over value axes.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct EfficiencyFrontierChart;

impl From<EfficiencyFrontierChart> for Widget {
    fn from(_: EfficiencyFrontierChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
