...
sidebar_label: Device quality parallel
description: Fission Charts reference for Device quality parallel.
---

# Device quality parallel

![Device quality parallel screenshot](/img/charts/parallel-device-quality.png)

Device quality parallel uses Fission Charts typed Rust data to render a production-ready statistical view. Use it when each item has several independent measures. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Device quality parallel is part of the **Statistical** chart family and is intended for `Rows of comparable numeric dimensions`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Rows of comparable numeric dimensions.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DeviceQualityParallelChart;

impl From<DeviceQualityParallelChart> for Widget {
    fn from(_: DeviceQualityParallelChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
