...
Quality outlier scatter is part of the **Statistical** chart family and is intended for `Highlighted x/y points over value axes`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct QualityOutlierScatterChart;

impl From<QualityOutlierScatterChart> for Widget {
    fn from(_: QualityOutlierScatterChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
