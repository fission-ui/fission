...
sidebar_label: Plan comparison parallel
description: Fission Charts reference for Plan comparison parallel.
---

# Plan comparison parallel

![Plan comparison parallel screenshot](/img/charts/parallel-plan-comparison.png)

Plan comparison parallel uses Fission Charts typed Rust data to render a production-ready statistical view. Use it when tradeoffs across measures matter. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Plan comparison parallel is part of the **Statistical** chart family and is intended for `Multi-dimensional rows on parallel axes`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Multi-dimensional rows on parallel axes.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct PlanComparisonParallelChart;

impl From<PlanComparisonParallelChart> for Widget {
    fn from(_: PlanComparisonParallelChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
