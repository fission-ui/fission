...
sidebar_label: Alert hotspot scatter
description: Fission Charts reference for Alert hotspot scatter.
---

# Alert hotspot scatter

![Alert hotspot scatter screenshot](/img/charts/scatter-alert-hotspots.png)

Alert hotspot scatter uses Fission Charts typed Rust data to render a production-ready statistical view. Use it when only important events should pulse visually. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Alert hotspot scatter is part of the **Statistical** chart family and is intended for `Selected alert points over numeric axes`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Selected alert points over numeric axes.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct AlertHotspotScatterChart;

impl From<AlertHotspotScatterChart> for Widget {
    fn from(_: AlertHotspotScatterChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
