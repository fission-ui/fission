...
sidebar_label: Risk bubble matrix
description: Fission Charts reference for Risk bubble matrix.
---

# Risk bubble matrix

![Risk bubble matrix screenshot](/img/charts/scatter-risk-bubbles.png)

Risk bubble matrix uses Fission Charts typed Rust data to render a production-ready statistical view. Use it when size carries a third measure. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Risk bubble matrix is part of the **Statistical** chart family and is intended for `X, y, and magnitude triples`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`X, y, and magnitude triples.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct RiskBubbleMatrixChart;

impl From<RiskBubbleMatrixChart> for Widget {
    fn from(_: RiskBubbleMatrixChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
