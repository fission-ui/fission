...
Portfolio risk return is part of the **Statistical** chart family and is intended for `Risk, return, and exposure values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct PortfolioRiskReturnChart;

impl From<PortfolioRiskReturnChart> for Widget {
    fn from(_: PortfolioRiskReturnChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
