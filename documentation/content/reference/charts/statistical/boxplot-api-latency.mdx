...
sidebar_label: API latency boxplot
description: Fission Charts reference for API latency boxplot.
---

# API latency boxplot

![API latency boxplot screenshot](/img/charts/boxplot-api-latency.png)

API latency boxplot uses Fission Charts typed Rust data to render a production-ready statistical view. Use it to compare distributions instead of averages. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

API latency boxplot is part of the **Statistical** chart family and is intended for `Five-number summaries across service groups`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Five-number summaries across service groups.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct APILatencyBoxplotChart;

impl From<APILatencyBoxplotChart> for Widget {
    fn from(_: APILatencyBoxplotChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
