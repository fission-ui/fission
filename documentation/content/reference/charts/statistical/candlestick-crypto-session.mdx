...
sidebar_label: Crypto session candlestick
description: Fission Charts reference for Crypto session candlestick.
---

# Crypto session candlestick

![Crypto session candlestick screenshot](/img/charts/candlestick-crypto-session.png)

Crypto session candlestick uses Fission Charts typed Rust data to render a production-ready statistical view. Use it when price direction and range must be visible together. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Crypto session candlestick is part of the **Statistical** chart family and is intended for `OHLC values over intraday buckets`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`OHLC values over intraday buckets.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct CryptoSessionCandlestickChart;

impl From<CryptoSessionCandlestickChart> for Widget {
    fn from(_: CryptoSessionCandlestickChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
