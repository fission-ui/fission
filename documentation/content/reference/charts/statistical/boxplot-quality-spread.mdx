...
sidebar_label: Quality spread boxplot
description: Fission Charts reference for Quality spread boxplot.
---

# Quality spread boxplot

![Quality spread boxplot screenshot](/img/charts/boxplot-quality-spread.png)

Quality spread boxplot uses Fission Charts typed Rust data to render a production-ready statistical view. Use it when spread and outliers matter. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Quality spread boxplot is part of the **Statistical** chart family and is intended for `Five-number summaries for quality scores`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Five-number summaries for quality scores.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct QualitySpreadBoxplotChart;

impl From<QualitySpreadBoxplotChart> for Widget {
    fn from(_: QualitySpreadBoxplotChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
