...
sidebar_label: Region spread boxplot
description: Fission Charts reference for Region spread boxplot.
---

# Region spread boxplot

![Region spread boxplot screenshot](/img/charts/boxplot-region-spread.png)

Region spread boxplot uses Fission Charts typed Rust data to render a production-ready statistical view. Use it for distribution comparison across regions. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Region spread boxplot is part of the **Statistical** chart family and is intended for `Five-number summaries by region`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Five-number summaries by region.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct RegionSpreadBoxplotChart;

impl From<RegionSpreadBoxplotChart> for Widget {
    fn from(_: RegionSpreadBoxplotChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
