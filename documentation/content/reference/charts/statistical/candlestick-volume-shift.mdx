...
sidebar_label: Volume shift candlestick
description: Fission Charts reference for Volume shift candlestick.
---

# Volume shift candlestick

![Volume shift candlestick screenshot](/img/charts/candlestick-volume-shift.png)

Volume shift candlestick uses Fission Charts typed Rust data to render a production-ready statistical view. Use it when directional change and range both matter. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Volume shift candlestick is part of the **Statistical** chart family and is intended for `OHLC-like movement values by period`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`OHLC-like movement values by period.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct VolumeShiftCandlestickChart;

impl From<VolumeShiftCandlestickChart> for Widget {
    fn from(_: VolumeShiftCandlestickChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
