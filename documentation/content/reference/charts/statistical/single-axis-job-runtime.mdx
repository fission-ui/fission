...
sidebar_label: Job runtime strip
description: Fission Charts reference for Job runtime strip.
---

# Job runtime strip

![Job runtime strip screenshot](/img/charts/single-axis-job-runtime.png)

Job runtime strip uses Fission Charts typed Rust data to render a production-ready statistical view. Use it for queue and schedule views. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Job runtime strip is part of the **Statistical** chart family and is intended for `Job positions with duration or intensity values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Job positions with duration or intensity values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct JobRuntimeStripChart;

impl From<JobRuntimeStripChart> for Widget {
    fn from(_: JobRuntimeStripChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
