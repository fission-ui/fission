...
sidebar_label: Customer value bubbles
description: Fission Charts reference for Customer value bubbles.
---

# Customer value bubbles

![Customer value bubbles screenshot](/img/charts/bubble-customer-value.png)

Customer value bubbles uses Fission Charts typed Rust data to render a production-ready statistical view. Use it when business value should influence point size. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Customer value bubbles is part of the **Statistical** chart family and is intended for `X, y, and value triples for customers or segments`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`X, y, and value triples for customers or segments.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct CustomerValueBubblesChart;

impl From<CustomerValueBubblesChart> for Widget {
    fn from(_: CustomerValueBubblesChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
