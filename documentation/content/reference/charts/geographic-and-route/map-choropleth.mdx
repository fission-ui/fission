...
description: Fission Charts reference for Choropleth map.
---
...
Regions colored by a numeric value. Use it for geography-first comparison. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Choropleth map is part of the **Geographic and route** chart family and is intended for `Region identifiers plus values and GeoJSON geometry`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct ChoroplethMapChart;

impl From<ChoroplethMapChart> for Widget {
    fn from(_: ChoroplethMapChart) -> Widget {
        Chart::new()
            .title("Choropleth map")
            .width(720.0)
            .height(480.0)
