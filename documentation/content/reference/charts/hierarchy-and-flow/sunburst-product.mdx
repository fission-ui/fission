...
sidebar_label: Product sunburst
description: Fission Charts reference for Product sunburst.
---

# Product sunburst

![Product sunburst screenshot](/img/charts/sunburst-product.png)

Hierarchical values in concentric rings. Use it for hierarchy depth and composition. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Product sunburst is part of the **Hierarchy and flow** chart family and is intended for `Nested value nodes`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Nested value nodes.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ProductSunburstChart;

impl From<ProductSunburstChart> for Widget {
    fn from(_: ProductSunburstChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
