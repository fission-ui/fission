...
sidebar_label: User-flow sankey
description: Fission Charts reference for User-flow sankey.
---

# User-flow sankey

![User-flow sankey screenshot](/img/charts/sankey-user-flow.png)

Sankey chart for user movement. Use it for conversion path analysis. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

User-flow sankey is part of the **Hierarchy and flow** chart family and is intended for `Vertex list plus edges`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Vertex list plus edges.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct UserFlowSankeyChart;

impl From<UserFlowSankeyChart> for Widget {
    fn from(_: UserFlowSankeyChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
