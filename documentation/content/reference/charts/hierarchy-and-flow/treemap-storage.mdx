...
sidebar_label: Storage treemap
description: Fission Charts reference for Storage treemap.
---

# Storage treemap

![Storage treemap screenshot](/img/charts/treemap-storage.png)

Treemap for storage allocation. Use it for disk and memory allocation. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Storage treemap is part of the **Hierarchy and flow** chart family and is intended for `Nested value nodes`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct StorageTreemapChart;

impl From<StorageTreemapChart> for Widget {
    fn from(_: StorageTreemapChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
