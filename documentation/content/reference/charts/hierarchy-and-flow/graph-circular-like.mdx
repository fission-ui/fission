...
sidebar_label: Circular-like graph
description: Fission Charts reference for Circular-like graph.
---

# Circular-like graph

![Circular-like graph screenshot](/img/charts/graph-circular-like.png)

Network layout with larger central nodes. Use it for relationship overview surfaces. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Circular-like graph is part of the **Hierarchy and flow** chart family and is intended for `Graph nodes plus edges`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Graph nodes plus edges.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct CircularLikeGraphChart;

impl From<CircularLikeGraphChart> for Widget {
    fn from(_: CircularLikeGraphChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
