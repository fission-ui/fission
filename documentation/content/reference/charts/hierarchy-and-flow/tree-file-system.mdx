...
sidebar_label: File-system tree
description: Fission Charts reference for File-system tree.
---

# File-system tree

![File-system tree screenshot](/img/charts/tree-file-system.png)

Nested file-style hierarchy. Use it for product and resource hierarchies. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

File-system tree is part of the **Hierarchy and flow** chart family and is intended for `Nested nodes with values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Nested nodes with values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct FileSystemTreeChart;

impl From<FileSystemTreeChart> for Widget {
    fn from(_: FileSystemTreeChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
