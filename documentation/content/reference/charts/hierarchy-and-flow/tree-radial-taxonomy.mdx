...
sidebar_label: Radial taxonomy
description: Fission Charts reference for Radial taxonomy.
---

# Radial taxonomy

![Radial taxonomy screenshot](/img/charts/tree-radial-taxonomy.png)

Radial layout for a shallow taxonomy. Use it when hierarchy symmetry matters. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Radial taxonomy is part of the **Hierarchy and flow** chart family and is intended for `Nested nodes`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Nested nodes.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct RadialTaxonomyChart;

impl From<RadialTaxonomyChart> for Widget {
    fn from(_: RadialTaxonomyChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
