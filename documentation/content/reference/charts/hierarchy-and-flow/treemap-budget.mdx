...
sidebar_label: Budget treemap
description: Fission Charts reference for Budget treemap.
---
...
Hierarchical values packed into rectangles. Use it for budget or storage breakdowns. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Budget treemap is part of the **Hierarchy and flow** chart family and is intended for `Nested value nodes`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct BudgetTreemapChart;

impl From<BudgetTreemapChart> for Widget {
    fn from(_: BudgetTreemapChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
