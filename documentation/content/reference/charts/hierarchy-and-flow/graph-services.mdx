...
description: Fission Charts reference for Service graph.
---

# Service graph

![Service graph screenshot](/img/charts/graph-services.png)

Service topology network. Use it for systems maps and service graphs. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Service graph is part of the **Hierarchy and flow** chart family and is intended for `Graph nodes plus edges`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct ServiceGraphChart;

impl From<ServiceGraphChart> for Widget {
    fn from(_: ServiceGraphChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
