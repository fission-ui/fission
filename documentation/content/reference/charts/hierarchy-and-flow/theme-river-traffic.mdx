...
Traffic theme river is part of the **Hierarchy and flow** chart family and is intended for `Time/value/category tuples`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct TrafficThemeRiverChart;

impl From<TrafficThemeRiverChart> for Widget {
    fn from(_: TrafficThemeRiverChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
