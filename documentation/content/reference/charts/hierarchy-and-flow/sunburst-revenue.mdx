...
sidebar_label: Revenue sunburst
description: Fission Charts reference for Revenue sunburst.
---

# Revenue sunburst

![Revenue sunburst screenshot](/img/charts/sunburst-revenue.png)

Revenue hierarchy as radial rings. Use it for nested part-to-whole data. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Revenue sunburst is part of the **Hierarchy and flow** chart family and is intended for `Nested value nodes`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct RevenueSunburstChart;

impl From<RevenueSunburstChart> for Widget {
    fn from(_: RevenueSunburstChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
