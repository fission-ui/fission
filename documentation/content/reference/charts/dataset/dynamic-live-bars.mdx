...
sidebar_label: Live bar update
description: Fission Charts reference for Live bar update.
---

# Live bar update

![Live bar update screenshot](/img/charts/dynamic-live-bars.png)

Live bar update uses Fission Charts typed Rust data to render a production-ready dataset view. Use it for updating status summaries. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Live bar update is part of the **Dataset** chart family and is intended for `Changing category values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Changing category values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct LiveBarUpdateChart;

impl From<LiveBarUpdateChart> for Widget {
    fn from(_: LiveBarUpdateChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
