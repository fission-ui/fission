...
sidebar_label: Brush telemetry
description: Fission Charts reference for Brush telemetry.
---
...
Brush telemetry uses Fission Charts typed Rust data to render a production-ready dataset view. Use it for interactive telemetry exploration. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Brush telemetry is part of the **Dataset** chart family and is intended for `Point samples with brush selection`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Point samples with brush selection.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct BrushTelemetryChart;

impl From<BrushTelemetryChart> for Widget {
    fn from(_: BrushTelemetryChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
