...
sidebar_label: Coverage dataset map
description: Fission Charts reference for Coverage dataset map.
---

# Coverage dataset map

![Coverage dataset map screenshot](/img/charts/dataset-map-coverage.png)

Coverage dataset map uses Fission Charts typed Rust data to render a production-ready dataset view. Use it when geographic views come from named records. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Coverage dataset map is part of the **Dataset** chart family and is intended for `Region/value rows`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Region/value rows.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct CoverageDatasetMapChart;

impl From<CoverageDatasetMapChart> for Widget {
    fn from(_: CoverageDatasetMapChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
