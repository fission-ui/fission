...
sidebar_label: Product trend dataset
description: Fission Charts reference for Product trend dataset.
---

# Product trend dataset

![Product trend dataset screenshot](/img/charts/dataset-product-trends.png)

Product trend dataset uses Fission Charts typed Rust data to render a production-ready dataset view. Use it when named fields should drive series values. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Product trend dataset is part of the **Dataset** chart family and is intended for `Tabular data encoded into line series`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Tabular data encoded into line series.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ProductTrendDatasetChart;

impl From<ProductTrendDatasetChart> for Widget {
    fn from(_: ProductTrendDatasetChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
