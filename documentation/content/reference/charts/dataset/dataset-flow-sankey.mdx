...
sidebar_label: Flow dataset sankey
description: Fission Charts reference for Flow dataset sankey.
---

# Flow dataset sankey

![Flow dataset sankey screenshot](/img/charts/dataset-flow-sankey.png)

Flow dataset sankey uses Fission Charts typed Rust data to render a production-ready dataset view. Use it when flow data is modeled explicitly. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Flow dataset sankey is part of the **Dataset** chart family and is intended for `Vertex and edge records`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Vertex and edge records.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct FlowDatasetSankeyChart;

impl From<FlowDatasetSankeyChart> for Widget {
    fn from(_: FlowDatasetSankeyChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
