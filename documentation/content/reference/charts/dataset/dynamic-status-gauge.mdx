...
sidebar_label: Live status gauge
description: Fission Charts reference for Live status gauge.
---

# Live status gauge

![Live status gauge screenshot](/img/charts/dynamic-status-gauge.png)

Live status gauge uses Fission Charts typed Rust data to render a production-ready dataset view. Use it when a live number needs a strong visual shape. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Live status gauge is part of the **Dataset** chart family and is intended for `Single changing status value`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Single changing status value.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct LiveStatusGaugeChart;

impl From<LiveStatusGaugeChart> for Widget {
    fn from(_: LiveStatusGaugeChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
