...
sidebar_label: Live line update
description: Fission Charts reference for Live line update.
---

# Live line update

![Live line update screenshot](/img/charts/dynamic-live-line.png)

Live line update uses Fission Charts typed Rust data to render a production-ready dataset view. Use it for live monitoring views. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Live line update is part of the **Dataset** chart family and is intended for `Append-like ordered telemetry samples`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Append-like ordered telemetry samples.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct LiveLineUpdateChart;

impl From<LiveLineUpdateChart> for Widget {
    fn from(_: LiveLineUpdateChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
