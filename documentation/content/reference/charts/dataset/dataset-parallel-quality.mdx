...
sidebar_label: Quality dataset parallel
description: Fission Charts reference for Quality dataset parallel.
---

# Quality dataset parallel

![Quality dataset parallel screenshot](/img/charts/dataset-parallel-quality.png)

Quality dataset parallel uses Fission Charts typed Rust data to render a production-ready dataset view. Use it when the same table powers multidimensional analysis. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Quality dataset parallel is part of the **Dataset** chart family and is intended for `Rows with several numeric dimensions`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Rows with several numeric dimensions.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct QualityDatasetParallelChart;

impl From<QualityDatasetParallelChart> for Widget {
    fn from(_: QualityDatasetParallelChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
...
Tags: `dataset`, `parallel`.

[Dataset family overview](./overview)
