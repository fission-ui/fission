...
sidebar_label: Live radar score
description: Fission Charts reference for Live radar score.
---

# Live radar score

![Live radar score screenshot](/img/charts/dynamic-radar-score.png)

Live radar score uses Fission Charts typed Rust data to render a production-ready dataset view. Use it for live multidimensional scorecards. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Live radar score is part of the **Dataset** chart family and is intended for `Changing dimension vectors`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Changing dimension vectors.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct LiveRadarScoreChart;

impl From<LiveRadarScoreChart> for Widget {
    fn from(_: LiveRadarScoreChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
