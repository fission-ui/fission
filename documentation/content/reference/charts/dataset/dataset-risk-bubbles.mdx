...
sidebar_label: Risk bubbles dataset
description: Fission Charts reference for Risk bubbles dataset.
---

# Risk bubbles dataset

![Risk bubbles dataset screenshot](/img/charts/dataset-risk-bubbles.png)

Risk bubbles dataset uses Fission Charts typed Rust data to render a production-ready dataset view. Use it when one table feeds bubble visualization. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Risk bubbles dataset is part of the **Dataset** chart family and is intended for `Rows encoded into x, y, and size`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Rows encoded into x, y, and size.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct RiskBubblesDatasetChart;

impl From<RiskBubblesDatasetChart> for Widget {
    fn from(_: RiskBubblesDatasetChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
