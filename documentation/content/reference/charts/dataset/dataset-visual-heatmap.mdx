...
sidebar_label: Dataset visual heatmap
description: Fission Charts reference for Dataset visual heatmap.
---

# Dataset visual heatmap

![Dataset visual heatmap screenshot](/img/charts/dataset-visual-heatmap.png)

Dataset visual heatmap uses Fission Charts typed Rust data to render a production-ready dataset view. Use it when heatmaps come from generic records. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Dataset visual heatmap is part of the **Dataset** chart family and is intended for `Encoded grid cells with values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Encoded grid cells with values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DatasetVisualHeatmapChart;

impl From<DatasetVisualHeatmapChart> for Widget {
    fn from(_: DatasetVisualHeatmapChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
