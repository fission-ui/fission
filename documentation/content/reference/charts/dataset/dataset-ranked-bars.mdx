...
sidebar_label: Ranked dataset bars
description: Fission Charts reference for Ranked dataset bars.
---

# Ranked dataset bars

![Ranked dataset bars screenshot](/img/charts/dataset-ranked-bars.png)

Ranked dataset bars uses Fission Charts typed Rust data to render a production-ready dataset view. Use it when rank comes from data transformation. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Ranked dataset bars is part of the **Dataset** chart family and is intended for `Rows sorted into category/value pairs`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Rows sorted into category/value pairs.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct RankedDatasetBarsChart;

impl From<RankedDatasetBarsChart> for Widget {
    fn from(_: RankedDatasetBarsChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
