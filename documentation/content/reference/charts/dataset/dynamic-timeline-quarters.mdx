...
sidebar_label: Quarter timeline
description: Fission Charts reference for Quarter timeline.
---

# Quarter timeline

![Quarter timeline screenshot](/img/charts/dynamic-timeline-quarters.png)

Quarter timeline uses Fission Charts typed Rust data to render a production-ready dataset view. Use it for period-over-period analysis. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Quarter timeline is part of the **Dataset** chart family and is intended for `Period labels switching chart state`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Period labels switching chart state.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct QuarterTimelineChart;

impl From<QuarterTimelineChart> for Widget {
    fn from(_: QuarterTimelineChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
