...
sidebar_label: Telemetry toolbox
description: Fission Charts reference for Telemetry toolbox.
---

# Telemetry toolbox

![Telemetry toolbox screenshot](/img/charts/dynamic-toolbox-telemetry.png)

Telemetry toolbox uses Fission Charts typed Rust data to render a production-ready dataset view. Use it when live charts need controlled exploration. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Telemetry toolbox is part of the **Dataset** chart family and is intended for `Telemetry chart with built-in actions`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Telemetry chart with built-in actions.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct TelemetryToolboxChart;

impl From<TelemetryToolboxChart> for Widget {
    fn from(_: TelemetryToolboxChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
