...
sidebar_label: Live activation funnel
description: Fission Charts reference for Live activation funnel.
---

# Live activation funnel

![Live activation funnel screenshot](/img/charts/dynamic-funnel-activation.png)

Live activation funnel uses Fission Charts typed Rust data to render a production-ready dataset view. Use it when pipeline stages update during the session. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Live activation funnel is part of the **Dataset** chart family and is intended for `Changing stage values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Changing stage values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct LiveActivationFunnelChart;

impl From<LiveActivationFunnelChart> for Widget {
    fn from(_: LiveActivationFunnelChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
