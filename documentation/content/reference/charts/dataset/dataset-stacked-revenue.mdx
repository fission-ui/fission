...
sidebar_label: Stacked revenue dataset
description: Fission Charts reference for Stacked revenue dataset.
---

# Stacked revenue dataset

![Stacked revenue dataset screenshot](/img/charts/dataset-stacked-revenue.png)

Stacked revenue dataset uses Fission Charts typed Rust data to render a production-ready dataset view. Use it when stack series share the same source table. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Stacked revenue dataset is part of the **Dataset** chart family and is intended for `Multiple encoded measures over one category axis`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Multiple encoded measures over one category axis.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct StackedRevenueDatasetChart;

impl From<StackedRevenueDatasetChart> for Widget {
    fn from(_: StackedRevenueDatasetChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
