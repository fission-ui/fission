...
sidebar_label: Live alert scatter
description: Fission Charts reference for Live alert scatter.
---

# Live alert scatter

![Live alert scatter screenshot](/img/charts/dynamic-alert-scatter.png)

Live alert scatter uses Fission Charts typed Rust data to render a production-ready dataset view. Use it for active events and outliers. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Live alert scatter is part of the **Dataset** chart family and is intended for `Highlighted alert points`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Highlighted alert points.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct LiveAlertScatterChart;

impl From<LiveAlertScatterChart> for Widget {
    fn from(_: LiveAlertScatterChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
