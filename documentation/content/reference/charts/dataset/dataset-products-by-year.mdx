...
sidebar_label: Products by year dataset
description: Fission Charts reference for Products by year dataset.
---

# Products by year dataset

![Products by year dataset screenshot](/img/charts/dataset-products-by-year.png)

Products by year dataset uses Fission Charts typed Rust data to render a production-ready dataset view. Use it when several chart views should share one dataset. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Products by year dataset is part of the **Dataset** chart family and is intended for `Tabular product/year/value records`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Tabular product/year/value records.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ProductsByYearDatasetChart;

impl From<ProductsByYearDatasetChart> for Widget {
    fn from(_: ProductsByYearDatasetChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
