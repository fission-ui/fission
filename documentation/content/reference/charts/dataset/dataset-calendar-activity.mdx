...
sidebar_label: Calendar dataset activity
description: Fission Charts reference for Calendar dataset activity.
---

# Calendar dataset activity

![Calendar dataset activity screenshot](/img/charts/dataset-calendar-activity.png)

Calendar dataset activity uses Fission Charts typed Rust data to render a production-ready dataset view. Use it when dates are a first-class data field. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Calendar dataset activity is part of the **Dataset** chart family and is intended for `Date/value rows mapped to a calendar`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Date/value rows mapped to a calendar.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct CalendarDatasetActivityChart;

impl From<CalendarDatasetActivityChart> for Widget {
    fn from(_: CalendarDatasetActivityChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
