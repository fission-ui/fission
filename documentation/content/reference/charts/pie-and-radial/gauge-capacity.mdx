...
![Capacity gauge screenshot](/img/charts/gauge-capacity.png)

Gauge showing capacity consumed. Use it for operational capacity panels. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Capacity gauge is part of the **Pie and radial** chart family and is intended for `One bounded value`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`One bounded value.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
