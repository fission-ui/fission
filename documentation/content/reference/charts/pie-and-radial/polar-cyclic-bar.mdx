...
sidebar_label: Cyclic polar bars
description: Fission Charts reference for Cyclic polar bars.
---

# Cyclic polar bars

![Cyclic polar bars screenshot](/img/charts/polar-cyclic-bar.png)

Radial bars around a cycle. Use it for cyclical categories. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Cyclic polar bars is part of the **Pie and radial** chart family and is intended for `Label/value pairs in polar coordinates`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Label/value pairs in polar coordinates.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct CyclicPolarBarsChart;

impl From<CyclicPolarBarsChart> for Widget {
    fn from(_: CyclicPolarBarsChart) -> Widget {
        Chart::new()
            .title("Share")
            .legend(Legend::top_right())
            .series(vec![PieSeries::new("Share")
