...
sidebar_label: Nested-style donut
description: Fission Charts reference for Nested-style donut.
---

# Nested-style donut

![Nested-style donut screenshot](/img/charts/pie-nested-like.png)

Donut-style chart with more categories and central whitespace. Use it when a central status label will be added by surrounding UI. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Nested-style donut is part of the **Pie and radial** chart family and is intended for `Label/value pairs plus inner radius`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Label/value pairs plus inner radius.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct NestedStyleDonutChart;

impl From<NestedStyleDonutChart> for Widget {
    fn from(_: NestedStyleDonutChart) -> Widget {
        Chart::new()
            .title("Share")
            .legend(Legend::top_right())
            .series(vec![PieSeries::new("Share")
