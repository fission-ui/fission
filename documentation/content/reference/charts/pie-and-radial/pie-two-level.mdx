...
sidebar_label: Two-level donut
description: Fission Charts reference for Two-level donut.
---

# Two-level donut

![Two-level donut screenshot](/img/charts/pie-two-level.png)

Donut chart for a small part-to-whole breakdown. Use it for compact composition summaries. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Two-level donut is part of the **Pie and radial** chart family and is intended for `Label/value pairs plus inner radius`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct TwoLevelDonutChart;

impl From<TwoLevelDonutChart> for Widget {
    fn from(_: TwoLevelDonutChart) -> Widget {
        Chart::new()
            .title("Share")
            .legend(Legend::top_right())
            .series(vec![PieSeries::new("Share")
