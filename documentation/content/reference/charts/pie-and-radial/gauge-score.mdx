---
title: Score gauge
sidebar_label: Score gauge
description: Fission Charts reference for Score gauge.
---

# Score gauge

![Score gauge screenshot](/img/charts/gauge-score.png)

Bounded score shown as an instrument state. Use it for health or readiness scores. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Score gauge is part of the **Pie and radial** chart family and is intended for `One label/value pair`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`One label/value pair.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ScoreGaugeChart;

impl From<ScoreGaugeChart> for Widget {
    fn from(_: ScoreGaugeChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
