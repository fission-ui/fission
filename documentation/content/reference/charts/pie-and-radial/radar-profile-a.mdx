...
sidebar_label: Radar profile A
description: Fission Charts reference for Radar profile A.
---

# Radar profile A

![Radar profile A screenshot](/img/charts/radar-profile-a.png)

Profile comparison across fixed dimensions. Use it for capability or budget profiles. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Radar profile A is part of the **Pie and radial** chart family and is intended for `Vec<Vec<f32>> metric profiles`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct RadarProfileAChart;

impl From<RadarProfileAChart> for Widget {
    fn from(_: RadarProfileAChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
