...
sidebar_label: Presentation rose
description: Fission Charts reference for Presentation rose.
---

# Presentation rose

![Presentation rose screenshot](/img/charts/pie-rose-presentation.png)

Rose chart emphasizing category differences with radius. Use it for presentation views with few categories. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Presentation rose is part of the **Pie and radial** chart family and is intended for `Label/value pairs with rose radius mode`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Label/value pairs with rose radius mode.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct PresentationRoseChart;

impl From<PresentationRoseChart> for Widget {
    fn from(_: PresentationRoseChart) -> Widget {
        Chart::new()
            .title("Share")
            .legend(Legend::top_right())
            .series(vec![PieSeries::new("Share")
