---
title: Mesh surface
sidebar_label: Mesh surface
description: Fission Charts reference for Mesh surface.
---

# Mesh surface

![Mesh surface screenshot](/img/charts/mesh-surface.png)

Generic mesh-rendered surface. Use it for custom mesh data. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Mesh surface is part of the **3D and GL** chart family and is intended for `Scene3D mesh primitive`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Scene3D mesh primitive.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct MeshSurfaceChart;

impl From<MeshSurfaceChart> for Widget {
    fn from(_: MeshSurfaceChart) -> Widget {
        Chart::new()
            .title("Mesh surface")
