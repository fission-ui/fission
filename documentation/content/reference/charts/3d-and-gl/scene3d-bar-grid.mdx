---
title: 3D grid bars
sidebar_label: 3D grid bars
description: Fission Charts reference for 3D grid bars.
---

# 3D grid bars

![3D grid bars screenshot](/img/charts/scene3d-bar-grid.png)

3D grid bars uses Fission Charts typed Rust data to render a production-ready 3d and gl view. Use it for spatial bar comparisons. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

3D grid bars is part of the **3D and GL** chart family and is intended for `Grid of cuboids in a native 3D scene`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Grid of cuboids in a native 3D scene.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct Fission3DGridBarsChart;

impl From<Fission3DGridBarsChart> for Widget {
    fn from(_: Fission3DGridBarsChart) -> Widget {
        Chart::new()
            .title("3D grid bars")
