...
sidebar_label: 3D outlier scatter
description: Fission Charts reference for 3D outlier scatter.
---

# 3D outlier scatter

![3D outlier scatter screenshot](/img/charts/scene3d-scatter-outliers.png)

3D outlier scatter uses Fission Charts typed Rust data to render a production-ready 3d and gl view. Use it to show outliers in a point cloud. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

3D outlier scatter is part of the **3D and GL** chart family and is intended for `3D points with varied position and radius`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`3D points with varied position and radius.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct Fission3DOutlierScatterChart;

impl From<Fission3DOutlierScatterChart> for Widget {
    fn from(_: Fission3DOutlierScatterChart) -> Widget {
        Chart::new()
            .title("3D outlier scatter")
