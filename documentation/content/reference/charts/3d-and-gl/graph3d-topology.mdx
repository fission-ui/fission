---
title: 3D topology
sidebar_label: 3D topology
description: Fission Charts reference for 3D topology.
---

# 3D topology

![3D topology screenshot](/img/charts/graph3d-topology.png)

3D topology graph variant. Use it for spatial relationship maps. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

3D topology is part of the **3D and GL** chart family and is intended for `Scene3D nodes and segment meshes`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Scene3D nodes and segment meshes.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct Fission3DTopologyChart;

impl From<Fission3DTopologyChart> for Widget {
    fn from(_: Fission3DTopologyChart) -> Widget {
        Chart::new()
            .title("3D topology")
