...
sidebar_label: 3D response surface
description: Fission Charts reference for 3D response surface.
---

# 3D response surface

![3D response surface screenshot](/img/charts/scene3d-surface-response.png)

3D response surface uses Fission Charts typed Rust data to render a production-ready 3d and gl view. Use it for continuous spatial fields. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

3D response surface is part of the **3D and GL** chart family and is intended for `Mesh vertices and indices forming a surface`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Mesh vertices and indices forming a surface.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct Fission3DResponseSurfaceChart;

impl From<Fission3DResponseSurfaceChart> for Widget {
    fn from(_: Fission3DResponseSurfaceChart) -> Widget {
        Chart::new()
            .title("3D response surface")
