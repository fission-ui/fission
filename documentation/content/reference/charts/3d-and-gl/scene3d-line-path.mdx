---
title: 3D line path
sidebar_label: 3D line path
description: Fission Charts reference for 3D line path.
---

# 3D line path

![3D line path screenshot](/img/charts/scene3d-line-path.png)

3D line path uses Fission Charts typed Rust data to render a production-ready 3d and gl view. Use it for trajectories and movement. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

3D line path is part of the **3D and GL** chart family and is intended for `Spheres and segment meshes forming a path`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Spheres and segment meshes forming a path.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct Fission3DLinePathChart;

impl From<Fission3DLinePathChart> for Widget {
    fn from(_: Fission3DLinePathChart) -> Widget {
        Chart::new()
            .title("3D line path")
