...
sidebar_label: 3D spiral path
description: Fission Charts reference for 3D spiral path.
---

# 3D spiral path

![3D spiral path screenshot](/img/charts/scene3d-line-spiral.png)

3D spiral path uses Fission Charts typed Rust data to render a production-ready 3d and gl view. Use it for trajectory demos and spatial flows. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

3D spiral path is part of the **3D and GL** chart family and is intended for `Line path through depth`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Line path through depth.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct Fission3DSpiralPathChart;

impl From<Fission3DSpiralPathChart> for Widget {
    fn from(_: Fission3DSpiralPathChart) -> Widget {
        Chart::new()
            .title("3D spiral path")
