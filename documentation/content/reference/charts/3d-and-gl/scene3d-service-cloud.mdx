...
sidebar_label: 3D service cloud
description: Fission Charts reference for 3D service cloud.
---

# 3D service cloud

![3D service cloud screenshot](/img/charts/scene3d-service-cloud.png)

3D service cloud uses Fission Charts typed Rust data to render a production-ready 3d and gl view. Use it to visualize clusters of service observations. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

3D service cloud is part of the **3D and GL** chart family and is intended for `Service points in 3D space`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Service points in 3D space.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct Fission3DServiceCloudChart;

impl From<Fission3DServiceCloudChart> for Widget {
    fn from(_: Fission3DServiceCloudChart) -> Widget {
        Chart::new()
            .title("3D service cloud")
