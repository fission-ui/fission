...
sidebar_label: 3D topology scene
description: Fission Charts reference for 3D topology scene.
---

# 3D topology scene

![3D topology scene screenshot](/img/charts/scene3d-topology.png)

3D topology scene uses Fission Charts typed Rust data to render a production-ready 3d and gl view. Use it when topology benefits from depth. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

3D topology scene is part of the **3D and GL** chart family and is intended for `3D graph nodes and links`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`3D graph nodes and links.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct Fission3DTopologySceneChart;

impl From<Fission3DTopologySceneChart> for Widget {
    fn from(_: Fission3DTopologySceneChart) -> Widget {
        Chart::new()
            .title("3D topology scene")
