---
title: 3D network
sidebar_label: 3D network
description: Fission Charts reference for 3D network.
---

# 3D network

![3D network screenshot](/img/charts/graph3d-network.png)

3D graph nodes and links. Use it for topology in depth. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

3D network is part of the **3D and GL** chart family and is intended for `Scene3D nodes and segment meshes`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct Fission3DNetworkChart;

impl From<Fission3DNetworkChart> for Widget {
    fn from(_: Fission3DNetworkChart) -> Widget {
        Chart::new()
            .title("3D network")
