...
sidebar_label: 3D globe status
description: Fission Charts reference for 3D globe status.
---

# 3D globe status

![3D globe status screenshot](/img/charts/scene3d-globe-status.png)

3D globe status uses Fission Charts typed Rust data to render a production-ready 3d and gl view. Use it when global context matters. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

3D globe status is part of the **3D and GL** chart family and is intended for `Globe sphere with visible markers`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Globe sphere with visible markers.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct Fission3DGlobeStatusChart;

impl From<Fission3DGlobeStatusChart> for Widget {
    fn from(_: Fission3DGlobeStatusChart) -> Widget {
        Chart::new()
            .title("3D globe status")
