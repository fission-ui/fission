...
3D scatter is part of the **3D and GL** chart family and is intended for `Vec<(x, y, z, radius)> lowered into Scene3D spheres`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct Fission3DScatterChart;

impl From<Fission3DScatterChart> for Widget {
    fn from(_: Fission3DScatterChart) -> Widget {
        Chart::new()
            .title("3D scatter")
