...
sidebar_label: 3D scatter outliers
description: Fission Charts reference for 3D scatter outliers.
---

# 3D scatter outliers

![3D scatter outliers screenshot](/img/charts/scatter3d-outliers.png)

3D scatter variant with separated points. Use it for spatial outlier inspection. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

3D scatter outliers is part of the **3D and GL** chart family and is intended for `Scene3D spheres`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct Fission3DScatterOutliersChart;

impl From<Fission3DScatterOutliersChart> for Widget {
    fn from(_: Fission3DScatterOutliersChart) -> Widget {
        Chart::new()
            .title("3D scatter outliers")
