...
sidebar_label: 3D scatter cluster
description: Fission Charts reference for 3D scatter cluster.
---

# 3D scatter cluster

![3D scatter cluster screenshot](/img/charts/scatter3d-cluster.png)

3D scatter points in a native scene. Use it for spatial sample clusters. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

3D scatter cluster is part of the **3D and GL** chart family and is intended for `Scene3D spheres`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct Fission3DScatterClusterChart;

impl From<Fission3DScatterClusterChart> for Widget {
    fn from(_: Fission3DScatterClusterChart) -> Widget {
        Chart::new()
            .title("3D scatter cluster")
