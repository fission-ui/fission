...
sidebar_label: 3D capacity bars
description: Fission Charts reference for 3D capacity bars.
---

# 3D capacity bars

![3D capacity bars screenshot](/img/charts/scene3d-bar-capacity.png)

3D capacity bars uses Fission Charts typed Rust data to render a production-ready 3d and gl view. Use it when depth is part of the visual story. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

3D capacity bars is part of the **3D and GL** chart family and is intended for `Scene3D cuboids representing numeric height`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Scene3D cuboids representing numeric height.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct Fission3DCapacityBarsChart;

impl From<Fission3DCapacityBarsChart> for Widget {
    fn from(_: Fission3DCapacityBarsChart) -> Widget {
        Chart::new()
            .title("3D capacity bars")
