...
sidebar_label: 3D risk terrain
description: Fission Charts reference for 3D risk terrain.
---

# 3D risk terrain

![3D risk terrain screenshot](/img/charts/scene3d-terrain-risk.png)

3D risk terrain uses Fission Charts typed Rust data to render a production-ready 3d and gl view. Use it for risk and elevation-like surfaces. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

3D risk terrain is part of the **3D and GL** chart family and is intended for `Terrain surface using native mesh rendering`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Terrain surface using native mesh rendering.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct Fission3DRiskTerrainChart;

impl From<Fission3DRiskTerrainChart> for Widget {
    fn from(_: Fission3DRiskTerrainChart) -> Widget {
        Chart::new()
            .title("3D risk terrain")
