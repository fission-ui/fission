...
sidebar_label: 3D terrain response
description: Fission Charts reference for 3D terrain response.
---

# 3D terrain response

![3D terrain response screenshot](/img/charts/scene3d-surface-terrain.png)

3D terrain response uses Fission Charts typed Rust data to render a production-ready 3d and gl view. Use it for elevation and field surfaces. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

3D terrain response is part of the **3D and GL** chart family and is intended for `Raised terrain-like mesh`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Raised terrain-like mesh.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct Fission3DTerrainResponseChart;

impl From<Fission3DTerrainResponseChart> for Widget {
    fn from(_: Fission3DTerrainResponseChart) -> Widget {
        Chart::new()
            .title("3D terrain response")
