...
sidebar_label: 3D spiral line
description: Fission Charts reference for 3D spiral line.
---

# 3D spiral line

![3D spiral line screenshot](/img/charts/line3d-spiral.png)

Spiral-like 3D line path. Use it for ordered spatial signals. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

3D spiral line is part of the **3D and GL** chart family and is intended for `Scene3D mesh segments plus points`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Scene3D mesh segments plus points.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct Fission3DSpiralLineChart;

impl From<Fission3DSpiralLineChart> for Widget {
    fn from(_: Fission3DSpiralLineChart) -> Widget {
        Chart::new()
            .title("3D spiral line")
