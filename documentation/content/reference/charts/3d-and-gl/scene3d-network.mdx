...
3D network scene is part of the **3D and GL** chart family and is intended for `Vertex spheres connected by segment meshes`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Vertex spheres connected by segment meshes.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct Fission3DNetworkSceneChart;

impl From<Fission3DNetworkSceneChart> for Widget {
    fn from(_: Fission3DNetworkSceneChart) -> Widget {
        Chart::new()
            .title("3D network scene")
