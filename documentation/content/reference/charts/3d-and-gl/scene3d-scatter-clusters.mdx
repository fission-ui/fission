...
sidebar_label: 3D cluster scatter
description: Fission Charts reference for 3D cluster scatter.
---

# 3D cluster scatter

![3D cluster scatter screenshot](/img/charts/scene3d-scatter-clusters.png)

3D cluster scatter uses Fission Charts typed Rust data to render a production-ready 3d and gl view. Use it for spatial point clusters. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

3D cluster scatter is part of the **3D and GL** chart family and is intended for `Spheres positioned in 3D space`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Spheres positioned in 3D space.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct Fission3DClusterScatterChart;

impl From<Fission3DClusterScatterChart> for Widget {
    fn from(_: Fission3DClusterScatterChart) -> Widget {
        Chart::new()
            .title("3D cluster scatter")
