...
sidebar_label: 3D operations bars
description: Fission Charts reference for 3D operations bars.
---

# 3D operations bars

![3D operations bars screenshot](/img/charts/scene3d-operations-bars.png)

3D operations bars uses Fission Charts typed Rust data to render a production-ready 3d and gl view. Use it when spatial placement helps compare operational metrics. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

3D operations bars is part of the **3D and GL** chart family and is intended for `3D bars over an operational grid`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`3D bars over an operational grid.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct Fission3DOperationsBarsChart;

impl From<Fission3DOperationsBarsChart> for Widget {
    fn from(_: Fission3DOperationsBarsChart) -> Widget {
        Chart::new()
            .title("3D operations bars")
