...
![3D capacity bars screenshot](/img/charts/bar3d-capacity.png)

3D bar scene for capacity values. Use it for spatial capacity displays. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

3D capacity bars is part of the **3D and GL** chart family and is intended for `Scene3D mesh cuboids`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Scene3D mesh cuboids.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
