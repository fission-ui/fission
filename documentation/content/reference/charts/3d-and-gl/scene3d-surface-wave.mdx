...
sidebar_label: 3D wave surface
description: Fission Charts reference for 3D wave surface.
---

# 3D wave surface

![3D wave surface screenshot](/img/charts/scene3d-surface-wave.png)

3D wave surface uses Fission Charts typed Rust data to render a production-ready 3d and gl view. Use it for continuous response fields. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

3D wave surface is part of the **3D and GL** chart family and is intended for `Surface mesh with wave-like height variation`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Surface mesh with wave-like height variation.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct Fission3DWaveSurfaceChart;

impl From<Fission3DWaveSurfaceChart> for Widget {
    fn from(_: Fission3DWaveSurfaceChart) -> Widget {
        Chart::new()
            .title("3D wave surface")
