...
sidebar_label: 3D volume points
description: Fission Charts reference for 3D volume points.
---

# 3D volume points

![3D volume points screenshot](/img/charts/scene3d-volume-points.png)

3D volume points uses Fission Charts typed Rust data to render a production-ready 3d and gl view. Use it as the native point-based volume path. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

3D volume points is part of the **3D and GL** chart family and is intended for `Point field suggesting volume data`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Point field suggesting volume data.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct Fission3DVolumePointsChart;

impl From<Fission3DVolumePointsChart> for Widget {
    fn from(_: Fission3DVolumePointsChart) -> Widget {
        Chart::new()
            .title("3D volume points")
