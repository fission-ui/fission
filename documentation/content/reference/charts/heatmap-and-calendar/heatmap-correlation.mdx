...
sidebar_label: Correlation heatmap
description: Fission Charts reference for Correlation heatmap.
---

# Correlation heatmap

![Correlation heatmap screenshot](/img/charts/heatmap-correlation.png)

Square-style heatmap for correlation-like data. Use it for relationship strength matrices. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Correlation heatmap is part of the **Heatmap and calendar** chart family and is intended for `Matrix value triples`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct CorrelationHeatmapChart;

impl From<CorrelationHeatmapChart> for Widget {
    fn from(_: CorrelationHeatmapChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
