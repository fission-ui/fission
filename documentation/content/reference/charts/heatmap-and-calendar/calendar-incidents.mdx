...
sidebar_label: Incident calendar
description: Fission Charts reference for Incident calendar.
---

# Incident calendar

![Incident calendar screenshot](/img/charts/calendar-incidents.png)

Calendar heatmap for incident counts. Use it for reliability and support reporting. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Incident calendar is part of the **Heatmap and calendar** chart family and is intended for `Date/value pairs`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct IncidentCalendarChart;

impl From<IncidentCalendarChart> for Widget {
    fn from(_: IncidentCalendarChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
