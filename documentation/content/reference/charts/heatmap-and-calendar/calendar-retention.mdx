...
sidebar_label: Retention calendar
description: Fission Charts reference for Retention calendar.
---

# Retention calendar

![Retention calendar screenshot](/img/charts/calendar-retention.png)

Calendar heatmap for retained activity. Use it for habit and retention surfaces. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Retention calendar is part of the **Heatmap and calendar** chart family and is intended for `Date/value pairs`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct RetentionCalendarChart;

impl From<RetentionCalendarChart> for Widget {
    fn from(_: RetentionCalendarChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
