...
sidebar_label: Build calendar
description: Fission Charts reference for Build calendar.
---

# Build calendar

![Build calendar screenshot](/img/charts/calendar-builds.png)

Calendar heatmap for build volume. Use it for engineering operations. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Build calendar is part of the **Heatmap and calendar** chart family and is intended for `Date/value pairs`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct BuildCalendarChart;

impl From<BuildCalendarChart> for Widget {
    fn from(_: BuildCalendarChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
