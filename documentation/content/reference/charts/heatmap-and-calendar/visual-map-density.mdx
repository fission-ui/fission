...
sidebar_label: Visual map density
description: Fission Charts reference for Visual map density.
---

# Visual map density

![Visual map density screenshot](/img/charts/visual-map-density.png)

Heatmap with explicit visual map color scale. Use it when color needs a visible numeric range. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Visual map density is part of the **Heatmap and calendar** chart family and is intended for `HeatmapSeries plus VisualMap`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`HeatmapSeries plus VisualMap.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct VisualMapDensityChart;

impl From<VisualMapDensityChart> for Widget {
    fn from(_: VisualMapDensityChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
