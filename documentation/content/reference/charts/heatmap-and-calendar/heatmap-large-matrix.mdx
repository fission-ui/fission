...
sidebar_label: Large matrix heatmap
description: Fission Charts reference for Large matrix heatmap.
---

# Large matrix heatmap

![Large matrix heatmap screenshot](/img/charts/heatmap-large-matrix.png)

Larger heatmap with more cells. Use it for activity grids with many columns. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Large matrix heatmap is part of the **Heatmap and calendar** chart family and is intended for `Dense matrix value triples`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Dense matrix value triples.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct LargeMatrixHeatmapChart;

impl From<LargeMatrixHeatmapChart> for Widget {
    fn from(_: LargeMatrixHeatmapChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
