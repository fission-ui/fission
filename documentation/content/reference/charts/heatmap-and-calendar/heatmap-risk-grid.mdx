...
Risk grid heatmap is part of the **Heatmap and calendar** chart family and is intended for `Matrix value triples`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct RiskGridHeatmapChart;

impl From<RiskGridHeatmapChart> for Widget {
    fn from(_: RiskGridHeatmapChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
