...
Quarter calendar heatmap is part of the **Heatmap and calendar** chart family and is intended for `Date/value pairs`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct QuarterCalendarHeatmapChart;

impl From<QuarterCalendarHeatmapChart> for Widget {
    fn from(_: QuarterCalendarHeatmapChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
