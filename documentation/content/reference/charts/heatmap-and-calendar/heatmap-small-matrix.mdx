...
sidebar_label: Small matrix heatmap
description: Fission Charts reference for Small matrix heatmap.
---

# Small matrix heatmap

![Small matrix heatmap screenshot](/img/charts/heatmap-small-matrix.png)

Compact heatmap for a small matrix. Use it for dashboards with limited space. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Small matrix heatmap is part of the **Heatmap and calendar** chart family and is intended for `Matrix value triples`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct SmallMatrixHeatmapChart;

impl From<SmallMatrixHeatmapChart> for Widget {
    fn from(_: SmallMatrixHeatmapChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
