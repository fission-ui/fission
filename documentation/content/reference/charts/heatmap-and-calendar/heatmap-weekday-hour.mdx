...
sidebar_label: Weekday hour heatmap
description: Fission Charts reference for Weekday hour heatmap.
---

# Weekday hour heatmap

![Weekday hour heatmap screenshot](/img/charts/heatmap-weekday-hour.png)

Matrix heatmap for activity by hour and weekday. Use it for operational intensity matrices. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Weekday hour heatmap is part of the **Heatmap and calendar** chart family and is intended for `x index, y index, value triples`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`x index, y index, value triples.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct WeekdayHourHeatmapChart;

impl From<WeekdayHourHeatmapChart> for Widget {
    fn from(_: WeekdayHourHeatmapChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
