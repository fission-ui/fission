...
sidebar_label: Availability heatmap
description: Fission Charts reference for Availability heatmap.
---
...
Availability values across services and periods. Use it for SRE and operations dashboards. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Availability heatmap is part of the **Heatmap and calendar** chart family and is intended for `Matrix value triples`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct AvailabilityHeatmapChart;

impl From<AvailabilityHeatmapChart> for Widget {
    fn from(_: AvailabilityHeatmapChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
