...
sidebar_label: Dynamic toolbox line
description: Fission Charts reference for Dynamic toolbox line.
---

# Dynamic toolbox line

![Dynamic toolbox line screenshot](/img/charts/dynamic-toolbox-line.png)

Line chart with explicit chart utility actions. Use it when zoom, brush, restore, and export controls belong with the chart. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Dynamic toolbox line is part of the **Dataset and dynamic** chart family and is intended for `Ordered samples plus toolbox action configuration`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Ordered samples plus toolbox action configuration.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DynamicToolboxLineChart;

impl From<DynamicToolboxLineChart> for Widget {
    fn from(_: DynamicToolboxLineChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
