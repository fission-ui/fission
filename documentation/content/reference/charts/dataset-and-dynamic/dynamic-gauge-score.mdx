...
sidebar_label: Dynamic score gauge
description: Fission Charts reference for Dynamic score gauge.
---

# Dynamic score gauge

![Dynamic score gauge screenshot](/img/charts/dynamic-gauge-score.png)

Gauge configured for a score or quality metric. Use it for scorecards where the main result should be immediately visible. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Dynamic score gauge is part of the **Dataset and dynamic** chart family and is intended for `Single label/value pair scaled into a gauge arc`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Single label/value pair scaled into a gauge arc.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DynamicScoreGaugeChart;

impl From<DynamicScoreGaugeChart> for Widget {
    fn from(_: DynamicScoreGaugeChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
