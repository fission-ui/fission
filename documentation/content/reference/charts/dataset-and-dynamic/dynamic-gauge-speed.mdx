...
sidebar_label: Dynamic speed gauge
description: Fission Charts reference for Dynamic speed gauge.
---

# Dynamic speed gauge

![Dynamic speed gauge screenshot](/img/charts/dynamic-gauge-speed.png)

Gauge configured for a changing bounded measure. Use it for operational status values where one number dominates the view. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Dynamic speed gauge is part of the **Dataset and dynamic** chart family and is intended for `Single label/value pair in a known range`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Single label/value pair in a known range.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DynamicSpeedGaugeChart;

impl From<DynamicSpeedGaugeChart> for Widget {
    fn from(_: DynamicSpeedGaugeChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
