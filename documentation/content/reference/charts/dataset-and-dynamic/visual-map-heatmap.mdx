...
sidebar_label: Visual map heatmap
description: Fission Charts reference for Visual map heatmap.
---

# Visual map heatmap

![Visual map heatmap screenshot](/img/charts/visual-map-heatmap.png)

Heatmap with a continuous value scale. Use it for dense comparisons where color communicates rank faster than labels. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Visual map heatmap is part of the **Dataset and dynamic** chart family and is intended for `Grid cell coordinates with numeric intensity`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Grid cell coordinates with numeric intensity.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct VisualMapHeatmapChart;

impl From<VisualMapHeatmapChart> for Widget {
    fn from(_: VisualMapHeatmapChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
