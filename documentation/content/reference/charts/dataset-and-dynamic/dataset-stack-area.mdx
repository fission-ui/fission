...
sidebar_label: Dataset stacked area
description: Fission Charts reference for Dataset stacked area.
---
...
Stacked area view built from reusable dataset-like series. Use it when the data pipeline should stay visible instead of hidden in chart-specific arrays. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Dataset stacked area is part of the **Dataset and dynamic** chart family and is intended for `Several named measures sharing ordered categories`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Several named measures sharing ordered categories.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DatasetStackedAreaChart;

impl From<DatasetStackedAreaChart> for Widget {
    fn from(_: DatasetStackedAreaChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
