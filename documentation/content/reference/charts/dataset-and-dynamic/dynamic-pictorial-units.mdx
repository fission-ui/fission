...
sidebar_label: Dynamic pictorial units
description: Fission Charts reference for Dynamic pictorial units.
---

# Dynamic pictorial units

![Dynamic pictorial units screenshot](/img/charts/dynamic-pictorial-units.png)

Pictorial bar using symbolic marks for count-like values. Use it for unit counts that benefit from a branded or icon-like visual form. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Dynamic pictorial units is part of the **Dataset and dynamic** chart family and is intended for `Category labels plus numeric values and a symbol choice`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Category labels plus numeric values and a symbol choice.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DynamicPictorialUnitsChart;

impl From<DynamicPictorialUnitsChart> for Widget {
    fn from(_: DynamicPictorialUnitsChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
