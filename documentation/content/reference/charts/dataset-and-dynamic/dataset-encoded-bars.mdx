...
sidebar_label: Encoded bars dataset
description: Fission Charts reference for Encoded bars dataset.
---

# Encoded bars dataset

![Encoded bars dataset screenshot](/img/charts/dataset-encoded-bars.png)

Grouped bars generated from named encoded fields. Use it when chart code should name fields once and reuse them across bar variants. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Encoded bars dataset is part of the **Dataset and dynamic** chart family and is intended for `Records with category, group, and numeric value fields`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Records with category, group, and numeric value fields.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct EncodedBarsDatasetChart;

impl From<EncodedBarsDatasetChart> for Widget {
    fn from(_: EncodedBarsDatasetChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
