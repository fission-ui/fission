...
sidebar_label: Dynamic alert scatter
description: Fission Charts reference for Dynamic alert scatter.
---

# Dynamic alert scatter

![Dynamic alert scatter screenshot](/img/charts/dynamic-effect-alerts.png)

Effect scatter highlighting important points over numeric axes. Use it when outliers or active alerts need immediate attention. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Dynamic alert scatter is part of the **Dataset and dynamic** chart family and is intended for `Small list of highlighted x/y samples`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Small list of highlighted x/y samples.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DynamicAlertScatterChart;

impl From<DynamicAlertScatterChart> for Widget {
    fn from(_: DynamicAlertScatterChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
