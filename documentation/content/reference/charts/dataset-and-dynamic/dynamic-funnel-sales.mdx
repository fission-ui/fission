...
sidebar_label: Dynamic sales funnel
description: Fission Charts reference for Dynamic sales funnel.
---

# Dynamic sales funnel

![Dynamic sales funnel screenshot](/img/charts/dynamic-funnel-sales.png)

Funnel showing staged conversion through a pipeline. Use it when the loss between stages matters more than exact axis measurement. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Dynamic sales funnel is part of the **Dataset and dynamic** chart family and is intended for `Stage/value pairs ordered by process position`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Stage/value pairs ordered by process position.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DynamicSalesFunnelChart;

impl From<DynamicSalesFunnelChart> for Widget {
    fn from(_: DynamicSalesFunnelChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
