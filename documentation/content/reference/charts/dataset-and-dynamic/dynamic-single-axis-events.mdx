...
sidebar_label: Dynamic single-axis events
description: Fission Charts reference for Dynamic single-axis events.
---

# Dynamic single-axis events

![Dynamic single-axis events screenshot](/img/charts/dynamic-single-axis-events.png)

Single-axis event distribution with value-coded marks. Use it for timelines or compact event strips. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Dynamic single-axis events is part of the **Dataset and dynamic** chart family and is intended for `Position/value pairs on one continuous axis`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Position/value pairs on one continuous axis.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DynamicSingleAxisEventsChart;

impl From<DynamicSingleAxisEventsChart> for Widget {
    fn from(_: DynamicSingleAxisEventsChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
