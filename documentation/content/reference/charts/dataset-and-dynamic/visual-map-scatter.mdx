...
sidebar_label: Visual map scatter
description: Fission Charts reference for Visual map scatter.
---

# Visual map scatter

![Visual map scatter screenshot](/img/charts/visual-map-scatter.png)

Bubble scatter with value-driven size and color scale. Use it when density and magnitude both matter. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Visual map scatter is part of the **Dataset and dynamic** chart family and is intended for `Triples of x, y, and magnitude values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Triples of x, y, and magnitude values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct VisualMapScatterChart;

impl From<VisualMapScatterChart> for Widget {
    fn from(_: VisualMapScatterChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
