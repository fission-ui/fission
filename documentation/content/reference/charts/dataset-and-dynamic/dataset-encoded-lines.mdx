...
sidebar_label: Encoded lines dataset
description: Fission Charts reference for Encoded lines dataset.
---

# Encoded lines dataset

![Encoded lines dataset screenshot](/img/charts/dataset-encoded-lines.png)

Line series generated from named encoded fields. Use it when the same dataset powers several line views. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Encoded lines dataset is part of the **Dataset and dynamic** chart family and is intended for `Records with ordered label, measure, and series fields`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Records with ordered label, measure, and series fields.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct EncodedLinesDatasetChart;

impl From<EncodedLinesDatasetChart> for Widget {
    fn from(_: EncodedLinesDatasetChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
