...
Dynamic polar score is part of the **Dataset and dynamic** chart family and is intended for `Label/value pairs mapped around a circle`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct DynamicPolarScoreChart;

impl From<DynamicPolarScoreChart> for Widget {
    fn from(_: DynamicPolarScoreChart) -> Widget {
        Chart::new()
            .title("Share")
            .legend(Legend::top_right())
            .series(vec![PieSeries::new("Share")
