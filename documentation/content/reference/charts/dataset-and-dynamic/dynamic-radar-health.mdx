...
sidebar_label: Dynamic radar health
description: Fission Charts reference for Dynamic radar health.
---

# Dynamic radar health

![Dynamic radar health screenshot](/img/charts/dynamic-radar-health.png)

Radar profile with multiple dimensions on one shape. Use it for multidimensional status summaries. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Dynamic radar health is part of the **Dataset and dynamic** chart family and is intended for `Several same-length vectors across named indicators`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Several same-length vectors across named indicators.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DynamicRadarHealthChart;

impl From<DynamicRadarHealthChart> for Widget {
    fn from(_: DynamicRadarHealthChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
