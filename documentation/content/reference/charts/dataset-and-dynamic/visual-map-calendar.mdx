...
Visual map calendar is part of the **Dataset and dynamic** chart family and is intended for `Date/value pairs over a calendar range`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct VisualMapCalendarChart;

impl From<VisualMapCalendarChart> for Widget {
    fn from(_: VisualMapCalendarChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
