...
sidebar_label: Dynamic brush scatter
description: Fission Charts reference for Dynamic brush scatter.
---

# Dynamic brush scatter

![Dynamic brush scatter screenshot](/img/charts/dynamic-brush-scatter.png)

Scatter chart with a visible brush selection preview. Use it when users need to select a region before drilling into data. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Dynamic brush scatter is part of the **Dataset and dynamic** chart family and is intended for `Point samples plus a brush rectangle`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Point samples plus a brush rectangle.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DynamicBrushScatterChart;

impl From<DynamicBrushScatterChart> for Widget {
    fn from(_: DynamicBrushScatterChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
