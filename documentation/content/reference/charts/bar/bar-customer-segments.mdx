...
sidebar_label: Customer segment bars
description: Fission Charts reference for Customer segment bars.
---

# Customer segment bars

![Customer segment bars screenshot](/img/charts/bar-customer-segments.png)

Customer segment bars uses Fission Charts typed Rust data to render a production-ready bar view. Use it to compare segments without splitting the page. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Customer segment bars is part of the **Bar** chart family and is intended for `Segment values grouped by period`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Segment values grouped by period.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct CustomerSegmentBarsChart;

impl From<CustomerSegmentBarsChart> for Widget {
    fn from(_: CustomerSegmentBarsChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
