---
title: Capacity bars
sidebar_label: Capacity bars
description: Fission Charts reference for Capacity bars.
---

# Capacity bars

![Capacity bars screenshot](/img/charts/bar-capacity.png)

Bars drawn inside visible capacity tracks. Use it for quota and inventory screens. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Capacity bars is part of the **Bar** chart family and is intended for `BarSeries plus maximum axis and background`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`BarSeries plus maximum axis and background.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct CapacityBarsChart;

impl From<CapacityBarsChart> for Widget {
    fn from(_: CapacityBarsChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
