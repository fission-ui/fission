...
sidebar_label: Grouped quarterly bars
description: Fission Charts reference for Grouped quarterly bars.
---

# Grouped quarterly bars

![Grouped quarterly bars screenshot](/img/charts/bar-grouped-quarter.png)

Grouped bars comparing several periods per category. Use it for side-by-side period comparisons. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Grouped quarterly bars is part of the **Bar** chart family and is intended for `Multiple BarSeries on the same categories`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Multiple BarSeries on the same categories.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct GroupedQuarterlyBarsChart;

impl From<GroupedQuarterlyBarsChart> for Widget {
    fn from(_: GroupedQuarterlyBarsChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
