...
sidebar_label: Retail waterfall
description: Fission Charts reference for Retail waterfall.
---

# Retail waterfall

![Retail waterfall screenshot](/img/charts/bar-retail-waterfall.png)

Retail waterfall uses Fission Charts typed Rust data to render a production-ready bar view. Use it for retail contribution analysis. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Retail waterfall is part of the **Bar** chart family and is intended for `Sequential sales and cost changes`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Sequential sales and cost changes.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct RetailWaterfallChart;

impl From<RetailWaterfallChart> for Widget {
    fn from(_: RetailWaterfallChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
