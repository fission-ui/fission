...
sidebar_label: Negative delta bars
description: Fission Charts reference for Negative delta bars.
---

# Negative delta bars

![Negative delta bars screenshot](/img/charts/bar-negative-delta.png)

Vertical bars with positive and negative changes. Use it for variance and profit/loss dashboards. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Negative delta bars is part of the **Bar** chart family and is intended for `BarSeries values crossing zero`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`BarSeries values crossing zero.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct NegativeDeltaBarsChart;

impl From<NegativeDeltaBarsChart> for Widget {
    fn from(_: NegativeDeltaBarsChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
