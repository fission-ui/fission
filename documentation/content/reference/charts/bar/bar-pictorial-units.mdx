...
sidebar_label: Pictorial units
description: Fission Charts reference for Pictorial units.
---

# Pictorial units

![Pictorial units screenshot](/img/charts/bar-pictorial-units.png)

Repeated symbols representing category values. Use it for branded but still quantitative category displays. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Pictorial units is part of the **Bar** chart family and is intended for `PictorialBarSeries values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`PictorialBarSeries values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct PictorialUnitsChart;

impl From<PictorialUnitsChart> for Widget {
    fn from(_: PictorialUnitsChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
