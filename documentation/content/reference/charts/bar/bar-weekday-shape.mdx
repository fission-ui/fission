...
sidebar_label: Weekday shape bars
description: Fission Charts reference for Weekday shape bars.
---

# Weekday shape bars

![Weekday shape bars screenshot](/img/charts/bar-weekday-shape.png)

Weekday shape bars uses Fission Charts typed Rust data to render a production-ready bar view. Use it when the weekly rhythm is easier as bars than a line. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Weekday shape bars is part of the **Bar** chart family and is intended for `Seven ordered weekday values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Seven ordered weekday values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct WeekdayShapeBarsChart;

impl From<WeekdayShapeBarsChart> for Widget {
    fn from(_: WeekdayShapeBarsChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
