...
sidebar_label: Wide-label horizontal bars
description: Fission Charts reference for Wide-label horizontal bars.
---

# Wide-label horizontal bars

![Wide-label horizontal bars screenshot](/img/charts/bar-wide-labels.png)

Horizontal layout preserving long category labels. Use it when labels would collide on the x-axis. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Wide-label horizontal bars is part of the **Bar** chart family and is intended for `BarSeries with category y-axis`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`BarSeries with category y-axis.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct WideLabelHorizontalBarsChart;

impl From<WideLabelHorizontalBarsChart> for Widget {
    fn from(_: WideLabelHorizontalBarsChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
