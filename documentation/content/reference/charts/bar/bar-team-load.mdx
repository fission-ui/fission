...
Team load bars is part of the **Bar** chart family and is intended for `Team categories ranked by workload`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct TeamLoadBarsChart;

impl From<TeamLoadBarsChart> for Widget {
    fn from(_: TeamLoadBarsChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
