---
title: Return deltas
sidebar_label: Return deltas
description: Fission Charts reference for Return deltas.
---

# Return deltas

![Return deltas screenshot](/img/charts/bar-return-deltas.png)

Return deltas uses Fission Charts typed Rust data to render a production-ready bar view. Use it for change analysis around zero. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Return deltas is part of the **Bar** chart family and is intended for `Category deltas with negative values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Category deltas with negative values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ReturnDeltasChart;

impl From<ReturnDeltasChart> for Widget {
    fn from(_: ReturnDeltasChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
