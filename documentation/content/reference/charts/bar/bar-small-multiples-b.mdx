...
sidebar_label: Small multiple bars B
description: Fission Charts reference for Small multiple bars B.
---

# Small multiple bars B

![Small multiple bars B screenshot](/img/charts/bar-small-multiples-b.png)

A second compact bar chart with different distribution. Use it when multiple products use the same visual scale. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Small multiple bars B is part of the **Bar** chart family and is intended for `BarSeries values on a category axis`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`BarSeries values on a category axis.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct SmallMultipleBarsBChart;

impl From<SmallMultipleBarsBChart> for Widget {
    fn from(_: SmallMultipleBarsBChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
