...
sidebar_label: Store comparison
description: Fission Charts reference for Store comparison.
---

# Store comparison

![Store comparison screenshot](/img/charts/bar-store-comparison.png)

Store comparison uses Fission Charts typed Rust data to render a production-ready bar view. Use it to compare periods inside each category. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Store comparison is part of the **Bar** chart family and is intended for `Grouped category values across periods`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Grouped category values across periods.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct StoreComparisonChart;

impl From<StoreComparisonChart> for Widget {
    fn from(_: StoreComparisonChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
