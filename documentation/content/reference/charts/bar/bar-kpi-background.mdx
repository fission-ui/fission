...
sidebar_label: KPI background bars
description: Fission Charts reference for KPI background bars.
---

# KPI background bars

![KPI background bars screenshot](/img/charts/bar-kpi-background.png)

KPI bars with background tracks. Use it for KPI completion panels. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

KPI background bars is part of the **Bar** chart family and is intended for `BarSeries with background styling`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`BarSeries with background styling.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct KPIBackgroundBarsChart;

impl From<KPIBackgroundBarsChart> for Widget {
    fn from(_: KPIBackgroundBarsChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
