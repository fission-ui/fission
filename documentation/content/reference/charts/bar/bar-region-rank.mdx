...
sidebar_label: Region ranking
description: Fission Charts reference for Region ranking.
---

# Region ranking

![Region ranking screenshot](/img/charts/bar-region-rank.png)

Region ranking uses Fission Charts typed Rust data to render a production-ready bar view. Use it when exact rank is the product question. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Region ranking is part of the **Bar** chart family and is intended for `Category/value pairs sorted for comparison`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Category/value pairs sorted for comparison.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct RegionRankingChart;

impl From<RegionRankingChart> for Widget {
    fn from(_: RegionRankingChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
