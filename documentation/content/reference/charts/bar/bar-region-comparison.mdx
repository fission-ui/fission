...
sidebar_label: Region comparison bars
description: Fission Charts reference for Region comparison bars.
---

# Region comparison bars

![Region comparison bars screenshot](/img/charts/bar-region-comparison.png)

Grouped regional values over shared categories. Use it for region-by-period comparisons. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Region comparison bars is part of the **Bar** chart family and is intended for `Multiple BarSeries`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Multiple BarSeries.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct RegionComparisonBarsChart;

impl From<RegionComparisonBarsChart> for Widget {
    fn from(_: RegionComparisonBarsChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
