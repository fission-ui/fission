...
sidebar_label: Compact category bars
description: Fission Charts reference for Compact category bars.
---

# Compact category bars

![Compact category bars screenshot](/img/charts/bar-compact.png)

Small category comparison with a clean vertical bar layout. Use it for short lists and cards. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Compact category bars is part of the **Bar** chart family and is intended for `Vec<f32> aligned to categories`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Vec<f32> aligned to categories.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct CompactCategoryBarsChart;

impl From<CompactCategoryBarsChart> for Widget {
    fn from(_: CompactCategoryBarsChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
