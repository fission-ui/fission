...
sidebar_label: Quarter waterfall
description: Fission Charts reference for Quarter waterfall.
---

# Quarter waterfall

![Quarter waterfall screenshot](/img/charts/bar-quarter-waterfall.png)

Quarter waterfall uses Fission Charts typed Rust data to render a production-ready bar view. Use it for financial bridge and variance analysis. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Quarter waterfall is part of the **Bar** chart family and is intended for `Sequential deltas that explain a final value`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Sequential deltas that explain a final value.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct QuarterWaterfallChart;

impl From<QuarterWaterfallChart> for Widget {
    fn from(_: QuarterWaterfallChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
