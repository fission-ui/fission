...
sidebar_label: Utilization capacity
description: Fission Charts reference for Utilization capacity.
---

# Utilization capacity

![Utilization capacity screenshot](/img/charts/bar-utilization-capacity.png)

Utilization capacity uses Fission Charts typed Rust data to render a production-ready bar view. Use it for infrastructure and quota dashboards. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Utilization capacity is part of the **Bar** chart family and is intended for `Usage bars with capacity tracks`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Usage bars with capacity tracks.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct UtilizationCapacityChart;

impl From<UtilizationCapacityChart> for Widget {
    fn from(_: UtilizationCapacityChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
