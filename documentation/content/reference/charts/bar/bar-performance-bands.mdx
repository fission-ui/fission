...
sidebar_label: Performance bands
description: Fission Charts reference for Performance bands.
---

# Performance bands

![Performance bands screenshot](/img/charts/bar-performance-bands.png)

Performance bands uses Fission Charts typed Rust data to render a production-ready bar view. Use it for scorecards where progress is the dominant signal. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Performance bands is part of the **Bar** chart family and is intended for `Values shown against a visual capacity track`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Values shown against a visual capacity track.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct PerformanceBandsChart;

impl From<PerformanceBandsChart> for Widget {
    fn from(_: PerformanceBandsChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
