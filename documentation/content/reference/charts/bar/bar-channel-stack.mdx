...
![Channel stack screenshot](/img/charts/bar-channel-stack.png)

Channel stack uses Fission Charts typed Rust data to render a production-ready bar view. Use it when total and contribution both matter. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Channel stack is part of the **Bar** chart family and is intended for `Multiple series stacked per category`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Multiple series stacked per category.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
