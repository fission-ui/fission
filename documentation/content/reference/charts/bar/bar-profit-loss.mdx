...
sidebar_label: Profit and loss bars
description: Fission Charts reference for Profit and loss bars.
---

# Profit and loss bars

![Profit and loss bars screenshot](/img/charts/bar-profit-loss.png)

Profit and loss bars uses Fission Charts typed Rust data to render a production-ready bar view. Use it when gains and losses must share one baseline. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Profit and loss bars is part of the **Bar** chart family and is intended for `Positive and negative values on one value axis`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Positive and negative values on one value axis.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ProfitAndLossBarsChart;

impl From<ProfitAndLossBarsChart> for Widget {
    fn from(_: ProfitAndLossBarsChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
