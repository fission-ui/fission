...
sidebar_label: Priority queue bars
description: Fission Charts reference for Priority queue bars.
---

# Priority queue bars

![Priority queue bars screenshot](/img/charts/bar-priority-queue.png)

Priority queue bars uses Fission Charts typed Rust data to render a production-ready bar view. Use it when bars sit inside a polished app surface. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Priority queue bars is part of the **Bar** chart family and is intended for `Rounded bars for a compact queue view`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Rounded bars for a compact queue view.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct PriorityQueueBarsChart;

impl From<PriorityQueueBarsChart> for Widget {
    fn from(_: PriorityQueueBarsChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
