...
sidebar_label: Ticket age distribution
description: Fission Charts reference for Ticket age distribution.
---

# Ticket age distribution

![Ticket age distribution screenshot](/img/charts/bar-ticket-age.png)

Ticket age distribution uses Fission Charts typed Rust data to render a production-ready bar view. Use it when the reader needs the shape of a queue. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Ticket age distribution is part of the **Bar** chart family and is intended for `Buckets with counts per age range`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Buckets with counts per age range.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct TicketAgeDistributionChart;

impl From<TicketAgeDistributionChart> for Widget {
    fn from(_: TicketAgeDistributionChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
