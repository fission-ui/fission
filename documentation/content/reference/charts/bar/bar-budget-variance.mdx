...
Budget variance is part of the **Bar** chart family and is intended for `Signed budget variances on a horizontal axis`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct BudgetVarianceChart;

impl From<BudgetVarianceChart> for Widget {
    fn from(_: BudgetVarianceChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
