...
sidebar_label: Track progress bars
description: Fission Charts reference for Track progress bars.
---

# Track progress bars

![Track progress bars screenshot](/img/charts/bar-track-progress.png)

Bars shown against full-range tracks. Use it for completion and capacity comparisons. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Track progress bars is part of the **Bar** chart family and is intended for `BarSeries with background color`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`BarSeries with background color.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct TrackProgressBarsChart;

impl From<TrackProgressBarsChart> for Widget {
    fn from(_: TrackProgressBarsChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
