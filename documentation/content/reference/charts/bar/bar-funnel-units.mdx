...
sidebar_label: Funnel unit pictorial bars
description: Fission Charts reference for Funnel unit pictorial bars.
---

# Funnel unit pictorial bars

![Funnel unit pictorial bars screenshot](/img/charts/bar-funnel-units.png)

Funnel unit pictorial bars uses Fission Charts typed Rust data to render a production-ready bar view. Use it when unit counts should feel more tactile than rectangles. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Funnel unit pictorial bars is part of the **Bar** chart family and is intended for `Values represented by repeated symbols`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Values represented by repeated symbols.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct FunnelUnitPictorialBarsChart;

impl From<FunnelUnitPictorialBarsChart> for Widget {
    fn from(_: FunnelUnitPictorialBarsChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
