...
sidebar_label: SLA breach delta
description: Fission Charts reference for SLA breach delta.
---

# SLA breach delta

![SLA breach delta screenshot](/img/charts/bar-sla-breach-delta.png)

SLA breach delta uses Fission Charts typed Rust data to render a production-ready bar view. Use it for operational exception reporting. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

SLA breach delta is part of the **Bar** chart family and is intended for `Deltas above and below a service baseline`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Deltas above and below a service baseline.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct SLABreachDeltaChart;

impl From<SLABreachDeltaChart> for Widget {
    fn from(_: SLABreachDeltaChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
