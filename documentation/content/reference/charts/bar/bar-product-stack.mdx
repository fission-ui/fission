...
sidebar_label: Product stack bars
description: Fission Charts reference for Product stack bars.
---

# Product stack bars

![Product stack bars screenshot](/img/charts/bar-product-stack.png)

Product stack bars uses Fission Charts typed Rust data to render a production-ready bar view. Use it for portfolio composition. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Product stack bars is part of the **Bar** chart family and is intended for `Product values stacked into category totals`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Product values stacked into category totals.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ProductStackBarsChart;

impl From<ProductStackBarsChart> for Widget {
    fn from(_: ProductStackBarsChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
