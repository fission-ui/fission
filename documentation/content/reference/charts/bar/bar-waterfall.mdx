---
title: Waterfall bar
sidebar_label: Waterfall bar
description: Fission Charts reference for Waterfall bar.
---

# Waterfall bar

![Waterfall bar screenshot](/img/charts/bar-waterfall.png)

Sequential changes displayed as rising and falling bars. Use it for profit bridges and cumulative change explanations. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Waterfall bar is part of the **Bar** chart family and is intended for `Delta values over ordered categories`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Delta values over ordered categories.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct WaterfallBarChart;

impl From<WaterfallBarChart> for Widget {
    fn from(_: WaterfallBarChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
