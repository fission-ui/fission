...
sidebar_label: Small multiple bars A
description: Fission Charts reference for Small multiple bars A.
---

# Small multiple bars A

![Small multiple bars A screenshot](/img/charts/bar-small-multiples-a.png)

A focused bar chart variant for compact dashboards. Use it in repeated dashboard cards. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Small multiple bars A is part of the **Bar** chart family and is intended for `BarSeries values on a category axis`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct SmallMultipleBarsAChart;

impl From<SmallMultipleBarsAChart> for Widget {
    fn from(_: SmallMultipleBarsAChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
