---
title: Diverging bar
sidebar_label: Diverging bar
description: Fission Charts reference for Diverging bar.
---

# Diverging bar

![Diverging bar screenshot](/img/charts/bar-diverging.png)

Horizontal bars extending in both directions from zero. Use it for sentiment, deltas, and balance views. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Diverging bar is part of the **Bar** chart family and is intended for `Positive and negative BarSeries values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Positive and negative BarSeries values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DivergingBarChart;

impl From<DivergingBarChart> for Widget {
    fn from(_: DivergingBarChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
