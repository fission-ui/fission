...
Polar bar is part of the **Coordinates** chart family and is intended for `Label/value pairs rendered as radial bar segments`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
```rust
use fission::charts::{Chart, PolarBarSeries};
...
impl From<PolarBarChart> for Widget {
    fn from(_: PolarBarChart) -> Widget {
        Chart::new()
            .title("Polar bar")
            .series(vec![PolarBarSeries::new("Radial spend")
                .data(vec![("Search", 82.0), ("Direct", 67.0), ("Email", 43.0), ("Ads", 58.0)])
