...
sidebar_label: Feature usage heatmap
description: Fission Charts reference for Feature usage heatmap.
---

# Feature usage heatmap

![Feature usage heatmap screenshot](/img/charts/heatmap-feature-usage.png)
...
Feature usage heatmap is part of the **Heatmap** chart family and is intended for `Feature/category values on a two-dimensional grid`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct FeatureUsageHeatmapChart;

impl From<FeatureUsageHeatmapChart> for Widget {
    fn from(_: FeatureUsageHeatmapChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
