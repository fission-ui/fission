...
sidebar_label: Daily retention calendar
description: Fission Charts reference for Daily retention calendar.
---

# Daily retention calendar

![Daily retention calendar screenshot](/img/charts/calendar-retention-daily.png)

Daily retention calendar uses Fission Charts typed Rust data to render a production-ready heatmap view. Use it to scan day-level consistency. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Daily retention calendar is part of the **Heatmap** chart family and is intended for `Date/value pairs for retention events`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Date/value pairs for retention events.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DailyRetentionCalendarChart;

impl From<DailyRetentionCalendarChart> for Widget {
    fn from(_: DailyRetentionCalendarChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
