...
sidebar_label: Load grid visual map
description: Fission Charts reference for Load grid visual map.
---

# Load grid visual map

![Load grid visual map screenshot](/img/charts/visual-map-load-grid.png)

Load grid visual map uses Fission Charts typed Rust data to render a production-ready heatmap view. Use it when color should explain intensity. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Load grid visual map is part of the **Heatmap** chart family and is intended for `Numeric grid values mapped to color`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Numeric grid values mapped to color.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct LoadGridVisualMapChart;

impl From<LoadGridVisualMapChart> for Widget {
    fn from(_: LoadGridVisualMapChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
