...
sidebar_label: Deployment hour heatmap
description: Fission Charts reference for Deployment hour heatmap.
---

# Deployment hour heatmap

![Deployment hour heatmap screenshot](/img/charts/heatmap-deployment-hours.png)

Deployment hour heatmap uses Fission Charts typed Rust data to render a production-ready heatmap view. Use it to find concentration by hour and day. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Deployment hour heatmap is part of the **Heatmap** chart family and is intended for `Grid coordinates with numeric intensity`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Grid coordinates with numeric intensity.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DeploymentHourHeatmapChart;

impl From<DeploymentHourHeatmapChart> for Widget {
    fn from(_: DeploymentHourHeatmapChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
