...
sidebar_label: User activity calendar
description: Fission Charts reference for User activity calendar.
---

# User activity calendar

![User activity calendar screenshot](/img/charts/calendar-user-activity.png)

User activity calendar uses Fission Charts typed Rust data to render a production-ready heatmap view. Use it to show consistency and spikes over time. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

User activity calendar is part of the **Heatmap** chart family and is intended for `Date/value activity pairs`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Date/value activity pairs.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct UserActivityCalendarChart;

impl From<UserActivityCalendarChart> for Widget {
    fn from(_: UserActivityCalendarChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
