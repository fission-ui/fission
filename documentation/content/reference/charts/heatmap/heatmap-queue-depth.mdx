...
sidebar_label: Queue depth matrix
description: Fission Charts reference for Queue depth matrix.
---

# Queue depth matrix

![Queue depth matrix screenshot](/img/charts/heatmap-queue-depth.png)

Queue depth matrix uses Fission Charts typed Rust data to render a production-ready heatmap view. Use it for operational queues. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Queue depth matrix is part of the **Heatmap** chart family and is intended for `Queue depth by queue and time bucket`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Queue depth by queue and time bucket.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct QueueDepthMatrixChart;

impl From<QueueDepthMatrixChart> for Widget {
    fn from(_: QueueDepthMatrixChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
