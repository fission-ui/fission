...
sidebar_label: Quality gates calendar
description: Fission Charts reference for Quality gates calendar.
---

# Quality gates calendar

![Quality gates calendar screenshot](/img/charts/calendar-quality-gates.png)

Quality gates calendar uses Fission Charts typed Rust data to render a production-ready heatmap view. Use it for quality gate consistency over time. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Quality gates calendar is part of the **Heatmap** chart family and is intended for `Daily quality values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Daily quality values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct QualityGatesCalendarChart;

impl From<QualityGatesCalendarChart> for Widget {
    fn from(_: QualityGatesCalendarChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
