...
sidebar_label: Daily sales calendar
description: Fission Charts reference for Daily sales calendar.
---

# Daily sales calendar

![Daily sales calendar screenshot](/img/charts/calendar-sales-daily.png)

Daily sales calendar uses Fission Charts typed Rust data to render a production-ready heatmap view. Use it when weekday and date position matter. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Daily sales calendar is part of the **Heatmap** chart family and is intended for `Daily sales values over a date range`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Daily sales values over a date range.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DailySalesCalendarChart;

impl From<DailySalesCalendarChart> for Widget {
    fn from(_: DailySalesCalendarChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
