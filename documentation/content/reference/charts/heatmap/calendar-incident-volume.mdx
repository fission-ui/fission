...
sidebar_label: Incident volume calendar
description: Fission Charts reference for Incident volume calendar.
---

# Incident volume calendar

![Incident volume calendar screenshot](/img/charts/calendar-incident-volume.png)

Incident volume calendar uses Fission Charts typed Rust data to render a production-ready heatmap view. Use it when the calendar shape carries meaning. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Incident volume calendar is part of the **Heatmap** chart family and is intended for `Daily incident counts over a date range`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Daily incident counts over a date range.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct IncidentVolumeCalendarChart;

impl From<IncidentVolumeCalendarChart> for Widget {
    fn from(_: IncidentVolumeCalendarChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
