...
sidebar_label: Visual map build calendar
description: Fission Charts reference for Visual map build calendar.
---

# Visual map build calendar

![Visual map build calendar screenshot](/img/charts/visual-map-calendar-builds.png)

Visual map build calendar uses Fission Charts typed Rust data to render a production-ready heatmap view. Use it when calendar and intensity both matter. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Visual map build calendar is part of the **Heatmap** chart family and is intended for `Daily build values with a color scale`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Daily build values with a color scale.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct VisualMapBuildCalendarChart;

impl From<VisualMapBuildCalendarChart> for Widget {
    fn from(_: VisualMapBuildCalendarChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
