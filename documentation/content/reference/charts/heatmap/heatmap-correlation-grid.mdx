...
sidebar_label: Correlation grid
description: Fission Charts reference for Correlation grid.
---

# Correlation grid

![Correlation grid screenshot](/img/charts/heatmap-correlation-grid.png)

Correlation grid uses Fission Charts typed Rust data to render a production-ready heatmap view. Use it for correlation-like analysis. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Correlation grid is part of the **Heatmap** chart family and is intended for `Grid of pairwise values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Grid of pairwise values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct CorrelationGridChart;

impl From<CorrelationGridChart> for Widget {
    fn from(_: CorrelationGridChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
