---
title: Access matrix
sidebar_label: Access matrix
description: Fission Charts reference for Access matrix.
---
...
Access matrix uses Fission Charts typed Rust data to render a production-ready heatmap view. Use it for dense access and security matrices. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Access matrix is part of the **Heatmap** chart family and is intended for `Permission-like matrix values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Permission-like matrix values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct AccessMatrixChart;

impl From<AccessMatrixChart> for Widget {
    fn from(_: AccessMatrixChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
