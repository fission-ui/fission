...
sidebar_label: Resource saturation
description: Fission Charts reference for Resource saturation.
---

# Resource saturation

![Resource saturation screenshot](/img/charts/heatmap-resource-saturation.png)

Resource saturation uses Fission Charts typed Rust data to render a production-ready heatmap view. Use it for infrastructure saturation. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Resource saturation is part of the **Heatmap** chart family and is intended for `Resource and time values on a matrix`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Resource and time values on a matrix.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ResourceSaturationChart;

impl From<ResourceSaturationChart> for Widget {
    fn from(_: ResourceSaturationChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
