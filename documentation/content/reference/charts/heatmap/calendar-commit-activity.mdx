...
sidebar_label: Commit activity calendar
description: Fission Charts reference for Commit activity calendar.
---

# Commit activity calendar

![Commit activity calendar screenshot](/img/charts/calendar-commit-activity.png)

Commit activity calendar uses Fission Charts typed Rust data to render a production-ready heatmap view. Use it to show daily activity patterns. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Commit activity calendar is part of the **Heatmap** chart family and is intended for `Date/value pairs over a calendar range`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct CommitActivityCalendarChart;

impl From<CommitActivityCalendarChart> for Widget {
    fn from(_: CommitActivityCalendarChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
