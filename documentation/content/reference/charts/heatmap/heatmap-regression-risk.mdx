...
sidebar_label: Regression risk matrix
description: Fission Charts reference for Regression risk matrix.
---

# Regression risk matrix

![Regression risk matrix screenshot](/img/charts/heatmap-regression-risk.png)

Regression risk matrix uses Fission Charts typed Rust data to render a production-ready heatmap view. Use it before releases to focus attention. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Regression risk matrix is part of the **Heatmap** chart family and is intended for `Risk values by component and area`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Risk values by component and area.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
