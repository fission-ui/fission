...
sidebar_label: Density visual map
description: Fission Charts reference for Density visual map.
---

# Density visual map

![Density visual map screenshot](/img/charts/visual-map-density-grid.png)

Density visual map uses Fission Charts typed Rust data to render a production-ready heatmap view. Use it for density and occupancy surfaces. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Density visual map is part of the **Heatmap** chart family and is intended for `Dense grid with continuous color scale`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Dense grid with continuous color scale.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DensityVisualMapChart;

impl From<DensityVisualMapChart> for Widget {
    fn from(_: DensityVisualMapChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
