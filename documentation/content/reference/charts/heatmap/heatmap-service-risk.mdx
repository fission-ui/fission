...
sidebar_label: Service risk matrix
description: Fission Charts reference for Service risk matrix.
---

# Service risk matrix

![Service risk matrix screenshot](/img/charts/heatmap-service-risk.png)

Service risk matrix uses Fission Charts typed Rust data to render a production-ready heatmap view. Use it when dense matrix comparison matters. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Service risk matrix is part of the **Heatmap** chart family and is intended for `Larger matrix of risk values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Larger matrix of risk values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ServiceRiskMatrixChart;

impl From<ServiceRiskMatrixChart> for Widget {
    fn from(_: ServiceRiskMatrixChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
