...
sidebar_label: Monitoring annotation
description: Fission Charts reference for Monitoring annotation.
---

# Monitoring annotation

![Monitoring annotation screenshot](/img/charts/monitoring-annotation.png)

Monitoring annotation uses Fission Charts typed Rust data to render a production-ready monitoring view. Use it when dashboards need explanatory context. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Monitoring annotation is part of the **Monitoring** chart family and is intended for `Trend with typed graphic callout`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Trend with typed graphic callout.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct MonitoringAnnotationChart;

impl From<MonitoringAnnotationChart> for Widget {
    fn from(_: MonitoringAnnotationChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
