...
sidebar_label: Monitoring traffic bars
description: Fission Charts reference for Monitoring traffic bars.
---

# Monitoring traffic bars

![Monitoring traffic bars screenshot](/img/charts/monitoring-traffic-bars.png)

Monitoring traffic bars uses Fission Charts typed Rust data to render a production-ready monitoring view. Use it for quick categorical traffic summaries. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Monitoring traffic bars is part of the **Monitoring** chart family and is intended for `Traffic counts by category`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Traffic counts by category.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct MonitoringTrafficBarsChart;

impl From<MonitoringTrafficBarsChart> for Widget {
    fn from(_: MonitoringTrafficBarsChart) -> Widget {
        Chart::new()
            .title("Orders")
            .x_axis(Axis::category(vec!["Q1", "Q2", "Q3", "Q4"]))
            .y_axis(Axis::value())
