---
title: Error scatter
sidebar_label: Error scatter
description: Fission Charts reference for Error scatter.
---

# Error scatter

![Error scatter screenshot](/img/charts/monitoring-error-scatter.png)

Error scatter uses Fission Charts typed Rust data to render a production-ready monitoring view. Use it for outlier-heavy operational views. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Error scatter is part of the **Monitoring** chart family and is intended for `Highlighted error points`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Highlighted error points.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ErrorScatterChart;

impl From<ErrorScatterChart> for Widget {
    fn from(_: ErrorScatterChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
