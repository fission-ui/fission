...
sidebar_label: Monitoring overview line
description: Fission Charts reference for Monitoring overview line.
---

# Monitoring overview line

![Monitoring overview line screenshot](/img/charts/monitoring-overview-line.png)

Monitoring overview line uses Fission Charts typed Rust data to render a production-ready monitoring view. Use it for top-level operational dashboards. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Monitoring overview line is part of the **Monitoring** chart family and is intended for `Dense telemetry samples`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Dense telemetry samples.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct MonitoringOverviewLineChart;

impl From<MonitoringOverviewLineChart> for Widget {
    fn from(_: MonitoringOverviewLineChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
