...
sidebar_label: Monitoring error band
description: Fission Charts reference for Monitoring error band.
---

# Monitoring error band

![Monitoring error band screenshot](/img/charts/monitoring-error-band.png)

Monitoring error band uses Fission Charts typed Rust data to render a production-ready monitoring view. Use it when alert ranges must be visible. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Monitoring error band is part of the **Monitoring** chart family and is intended for `Error values with threshold band`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Error values with threshold band.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct MonitoringErrorBandChart;

impl From<MonitoringErrorBandChart> for Widget {
    fn from(_: MonitoringErrorBandChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
