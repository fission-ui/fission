...
sidebar_label: Uptime calendar
description: Fission Charts reference for Uptime calendar.
---

# Uptime calendar

![Uptime calendar screenshot](/img/charts/monitoring-uptime-calendar.png)

Uptime calendar uses Fission Charts typed Rust data to render a production-ready monitoring view. Use it for long-term reliability summaries. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Uptime calendar is part of the **Monitoring** chart family and is intended for `Daily uptime values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Daily uptime values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct UptimeCalendarChart;

impl From<UptimeCalendarChart> for Widget {
    fn from(_: UptimeCalendarChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
