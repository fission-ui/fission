...
sidebar_label: Operational route lines
description: Fission Charts reference for Operational route lines.
---

# Operational route lines

![Operational route lines screenshot](/img/charts/monitoring-route-lines.png)

Operational route lines uses Fission Charts typed Rust data to render a production-ready monitoring view. Use it for route and link movement. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Operational route lines is part of the **Monitoring** chart family and is intended for `Connection lines with effect markers`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Connection lines with effect markers.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct OperationalRouteLinesChart;

impl From<OperationalRouteLinesChart> for Widget {
    fn from(_: OperationalRouteLinesChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
