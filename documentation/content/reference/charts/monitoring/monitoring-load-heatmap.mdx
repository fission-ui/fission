---
title: Load heatmap
sidebar_label: Load heatmap
description: Fission Charts reference for Load heatmap.
---

# Load heatmap

![Load heatmap screenshot](/img/charts/monitoring-load-heatmap.png)

Load heatmap uses Fission Charts typed Rust data to render a production-ready monitoring view. Use it for time and resource concentration. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Load heatmap is part of the **Monitoring** chart family and is intended for `Resource load matrix`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Resource load matrix.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct LoadHeatmapChart;

impl From<LoadHeatmapChart> for Widget {
    fn from(_: LoadHeatmapChart) -> Widget {
        Chart::new()
            .title("Load")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed"]))
            .y_axis(Axis::category(vec!["AM", "PM"]))
