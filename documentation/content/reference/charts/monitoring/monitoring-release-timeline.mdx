...
description: Fission Charts reference for Release timeline.
---

# Release timeline

![Release timeline screenshot](/img/charts/monitoring-release-timeline.png)

Release timeline uses Fission Charts typed Rust data to render a production-ready monitoring view. Use it for release-based dashboards. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Release timeline is part of the **Monitoring** chart family and is intended for `Timeline control over chart state`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Timeline control over chart state.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ReleaseTimelineChart;

impl From<ReleaseTimelineChart> for Widget {
    fn from(_: ReleaseTimelineChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
