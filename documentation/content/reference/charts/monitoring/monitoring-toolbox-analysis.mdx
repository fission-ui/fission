...
sidebar_label: Monitoring toolbox
description: Fission Charts reference for Monitoring toolbox.
---

# Monitoring toolbox

![Monitoring toolbox screenshot](/img/charts/monitoring-toolbox-analysis.png)

Monitoring toolbox uses Fission Charts typed Rust data to render a production-ready monitoring view. Use it when charts need local analysis controls. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Monitoring toolbox is part of the **Monitoring** chart family and is intended for `Line chart with analysis actions`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Line chart with analysis actions.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct MonitoringToolboxChart;

impl From<MonitoringToolboxChart> for Widget {
    fn from(_: MonitoringToolboxChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
