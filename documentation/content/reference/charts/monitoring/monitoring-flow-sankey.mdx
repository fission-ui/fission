...
sidebar_label: Operational flow sankey
description: Fission Charts reference for Operational flow sankey.
---

# Operational flow sankey

![Operational flow sankey screenshot](/img/charts/monitoring-flow-sankey.png)

Operational flow sankey uses Fission Charts typed Rust data to render a production-ready monitoring view. Use it for request and process flow. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Operational flow sankey is part of the **Monitoring** chart family and is intended for `Flow nodes and links`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Flow nodes and links.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct OperationalFlowSankeyChart;

impl From<OperationalFlowSankeyChart> for Widget {
    fn from(_: OperationalFlowSankeyChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
