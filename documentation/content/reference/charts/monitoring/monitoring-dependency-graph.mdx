...
description: Fission Charts reference for Dependency graph.
---

# Dependency graph

![Dependency graph screenshot](/img/charts/monitoring-dependency-graph.png)

Dependency graph uses Fission Charts typed Rust data to render a production-ready monitoring view. Use it for operational topology. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Dependency graph is part of the **Monitoring** chart family and is intended for `Service nodes and dependencies`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Service nodes and dependencies.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct DependencyGraphChart;

impl From<DependencyGraphChart> for Widget {
    fn from(_: DependencyGraphChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
