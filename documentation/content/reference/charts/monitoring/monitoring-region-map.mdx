...
sidebar_label: Operational region map
description: Fission Charts reference for Operational region map.
---

# Operational region map

![Operational region map screenshot](/img/charts/monitoring-region-map.png)

Operational region map uses Fission Charts typed Rust data to render a production-ready monitoring view. Use it for geographic operations. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Operational region map is part of the **Monitoring** chart family and is intended for `Region values with color scale`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Region values with color scale.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct OperationalRegionMapChart;

impl From<OperationalRegionMapChart> for Widget {
    fn from(_: OperationalRegionMapChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
