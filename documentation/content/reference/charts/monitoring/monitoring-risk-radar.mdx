...
sidebar_label: Monitoring risk radar
description: Fission Charts reference for Monitoring risk radar.
---

# Monitoring risk radar

![Monitoring risk radar screenshot](/img/charts/monitoring-risk-radar.png)

Monitoring risk radar uses Fission Charts typed Rust data to render a production-ready monitoring view. Use it for status summaries across dimensions. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Monitoring risk radar is part of the **Monitoring** chart family and is intended for `Multidimensional status vector`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Multidimensional status vector.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct MonitoringRiskRadarChart;

impl From<MonitoringRiskRadarChart> for Widget {
    fn from(_: MonitoringRiskRadarChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
