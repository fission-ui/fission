...
sidebar_label: Monitoring event strip
description: Fission Charts reference for Monitoring event strip.
---

# Monitoring event strip

![Monitoring event strip screenshot](/img/charts/monitoring-single-axis-events.png)

Monitoring event strip uses Fission Charts typed Rust data to render a production-ready monitoring view. Use it for compact event timeline rows. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Monitoring event strip is part of the **Monitoring** chart family and is intended for `Event positions with value-coded marks`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Event positions with value-coded marks.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct MonitoringEventStripChart;

impl From<MonitoringEventStripChart> for Widget {
    fn from(_: MonitoringEventStripChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
