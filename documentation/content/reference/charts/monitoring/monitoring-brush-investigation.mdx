...
sidebar_label: Brush investigation
description: Fission Charts reference for Brush investigation.
---
...
Brush investigation uses Fission Charts typed Rust data to render a production-ready monitoring view. Use it when incidents require selecting a point range. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Brush investigation is part of the **Monitoring** chart family and is intended for `Scatter points and brush selection`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Scatter points and brush selection.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct BrushInvestigationChart;

impl From<BrushInvestigationChart> for Widget {
    fn from(_: BrushInvestigationChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
