...
sidebar_label: Capacity gauge
description: Fission Charts reference for Capacity gauge.
---

# Capacity gauge

![Capacity gauge screenshot](/img/charts/monitoring-capacity-gauge.png)

Capacity gauge uses Fission Charts typed Rust data to render a production-ready monitoring view. Use it for critical status cards. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Capacity gauge is part of the **Monitoring** chart family and is intended for `One capacity value in a bounded range`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`One capacity value in a bounded range.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct CapacityGaugeChart;

impl From<CapacityGaugeChart> for Widget {
    fn from(_: CapacityGaugeChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
