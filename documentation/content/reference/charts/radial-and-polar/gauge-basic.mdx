...
title: Gauge
sidebar_label: Gauge
description: Fission Charts reference for Gauge.
---

# Gauge

![Gauge screenshot](/img/charts/gauge-basic.png)

A dial-style chart for one bounded measure. Use it when the value is read as a current instrument state. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Gauge is part of the **Radial and polar** chart family and is intended for `One label/value pair with an expected range`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
impl From<GaugeChart> for Widget {
    fn from(_: GaugeChart) -> Widget {
        Chart::new()
            .title("Gauge")
            .width(720.0)
            .height(480.0)
