...
sidebar_label: Rose by radius
description: Fission Charts reference for Rose by radius.
---

# Rose by radius

![Rose by radius screenshot](/img/charts/pie-rose-radius.png)

Slices use radius to make category differences more expressive. Use it for presentation-heavy part-to-whole views. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Rose by radius is part of the **Radial and polar** chart family and is intended for `Label/value pairs plus radius rose mode`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
```rust
use fission::charts::{Chart, PieSeries};
...
pub struct RoseByRadiusChart;

impl From<RoseByRadiusChart> for Widget {
    fn from(_: RoseByRadiusChart) -> Widget {
        Chart::new()
            .title("Share")
            .series(vec![PieSeries::new("Share")
                .data(vec![("Search", 1048.0), ("Direct", 735.0), ("Email", 580.0), ("Ads", 484.0)])
