...
Deployment marks is part of the **Interaction** chart family and is intended for `Trend data with event mark points`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct DeploymentMarksChart;

impl From<DeploymentMarksChart> for Widget {
    fn from(_: DeploymentMarksChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
