...
sidebar_label: Marked SLO chart
description: Fission Charts reference for Marked SLO chart.
---

# Marked SLO chart

![Marked SLO chart screenshot](/img/charts/interaction-marked-slo.png)

Marked SLO chart uses Fission Charts typed Rust data to render a production-ready interaction view. Use it to keep thresholds in the chart instead of external notes. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Marked SLO chart is part of the **Interaction** chart family and is intended for `Trend data plus mark lines and shaded bands`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Trend data plus mark lines and shaded bands.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct MarkedSLOChart;

impl From<MarkedSLOChart> for Widget {
    fn from(_: MarkedSLOChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
