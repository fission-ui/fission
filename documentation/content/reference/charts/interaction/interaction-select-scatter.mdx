...
sidebar_label: Selectable scatter
description: Fission Charts reference for Selectable scatter.
---

# Selectable scatter

![Selectable scatter screenshot](/img/charts/interaction-select-scatter.png)

Selectable scatter uses Fission Charts typed Rust data to render a production-ready interaction view. Use it when users inspect clusters interactively. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Selectable scatter is part of the **Interaction** chart family and is intended for `Scatter points with selection configuration`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Scatter points with selection configuration.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct SelectableScatterChart;

impl From<SelectableScatterChart> for Widget {
    fn from(_: SelectableScatterChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
