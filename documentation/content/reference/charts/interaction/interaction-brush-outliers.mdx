...
sidebar_label: Brush outliers
description: Fission Charts reference for Brush outliers.
---

# Brush outliers

![Brush outliers screenshot](/img/charts/interaction-brush-outliers.png)

Brush outliers uses Fission Charts typed Rust data to render a production-ready interaction view. Use it for exploratory selection. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Brush outliers is part of the **Interaction** chart family and is intended for `Point data with visible brush preview`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Point data with visible brush preview.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct BrushOutliersChart;

impl From<BrushOutliersChart> for Widget {
    fn from(_: BrushOutliersChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
