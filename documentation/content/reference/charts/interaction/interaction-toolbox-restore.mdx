...
sidebar_label: Restore toolbox
description: Fission Charts reference for Restore toolbox.
---

# Restore toolbox

![Restore toolbox screenshot](/img/charts/interaction-toolbox-restore.png)

Restore toolbox uses Fission Charts typed Rust data to render a production-ready interaction view. Use it when user exploration needs a safe reset. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Restore toolbox is part of the **Interaction** chart family and is intended for `Chart data with reset action included`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Chart data with reset action included.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct RestoreToolboxChart;

impl From<RestoreToolboxChart> for Widget {
    fn from(_: RestoreToolboxChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
