...
sidebar_label: Grouped tooltip
description: Fission Charts reference for Grouped tooltip.
---

# Grouped tooltip

![Grouped tooltip screenshot](/img/charts/interaction-tooltip-grouped.png)

Grouped tooltip uses Fission Charts typed Rust data to render a production-ready interaction view. Use it when comparison needs hover detail. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Grouped tooltip is part of the **Interaction** chart family and is intended for `Grouped series with tooltip intent`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Grouped series with tooltip intent.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct GroupedTooltipChart;

impl From<GroupedTooltipChart> for Widget {
    fn from(_: GroupedTooltipChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
