...
sidebar_label: Export toolbox
description: Fission Charts reference for Export toolbox.
---

# Export toolbox

![Export toolbox screenshot](/img/charts/interaction-toolbox-export.png)

Export toolbox uses Fission Charts typed Rust data to render a production-ready interaction view. Use it when chart controls need a consistent built-in place. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Export toolbox is part of the **Interaction** chart family and is intended for `Chart data with restore, brush, zoom, and save actions`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Chart data with restore, brush, zoom, and save actions.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ExportToolboxChart;

impl From<ExportToolboxChart> for Widget {
    fn from(_: ExportToolboxChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
