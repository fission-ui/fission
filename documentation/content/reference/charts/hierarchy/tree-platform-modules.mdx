...
sidebar_label: Platform module tree
description: Fission Charts reference for Platform module tree.
---

# Platform module tree

![Platform module tree screenshot](/img/charts/tree-platform-modules.png)

Platform module tree uses Fission Charts typed Rust data to render a production-ready hierarchy view. Use it for ownership and dependency hierarchy. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Platform module tree is part of the **Hierarchy** chart family and is intended for `Nested named nodes with values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Nested named nodes with values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct PlatformModuleTreeChart;

impl From<PlatformModuleTreeChart> for Widget {
    fn from(_: PlatformModuleTreeChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
