...
sidebar_label: Budget allocation treemap
description: Fission Charts reference for Budget allocation treemap.
---

# Budget allocation treemap

![Budget allocation treemap screenshot](/img/charts/treemap-budget-allocation.png)

Budget allocation treemap uses Fission Charts typed Rust data to render a production-ready hierarchy view. Use it for budget share exploration. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Budget allocation treemap is part of the **Hierarchy** chart family and is intended for `Nested budget values sized by area`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Nested budget values sized by area.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct BudgetAllocationTreemapChart;

impl From<BudgetAllocationTreemapChart> for Widget {
    fn from(_: BudgetAllocationTreemapChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
