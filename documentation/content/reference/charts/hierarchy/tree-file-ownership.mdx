...
sidebar_label: File ownership tree
description: Fission Charts reference for File ownership tree.
---

# File ownership tree

![File ownership tree screenshot](/img/charts/tree-file-ownership.png)

File ownership tree uses Fission Charts typed Rust data to render a production-ready hierarchy view. Use it for repository and ownership views. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

File ownership tree is part of the **Hierarchy** chart family and is intended for `Nested file or team ownership nodes`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Nested file or team ownership nodes.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct FileOwnershipTreeChart;

impl From<FileOwnershipTreeChart> for Widget {
    fn from(_: FileOwnershipTreeChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
