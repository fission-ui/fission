...
sidebar_label: Alert correlation graph
description: Fission Charts reference for Alert correlation graph.
---
...
Alert correlation graph uses Fission Charts typed Rust data to render a production-ready hierarchy view. Use it for incident analysis. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Alert correlation graph is part of the **Hierarchy** chart family and is intended for `Related alert nodes and links`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Related alert nodes and links.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct AlertCorrelationGraphChart;

impl From<AlertCorrelationGraphChart> for Widget {
    fn from(_: AlertCorrelationGraphChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
