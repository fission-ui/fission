...
sidebar_label: Energy balance sankey
description: Fission Charts reference for Energy balance sankey.
---

# Energy balance sankey

![Energy balance sankey screenshot](/img/charts/sankey-energy-balance.png)

Energy balance sankey uses Fission Charts typed Rust data to render a production-ready hierarchy view. Use it for transfer and loss diagrams. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Energy balance sankey is part of the **Hierarchy** chart family and is intended for `Flow nodes and links`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct EnergyBalanceSankeyChart;

impl From<EnergyBalanceSankeyChart> for Widget {
    fn from(_: EnergyBalanceSankeyChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
