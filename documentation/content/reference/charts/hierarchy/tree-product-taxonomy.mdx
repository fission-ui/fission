...
sidebar_label: Product taxonomy tree
description: Fission Charts reference for Product taxonomy tree.
---

# Product taxonomy tree

![Product taxonomy tree screenshot](/img/charts/tree-product-taxonomy.png)

Product taxonomy tree uses Fission Charts typed Rust data to render a production-ready hierarchy view. Use it when users browse hierarchical structure. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Product taxonomy tree is part of the **Hierarchy** chart family and is intended for `Nested product categories`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Nested product categories.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ProductTaxonomyTreeChart;

impl From<ProductTaxonomyTreeChart> for Widget {
    fn from(_: ProductTaxonomyTreeChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
