...
sidebar_label: Lead flow sankey
description: Fission Charts reference for Lead flow sankey.
---

# Lead flow sankey

![Lead flow sankey screenshot](/img/charts/sankey-lead-flow.png)

Lead flow sankey uses Fission Charts typed Rust data to render a production-ready hierarchy view. Use it when movement between stages is the question. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Lead flow sankey is part of the **Hierarchy** chart family and is intended for `Nodes and directed weighted links`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Nodes and directed weighted links.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct LeadFlowSankeyChart;

impl From<LeadFlowSankeyChart> for Widget {
    fn from(_: LeadFlowSankeyChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
