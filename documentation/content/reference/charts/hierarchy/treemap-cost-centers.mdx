...
sidebar_label: Cost center treemap
description: Fission Charts reference for Cost center treemap.
---

# Cost center treemap

![Cost center treemap screenshot](/img/charts/treemap-cost-centers.png)

Cost center treemap uses Fission Charts typed Rust data to render a production-ready hierarchy view. Use it for part-to-whole hierarchy analysis. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Cost center treemap is part of the **Hierarchy** chart family and is intended for `Hierarchical values sized by area`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Hierarchical values sized by area.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct CostCenterTreemapChart;

impl From<CostCenterTreemapChart> for Widget {
    fn from(_: CostCenterTreemapChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
