...
sidebar_label: Channel mix river
description: Fission Charts reference for Channel mix river.
---

# Channel mix river

![Channel mix river screenshot](/img/charts/theme-river-channel-mix.png)

Channel mix river uses Fission Charts typed Rust data to render a production-ready hierarchy view. Use it to show mix changes without losing continuity. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Channel mix river is part of the **Hierarchy** chart family and is intended for `Channel values over time`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Channel values over time.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ChannelMixRiverChart;

impl From<ChannelMixRiverChart> for Widget {
    fn from(_: ChannelMixRiverChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
