...
Customer segment sunburst is part of the **Hierarchy** chart family and is intended for `Nested customer groups by value`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
pub struct CustomerSegmentSunburstChart;

impl From<CustomerSegmentSunburstChart> for Widget {
    fn from(_: CustomerSegmentSunburstChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
