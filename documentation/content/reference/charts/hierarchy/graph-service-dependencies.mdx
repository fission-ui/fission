...
description: Fission Charts reference for Service dependencies.
---

# Service dependencies

![Service dependencies screenshot](/img/charts/graph-service-dependencies.png)

Service dependencies uses Fission Charts typed Rust data to render a production-ready hierarchy view. Use it to explain relationship topology. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Service dependencies is part of the **Hierarchy** chart family and is intended for `Nodes and links with values`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Nodes and links with values.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ServiceDependenciesChart;

impl From<ServiceDependenciesChart> for Widget {
    fn from(_: ServiceDependenciesChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
