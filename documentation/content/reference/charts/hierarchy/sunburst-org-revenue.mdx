...
sidebar_label: Org revenue sunburst
description: Fission Charts reference for Org revenue sunburst.
---

# Org revenue sunburst

![Org revenue sunburst screenshot](/img/charts/sunburst-org-revenue.png)

Org revenue sunburst uses Fission Charts typed Rust data to render a production-ready hierarchy view. Use it when hierarchy and whole composition both matter. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Org revenue sunburst is part of the **Hierarchy** chart family and is intended for `Hierarchy values as radial rings`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Hierarchy values as radial rings.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct OrgRevenueSunburstChart;

impl From<OrgRevenueSunburstChart> for Widget {
    fn from(_: OrgRevenueSunburstChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
