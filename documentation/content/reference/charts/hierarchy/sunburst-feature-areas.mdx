...
sidebar_label: Feature area sunburst
description: Fission Charts reference for Feature area sunburst.
---

# Feature area sunburst

![Feature area sunburst screenshot](/img/charts/sunburst-feature-areas.png)

Feature area sunburst uses Fission Charts typed Rust data to render a production-ready hierarchy view. Use it for hierarchical composition with depth. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Feature area sunburst is part of the **Hierarchy** chart family and is intended for `Nested values in radial layers`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Nested values in radial layers.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct FeatureAreaSunburstChart;

impl From<FeatureAreaSunburstChart> for Widget {
    fn from(_: FeatureAreaSunburstChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
