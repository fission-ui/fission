...
sidebar_label: Customer journey graph
description: Fission Charts reference for Customer journey graph.
---

# Customer journey graph

![Customer journey graph screenshot](/img/charts/graph-customer-journey.png)

Customer journey graph uses Fission Charts typed Rust data to render a production-ready hierarchy view. Use it when paths and relationships matter more than axes. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Customer journey graph is part of the **Hierarchy** chart family and is intended for `Relationship nodes and edges`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Relationship nodes and edges.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct CustomerJourneyGraphChart;

impl From<CustomerJourneyGraphChart> for Widget {
    fn from(_: CustomerJourneyGraphChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
