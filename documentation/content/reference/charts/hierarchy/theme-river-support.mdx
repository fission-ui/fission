...
sidebar_label: Support theme river
description: Fission Charts reference for Support theme river.
---

# Support theme river

![Support theme river screenshot](/img/charts/theme-river-support.png)

Support theme river uses Fission Charts typed Rust data to render a production-ready hierarchy view. Use it for shifting composition over time. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Support theme river is part of the **Hierarchy** chart family and is intended for `Stacked stream values by time and category`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Stacked stream values by time and category.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct SupportThemeRiverChart;

impl From<SupportThemeRiverChart> for Widget {
    fn from(_: SupportThemeRiverChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
