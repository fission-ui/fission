...
sidebar_label: Resolution path sankey
description: Fission Charts reference for Resolution path sankey.
---

# Resolution path sankey

![Resolution path sankey screenshot](/img/charts/sankey-resolution-path.png)

Resolution path sankey uses Fission Charts typed Rust data to render a production-ready hierarchy view. Use it to analyze process movement. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Resolution path sankey is part of the **Hierarchy** chart family and is intended for `Support stages as flow links`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Support stages as flow links.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct ResolutionPathSankeyChart;

impl From<ResolutionPathSankeyChart> for Widget {
    fn from(_: ResolutionPathSankeyChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
