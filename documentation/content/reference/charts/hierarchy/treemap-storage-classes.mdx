...
sidebar_label: Storage class treemap
description: Fission Charts reference for Storage class treemap.
---

# Storage class treemap

![Storage class treemap screenshot](/img/charts/treemap-storage-classes.png)

Storage class treemap uses Fission Charts typed Rust data to render a production-ready hierarchy view. Use it when area should communicate magnitude. The screenshot is captured from the native Fission chart gallery.

## What the chart is for

Storage class treemap is part of the **Hierarchy** chart family and is intended for `Nested values by storage class`. Use it when the visual form makes the user's question faster to answer than a table or a simpler chart would.
...
`Nested values by storage class.` Keep the data close to the Rust type that describes it. Fission Charts is typed Rust, so each chart uses explicit series and component structs instead of string configuration hidden in a loosely typed object.
...
pub struct StorageClassTreemapChart;

impl From<StorageClassTreemapChart> for Widget {
    fn from(_: StorageClassTreemapChart) -> Widget {
        Chart::new()
            .title("Trend")
            .x_axis(Axis::category(vec!["Mon", "Tue", "Wed", "Thu", "Fri"]))
            .y_axis(Axis::value())
