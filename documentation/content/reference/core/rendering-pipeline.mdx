...
| shell presentation | render output plus host target | presented frame | Connects the shared runtime to a real target host |
...
For the widget contract that starts this pipeline, see [Widget authoring boundary](/reference/core/widget-authoring-boundary). For the teaching version of the same staged model, see [Rendering pipeline](/docs/learn/rendering-pipeline). For stage-level investigation tools, continue to [Testing and diagnostics](/reference/core/testing-and-diagnostics).
