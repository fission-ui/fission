...
`PointerEvent` covers pointer down, up, move, cancellation, scroll, and
magnification. Down, move, up, and cancellation retain a `PointerId` and
`PointerKind`, so multiple touch or stylus contacts remain distinct. Scroll
events preserve `ScrollDeltaMode` and `PointerPhase`; magnification reports a
multiplicative scale factor and lifecycle phase. Pointer buttons are represented
by `PointerButton`.
