...
| Ember | `FissionEmberDesignSystem` | `crates/core/fission-theme/design/ember` |
| Graphite | `FissionGraphiteDesignSystem` | `crates/core/fission-theme/design/graphite` |
...
Generated design systems can expose packaged fonts through `DesignSystem::font_faces()`. Each `PackagedFont` contains the family, weight, style, format, embedded bytes, and optional variation axes. Shells register those fonts for rendering so components can use generated typography tokens without relying on platform font installation.

See [Fonts](/reference/core/fonts/) for the field-level reference and [Design systems and fonts](/docs/develop/design-systems-and-fonts/) for the app setup flow.
