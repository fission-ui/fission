...
Desktop exposes both `with_env(...)` and `with_sync_env(...)`. Mobile and web expose `with_sync_env(...)`. SSR apps expose `with_env(...)` and `with_request_env(...)`. Static site targets expose `with_env(...)`.
