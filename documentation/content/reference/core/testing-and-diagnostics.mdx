...
`fission-test-driver` exposes `LiveTestClient::compare_golden(...)` and `GoldenOptions` for image comparisons. Use this after the live shell has reached the desired state through selectors, keyboard input, scrolling, or routed navigation.

The comparison should report pass/fail, changed-pixel counts, thresholds, and diff artifacts where supported. See [GoldenOptions](/reference/testing/golden-options/) and [LiveTest commands](/reference/testing/live-test-commands/).
