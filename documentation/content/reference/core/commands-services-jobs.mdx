...
Jobs are also the normal place to consume large binary streams returned by host capabilities. A reducer should pass a `DataStreamId` in the typed job request, then the job opens it with `JobCtx::open_data_stream(...)` and reads the resulting `FissionDataStream`. SSR/server jobs expose the same stream handle contract through `ServerJobCtx`.
...
Services can consume large streams in the same way as jobs through `ServiceCtx::open_data_stream(...)`. Use this when the stream belongs to a long-lived conversation, such as upload progress, import progress, or an editor/language-service workflow that should keep emitting events.
...
If the lifetime should follow the widget tree instead of one reducer event, the next page is [Resources and capabilities](/reference/core/resources-and-capabilities). For the broader reducer-side contract, pair this page with [State system](/reference/core/state-system), [Resources and async](/docs/guides/resources-and-async/), and [Why reducers are synchronous](/docs/guides/why-reducers-are-synchronous/). For file-picker and upload streams, read [How do I do file uploads in Fission?](/docs/guides/file-uploads/).
