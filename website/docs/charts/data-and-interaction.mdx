...
description: Understand datasets, axes, visual maps, zoom, tooltips, and future interaction paths in Fission Charts.
---
...
## Tooltips, axis pointers, and zoom

Tooltips and axis pointers are interactive readout tools. They should answer "what exact value is under the pointer?" without forcing the user to estimate from grid lines. Data zoom lets the user inspect part of an ordered series without losing the overall context.

The current chart renderer stores tooltip, axis pointer, and data zoom configuration in the typed chart model. The next implementation work wires those configuration records into pointer handling, viewport filtering, and reducers so chart interaction follows the same explicit event path as the rest of Fission.
