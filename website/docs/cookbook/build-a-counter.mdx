...
This may feel more formal than an inline click handler, but that formality is the point. `Increment` gives the event a name that the whole app can understand. Tests can dispatch it. Reducers can handle it. Logs can refer to it. As apps grow, this becomes much easier to reason about than anonymous closures scattered across the widget tree.

Here again, the attribute is scaffolding.

- `pub struct Increment;` is the part to focus on first. It declares a named action type.
- Because there are no fields inside it, this action simply means "the increment event happened." It does not need to carry extra data.
- `#[fission_action]` is the framework-facing helper line that makes the type usable as a Fission action. It applies the standard action derives for you so you do not have to repeat them on every action.

## Step 3: write the reducer that changes state

Now that the app can name the event, it needs a place to decide what that event means.

That place is the reducer.

If the word "reducer" is new, think of it as a state update function. It receives the current state and an action, then updates the state in a controlled way.
...
Every line here matters.

`state: &mut CounterState` means the reducer is allowed to change the app's durable data. `_action: Increment` says this reducer runs when the increment action is dispatched. `_ctx` is available for more advanced cases such as side effects, but the counter does not need it yet, so we leave it unused.

If the function signature looks intimidating, here is the plain-English version:

- `fn on_increment(...)` means "define a function named `on_increment`."
- `&mut CounterState` means "give this function access to the state, and allow it to change that state."
- `Increment` means "this reducer handles the `Increment` action."
- `ReducerContext<CounterState>` means "if the reducer needs runtime help, that help is available for this app state type."
- The leading underscores in `_action` and `_ctx` are Rust's way of saying "these values exist, but this small example is not using them yet."

The body is intentionally boring: `state.count += 1;`.
