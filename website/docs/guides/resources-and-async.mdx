...
A capability is for host or device work that belongs to the shell side of the app boundary. Opening a web address, starting an authentication session, or showing a native file picker are good examples. Fission ships typed built-in capabilities such as `OPEN_URL`, `AUTHENTICATE`, and `PICK_OPEN_FILES`.
...
A common mistake is treating capabilities as a generic escape hatch for anything asynchronous. Resist that temptation. Use them for host-owned actions specifically.
