---
title: Runtime model
description: "Learn the core mental model of a Fission app: state, actions, reducers, widgets, build(), View, BuildCtx, ReducerContext, and selectors."
---
...
- `#[fission_action]` asks Fission to add the standard action helper behavior automatically.
- `impl Something for MyType {}` tells Rust that a type participates in a framework contract.
- angle brackets such as `Widget<NoteState>` or `ReducerContext<NoteState>` mean "this thing is being used with `NoteState`."
- `&` means "borrow this value without taking ownership" and `&mut` means "borrow it and allow changes."
- `reduce_with!(...)` is a small type hint for the compiler. At this stage, you can read it as "use this function as the reducer for that action."
...
#[fission_action]
#[serde(transparent)]
pub struct UpdateDraft(pub String);

#[fission_action]
pub struct SavePressed;

#[fission_action]
pub struct SaveSucceeded;

#[fission_action]
pub struct SaveFailed;
```

Again, the framework idea matters more than the syntax.

`UpdateDraft(pub String)` is a one-field wrapper around a string. In plain English, it means "an action that carries new draft text." Rust lets you write short wrapper types this way instead of always naming the field. Later, when the reducer reads `action.0`, that just means "the first and only value inside this wrapper."

`SavePressed`, `SaveSucceeded`, and `SaveFailed` are zero-field structs. They are simply named action types with no extra data attached.

The `#[fission_action]` line is the framework-facing scaffolding that makes each type usable as a Fission action. It applies the standard action derives for you, so for now you can read it as "make this type work as a serializable action value."
...
A reducer is just a function that receives the current state plus an action and updates the state in response. If you have never seen the word before, do not let it sound more mysterious than it is. A reducer reduces "what happened" into "the new state".

```rust
fn on_update_draft(
    state: &mut NoteState,
    action: UpdateDraft,
    _ctx: &mut ReducerContext<NoteState>,
) {
    state.draft = action.0;
    state.save_error = None;
}

fn on_save_succeeded(
    state: &mut NoteState,
    _action: SaveSucceeded,
    _ctx: &mut ReducerContext<NoteState>,
) {
    state.saving = false;
}
...
`state: &mut NoteState` means the reducer receives a mutable reference to the current app state, so it is allowed to change that state. `action: UpdateDraft` means this reducer only handles one specific action type. `_ctx: &mut ReducerContext<NoteState>` means the runtime also provides reducer context, but this reducer is not using it yet. The leading underscore is Rust's way of saying "yes, I know this parameter exists, but I am intentionally not reading it in this function."
            ...
            children: vec![
                ...
                TextInput {
                    ...
                    on_change: Some(ctx.bind(
                        UpdateDraft(String::new()),
                        reduce_with!(on_update_draft),
                    )),
                    ..Default::default()
                }
                .into_node(),
...
- `UpdateDraft(String::new())` is a placeholder action value used so `ctx.bind(...)` knows which action type to wire up. The runtime replaces the placeholder string with the real current input text when the user edits the field.
- `reduce_with!(on_update_draft)` and `reduce_with!(on_save_pressed)` are explicit type hints. They help Rust understand which reducer function should handle each action without making the example spell out the full handler type.
- `Box::new(...)` is Rust's way of putting one value inside another heap-allocated container. In these user interface examples, you can usually read it as "nest this child widget here."
...
fn on_save_pressed(
    state: &mut NoteState,
    _action: SavePressed,
    ctx: &mut ReducerContext<NoteState>,
) {
    if state.saving {
        return;
    }
...
Read [Rendering pipeline](/docs/learn/rendering-pipeline) next to see what happens after `build()` returns its user interface description.
...
If you want to understand the environment data that also flows through `View`, continue to [Environment, input, and text](/docs/guides/input-events-text-and-env).
