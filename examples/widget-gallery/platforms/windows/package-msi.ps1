...
$ExecutableName = if ($env:WINDOWS_EXECUTABLE_NAME) { $env:WINDOWS_EXECUTABLE_NAME } else { "widget-gallery.exe" }
...
$MsiPath = Join-Path $OutRoot "widget-gallery-$Profile.msi"
$Version = if ($env:WINDOWS_MSI_VERSION) { $env:WINDOWS_MSI_VERSION } else { "0.1.0" }
$UpgradeCode = if ($env:WINDOWS_MSI_UPGRADE_CODE) { $env:WINDOWS_MSI_UPGRADE_CODE } else { "264830bd-9a4e-4029-b894-7cbbb3ce9d28" }
...
<Wix xmlns="http://wixtoolset.org/schemas/v4/wxs">
  <Package Name="widget-gallery" Manufacturer="Fission Developer" Version="$Version" UpgradeCode="$UpgradeCode" Scope="perMachine">
    <MajorUpgrade DowngradeErrorMessage="A newer version of widget-gallery is already installed." />
    <MediaTemplate EmbedCab="yes" />
    <StandardDirectory Id="ProgramFiles6432Folder">
      <Directory Id="INSTALLFOLDER" Name="widget-gallery">
        <Component Id="MainExecutable" Guid="*">
          <File Id="AppExe" Source="$BinaryPath" KeyPath="yes" />
        </Component>
      </Directory>
    </StandardDirectory>
    <Feature Id="MainFeature" Title="widget-gallery" Level="1">
...
<Wix xmlns="http://schemas.microsoft.com/wix/2006/wi">
  <Product Id="*" Name="widget-gallery" Language="1033" Version="$Version" Manufacturer="Fission Developer" UpgradeCode="$UpgradeCode">
