...
$ExecutableName = if ($env:WINDOWS_EXECUTABLE_NAME) { $env:WINDOWS_EXECUTABLE_NAME } else { "motion-memory-repro.exe" }
...
$MsiPath = Join-Path $OutRoot "motion-memory-repro-$Profile.msi"
$Version = if ($env:WINDOWS_MSI_VERSION) { $env:WINDOWS_MSI_VERSION } else { "0.1.0" }
$UpgradeCode = if ($env:WINDOWS_MSI_UPGRADE_CODE) { $env:WINDOWS_MSI_UPGRADE_CODE } else { "581d3b0e-c5c4-4123-a737-e431ce9319d8" }
...
<Wix xmlns="http://wixtoolset.org/schemas/v4/wxs">
  <Package Name="motion-memory-repro" Manufacturer="Fission Developer" Version="$Version" UpgradeCode="$UpgradeCode" Scope="perMachine">
    <MajorUpgrade DowngradeErrorMessage="A newer version of motion-memory-repro is already installed." />
    <MediaTemplate EmbedCab="yes" />
    <StandardDirectory Id="ProgramFiles6432Folder">
      <Directory Id="INSTALLFOLDER" Name="motion-memory-repro">
        <Component Id="MainExecutable" Guid="*">
          <File Id="AppExe" Source="$BinaryPath" KeyPath="yes" />
        </Component>
      </Directory>
    </StandardDirectory>
    <Feature Id="MainFeature" Title="motion-memory-repro" Level="1">
...
<Wix xmlns="http://schemas.microsoft.com/wix/2006/wi">
  <Product Id="*" Name="motion-memory-repro" Language="1033" Version="$Version" Manufacturer="Fission Developer" UpgradeCode="$UpgradeCode">
