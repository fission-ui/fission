...
PACKAGE_NAME="interactive-canvas"
LIB_NAME="interactive_canvas"
...
"${GRADLE_CMD[@]}" -p "$SCRIPT_DIR" ":app:bundle$GRADLE_VARIANT"

ARTIFACT="$SCRIPT_DIR/app/build/outputs/bundle/$GRADLE_OUTPUT_DIR/app-$GRADLE_OUTPUT_DIR.aab"
if [[ ! -f "$ARTIFACT" ]]; then
  printf 'Gradle did not produce the expected AAB: %s\n' "$ARTIFACT" >&2
  exit 1
fi
printf '%s\n' "$ARTIFACT"
