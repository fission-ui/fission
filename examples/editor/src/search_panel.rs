...
use fission::core::ui::{
    Button, ButtonContentAlign, ButtonVariant, Column, Container, Scroll, Text, TextInput,
};
    ...
    fn build(
        ...
        fission::core::view::internal_node_widget({
            let node = {
                ...
                let search_row = Container::<fission::Node>::lowered(
                    HStack {
                        spacing: Some(0.0),
                        children: vec![
                            TextInput {
                                id: Some(fission::ir::NodeId::explicit(
                                    "editor_search_query_input",
                                )),
                        ...
                        result_nodes.push(
                            Button {
                                ...
                                on_press: Some(ActionEnvelope {
                                    id: open_id,
                                    payload: serde_json::to_vec(&OpenFile(result.path.clone()))
                                        .unwrap(),
                                }),
