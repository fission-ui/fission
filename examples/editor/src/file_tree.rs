            ...
            let node = {
                {
                    ...
                    let new_file_action = ActionEnvelope {
                        id: create_file_id,
                        payload: serde_json::to_vec(&CreateFile(format!(
                            "{}/untitled",
                            root_path_str
                        )))
                        .unwrap(),
                    ...
                    let toolbar = Container::<fission::Node>::lowered(
                        HStack {
                            spacing: Some(2.0),
                            children: vec![
                        Spacer {
                        ...
                        Button {
                            ...
                            child: Some(Box::new(
                                Icon::svg(
                                    fission::icons::material::file::create_new_folder::round(),
                                )
                                .size(18.0)
                                .color(icon_color)
                                .into_node(),
...
fn build_tree_rows(
    ...
    let row_content = Container::<fission::Node>::lowered(
        HStack {
            spacing: Some(4.0),
            children: vec![
                // Indentation spacer
    ...
    let row = if is_renaming {
        Container::<fission::Node>::lowered(row_content)
            .height(24.0)
            .into_node()
    } else {
