...
impl From<DocChart> for Widget {
    fn from(doc: DocChart) -> Self {
        let slug = doc.slug;
        let width = doc.width;
        let height = doc.height;
        let s = doc.scale;

        if let Some(meta) = deep_catalog::find_chart(&slug) {
            return deep_catalog::DeepChartNode {
                meta,
                width: Some(width),
                height: Some(height),
                scale: s,
        ...
        match slug.as_str() {
            "bar3d-basic" => {
                return dataset_3d::bar3d_scene(s)
                    .width(width)
                    .height(height)
                    .into()
            }
            "scatter3d-basic" => {
                return dataset_3d::scatter3d_scene(s)
                    .width(width)
                    .height(height)
                    .into()
            }
            "surface3d-basic" => {
                return dataset_3d::surface3d_scene(s)
                    .width(width)
                    .height(height)
                    .into()
            }
            "line3d-basic" => {
                return dataset_3d::line3d_scene(s)
                    .width(width)
                    .height(height)
                    .into()
            }
            "point-cloud" => {
                return dataset_3d::point_cloud_scene(s)
                    .width(width)
                    .height(height)
                    .into()
            }
            "globe-basic" => {
                return dataset_3d::globe_scene(s)
                    .width(width)
                    .height(height)
                    .into()
            }
            "graph3d-basic" => {
                return dataset_3d::graph3d_scene(s)
                    .width(width)
                    .height(height)
                    .into()
            }
            "terrain-surface" => {
                return dataset_3d::terrain_scene(s)
                    .width(width)
                    .height(height)
                    .into()
            }
        ...
        let chart = match slug.as_str() {
            ...
            "map-choropleth" => Chart::new()
                ...
                .series(vec![MapSeries::new("Regions", "demo")
                    ...
                    .into()]),
            _ => {
                return fission::core::ui::Container::new(fission::core::ui::Text::new(format!(
                    "Unknown chart doc slug: {slug}"
                )))
                .into();
            }
        };

        chart.width(width).height(height).into()
    }
}
