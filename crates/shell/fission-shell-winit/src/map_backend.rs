...
mod mac {
        ...
        fn create_controller(
            ...
            unsafe {
                ...
                Box::new(MacMapController {
                    map_view: RetainedId::owned(map_view),
                    widget_id,
                })
            }
        }
        ...
        fn present_surfaces(&self, frames: &[MapSurfaceFrame]) {
            ...
            for frame in frames {
                ...
                let entry = layers
                    ...
                    .or_insert_with(|| MapLayer::new(&ctx));
                entry.update(&ctx, frame.rect);
            }
    ...
    /// Wraps an MKMapView hosted as a subview.
    struct MapLayer {
        host_view: RetainedId,
    }

    impl MapLayer {
        fn new(ctx: &LayerContext) -> Self {
            unsafe {
                let frame = CGRect::new(&CGPoint::new(0.0, 0.0), &CGSize::new(1.0, 1.0));
        ...
        fn update(&mut self, ctx: &LayerContext, rect: LayoutRect) {
            unsafe {
                let cg_rect = cg_rect_from_layout(rect, ctx);
                let view = self.host_view.as_id();
                let () = msg_send![view, setFrame: cg_rect];

                // Check if the host view already has a map subview, if not
                // this is a layout-only update. The controller will manage
                // adding its MKMapView as a subview of the host.
                let () = msg_send![
                    ctx.parent_view,
                    addSubview: view
                    positioned: NSWindowOrderingMode::NSWindowAbove
    ...
    /// Expose the map view so the shell can parent it inside a MapLayer's host view.
    impl MacMapController {
        pub fn map_view_id(&self) -> id {
            self.map_view.as_id()
        }
    }
}
...
mod ios {
    ...
    impl MapBackend for IosMapBackend {
        ...
        fn present_surfaces(&self, frames: &[MapSurfaceFrame]) {
            ...
            for frame in frames {
                ...
                let entry = layers
                    .entry(frame.widget_id)
                    .or_insert_with(|| IosMapLayer::new(parent));
                entry.update(parent, frame.rect);
            }
    ...
    impl IosMapLayer {
        ...
        fn update(&mut self, parent: id, rect: LayoutRect) {
            unsafe {
