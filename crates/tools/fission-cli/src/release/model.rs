...
fn validate_provider_listing(
    ...
    let key = match provider {
        publish::DistributionProvider::PlayStore => "play_store",
        publish::DistributionProvider::AppStore => "app_store",
        publish::DistributionProvider::MicrosoftStore => "microsoft_store",
        _ => return,
    };
...
"#,
    ...
    fn release_config_validation_accepts_complete_release_files() {
        ...
        let report =
            validate_release_config_model(&dir, Some(publish::DistributionProvider::PlayStore))
                .unwrap();
