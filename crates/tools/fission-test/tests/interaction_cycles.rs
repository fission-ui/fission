...
fn text_input_focus_has_no_ir_cycles() -> Result<()> {
    ...
    impl Widget<AppState> for Root {
        fn build(&self, _ctx: &mut fission_core::BuildCtx<AppState>, _view: &View<AppState>) -> Node {
            TextInput { value: String::new(), placeholder: Some("type".into()), width: Some(200.0), height: Some(40.0), ..Default::default() }.into()
        }
    }
    ...
    h.send_event(InputEvent::Pointer(fission_core::event::PointerEvent::Down{ point: center, button: fission_core::event::PointerButton::Primary }))?;
    h.pump()?;

    let ir2 = h.last_ir.as_ref().unwrap();
    assert!(detect_ir_cycle(ir2).is_none(), "IR contains cycle after focusing TextInput");
    Ok(())
}
...
fn checkbox_toggle_has_no_ir_cycles() -> Result<()> {
    ...
    h.send_event(fission_core::InputEvent::Pointer(PointerEvent::Down{ point: center, button: PointerButton::Primary }))?;
    h.pump()?;
    h.send_event(fission_core::InputEvent::Pointer(PointerEvent::Up{ point: center, button: PointerButton::Primary }))?;
    h.pump()?;
