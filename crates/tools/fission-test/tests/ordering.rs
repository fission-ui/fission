...
impl Widget<AppState> for OrderRow {
    fn build(&self, _ctx: &mut BuildCtx<AppState>, _view: &View<AppState>) -> Node {
        Node::Row(Row {
            children: vec![
                Text { content: TextContent::Literal("A".into()), ..Default::default() }.into(),
                Text { content: TextContent::Literal("B".into()), ..Default::default() }.into(),
                Text { content: TextContent::Literal("C".into()), ..Default::default() }.into(),
            ],
            ..Default::default()
        })
    }
...
fn row_children_order_preserved_in_display_list() -> Result<()> {
    ...
    let texts: Vec<String> = dl.ops.iter().filter_map(|op| {
        if let DisplayOp::DrawText { text, .. } = op { Some(text.clone()) } else { None }
    }).collect();
    // We expect the first three DrawText ops to be A, B, C in order
    let prefix: Vec<String> = texts.into_iter().take(3).collect();
    assert_eq!(prefix, vec!["A".to_string(), "B".to_string(), "C".to_string()]);
    Ok(())
}
