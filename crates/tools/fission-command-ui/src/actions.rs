...
pub fn navigate(state: &mut UiState, route: UiRoute) {
    state.route = route;
}

#[fission_reducer(ToggleTheme)]
pub fn toggle_theme(state: &mut UiState) {
    state.theme_mode = state.theme_mode.toggle();
}

#[fission_reducer(ToggleCompactMode)]
pub fn toggle_compact_mode(state: &mut UiState) {
    state.compact_mode = !state.compact_mode;
}

#[fission_reducer(SelectTarget)]
pub fn select_target(state: &mut UiState, target: Target) {
...
pub fn select_device(state: &mut UiState, id: String) {
    state.selected_device = Some(id);
}

#[fission_reducer(SelectCommandSession)]
pub fn select_command_session(state: &mut UiState, id: u64) {
...
pub fn cancel_dialog(state: &mut UiState) {
