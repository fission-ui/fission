...
shadows. A paired `SkiaRasterProfile` retains SkParagraph paint data from the
authoritative layout result. In-memory images are resolved only from each
submitted frame's resource snapshot, decoded through SkCodec under a bounded
driver-owned cache, and painted with all current fit, alignment, clipping, and
device-scale semantics. Other image sources, GPU surfaces, SVG documents,
