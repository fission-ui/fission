...
impl ForceGraphLayout {
    pub fn compute_positions(
        ...
        for (i, node) in nodes.iter().enumerate() {
            ...
            pos.insert(node.id.clone(), (cx + r * angle.cos(), cy + r * angle.sin()));
        }

        if len == 0.0 { return pos; }
        ...
        for _ in 0..iterations {
            ...
            for edge in edges {
                let v = &edge.source;
                let u = &edge.target;
                if !pos.contains_key(v) || !pos.contains_key(u) { continue; }
...
mod tests {
    ...
    fn test_force_graph() {
        let nodes = vec![
            GraphNode { id: "1".into(), name: "1".into(), value: 1.0 },
            GraphNode { id: "2".into(), name: "2".into(), value: 1.0 },
        ];
