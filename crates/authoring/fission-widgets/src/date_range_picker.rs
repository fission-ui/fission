...
pub struct DateRangePicker {
    ...
    pub on_change: Option<Arc<dyn Fn(Option<NaiveDate>, Option<NaiveDate>) -> ActionEnvelope + Send + Sync>>,
        ...
        HStack {
            ...
            children: vec![
                DatePicker {
                    ...
                    on_change: cb.clone().map(|f| Arc::new(move |d| f(Some(d), e)) as Arc<dyn Fn(NaiveDate) -> ActionEnvelope + Send + Sync>),
                ...
                DatePicker {
                    ...
                    on_change: cb.map(|f| Arc::new(move |d| f(s, Some(d))) as Arc<dyn Fn(NaiveDate) -> ActionEnvelope + Send + Sync>),
