...
pub enum ActionTrigger {
    ...
    DragCancel,
    /// The primary pointer was held without becoming a drag for at least the
    /// platform-neutral long-press threshold.
    LongPress,
}
...
mod tests {
    ...
    #[test]
    fn long_press_round_trips_through_ir_serialization() {
        let encoded = serde_json::to_string(&ActionTrigger::LongPress).unwrap();
        assert_eq!(encoded, "\"LongPress\"");
        let decoded: ActionTrigger = serde_json::from_str(&encoded).unwrap();
        assert_eq!(decoded, ActionTrigger::LongPress);
    }
    ...
    fn existing_action_trigger_discriminants_remain_stable() {
        ...
        assert_eq!(ActionTrigger::LongPress as u8, 25);
    }
}
