...
use crate::{
    Action, ActionEnvelope, ActionId, ActionInput, AppState, Effect, EffectPayload, ReducerContext,
    SystemEffect,
};
use anyhow::Result;
...
fn on_fetch<'a, 'b, 'c>(
    ...
    let on_ok = ctx
        .effects
        .bind(DataLoaded, on_loaded as Handler<TestState, DataLoaded>);
    ctx.effects
...
fn test_effect_loop() -> Result<()> {
    ...
    let input = ActionInput::EffectOk {
        req_id: env.req_id,
        payload: EffectPayload::InlineBytes(result_bytes),
    };
